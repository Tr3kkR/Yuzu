...
- **`device` is the FOURTH family (`feat/split-a4-device-seam`)** — the narrowest method set yet
  ...
  operator tags `DeviceDetail` carries):** (1) the dashboard's `/devices` list search no longer matches
  on device tags — no test exercised tag-search, so this is a real but minor functional narrowing; the
  device *detail* page still shows operator tags unchanged. (2) `tags` is now ALWAYS present (as `[]`
