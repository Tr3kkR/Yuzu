...
- **The INV-31-4 GLOBAL contract test EXISTS and is green** — `scripts/ci/check-api-parity.py` (whole-tree
  source-parse CI gate) + `tests/unit/server/test_openapi_spec_completeness.cpp` (in-process `TestRouteSink`
  half), shipped out-of-band under **#842 / #3991 / #3992**; `/api/v1` drift is zero (196 routes / 195
  OpenAPI entries / 1 allowlisted CORS `OPTIONS`). **ADR-0031's "that test does not exist today" is STALE**
  (corrected 2026-09-08). So WS-A4's contract-test half is DONE; what remains is the *per-family* seam
  enforcement (gates WS-B2), the handler→API seam refactor, and the PII-audit relocation — do NOT rebuild
  the global test.
...
| **WS-A4** | **Logical seam enforcement + INV-31-4 contract test** — handlers/renderers call the API, never a store pointer; build fails on any registered route absent from the published OpenAPI; **relocate behavioural-PII audit from `*_ui.cpp`/`rest_audit.hpp` to the API call** (audit continuity); resolve interlock (j) by building the test against the literal OpenAPI and saying so. **⚠️ The GLOBAL INV-31-4 contract test + interlock-(j) testability SHIPPED out-of-band under #842/#3991/#3992** (`scripts/ci/check-api-parity.py` whole-tree gate + `tests/unit/server/test_openapi_spec_completeness.cpp`; `/api/v1` drift-zero, 1 allowlisted CORS OPTIONS) — the ADR-0031 "test does not exist" text was stale, corrected 2026-09-08. **REMAINING for WS-A4:** the *per-family* seam+contract enforcement that gates WS-B2, the handler→API seam refactor, and the PII-audit relocation. | A | THIS | WS-A2r, WS-A3 (that family) | **P (per family)** | architect + security-guardian + cpp-safety | **partial** — global contract test done (#842); per-family + seam refactor + PII relocation planned |
