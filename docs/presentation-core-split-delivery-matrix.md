...
- **WS-B1 Drogon canary — Linux green; the Windows MSVC static-link risk, stated concretely so a CI
  ...
  hand-wired static branch. **Two concrete Windows failure modes the canary is designed to surface:**
  (1) **LNK2038 `_ITERATOR_DEBUG_LEVEL`/`RuntimeLibrary` mismatch** — meson's cmake translator reads
  `IMPORTED_LOCATION_RELEASE` regardless of `--buildtype=debug` (the confirmed `#375` translator bug). For
  the C-API DLLs (sqlite3/libpq) that is harmless (CRT-neutral import libs, no STL across the boundary);
  Drogon is a C++ DLL whose API crosses STL types, so a debug (`/MDd`) canary linking Drogon's RELEASE
  (`/MD`) import lib CAN trip LNK2038 where the C libs do not. If Windows debug fails with LNK2038 from
  `drogon`/`trantor`, that is this bug and the fix mirrors `#375` option D (build-type-conditional import
  lib), NOT touching the grpc branch. (2) **c-ares double-linkage** — c-ares is forced STATIC by the
  `#375` override but is also a trantor dependency; on Windows a static c-ares linked into a dynamic
  trantor DLL is normal, but watch for duplicate-symbol (LNK2005) if both the grpc closure and trantor
  pull their own c-ares copy. Linux resolves the `$<LINK_ONLY:...>`-dropped transitive set explicitly
  (OpenSSL/c-ares/zlib/uuid/brotli via pkg-config); Windows dynamic DLLs resolve their own deps at load,
  so that append list is best-effort there. A Windows red here is the gate working — report it, do not
  paper over it.
