...
`result_set_routes.{hpp,cpp}` (#2542 PR-5); and the 4-route Approval API — `GET /api/approvals`,
`GET /api/approvals/pending/count`, `POST /api/approvals/:id/{approve,reject}` — is
`approval_routes.{hpp,cpp}` (#2542 PR-9). All six owner files register against the same
...
Approval API extraction (-4) has also landed.
