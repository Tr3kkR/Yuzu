...
is `execution_routes.{hpp,cpp}` (#2542 PR-7); and the 4-route Approval API — `GET /api/approvals`,
`GET /api/approvals/pending/count`, `POST /api/approvals/:id/{approve,reject}` — is
`approval_routes.{hpp,cpp}` (#2542 PR-9). All eight owner files register against the same
...
legacy pre-v1 Executions extraction (-20, PR-7) landed, and is 34 now that the Approval API
extraction (-4, PR-9) has also landed. 34 registrations remain outside the sink in total.
