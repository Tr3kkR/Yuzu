...
startup. `server.cpp` wires those owners, *and* registers a further **74 routes inline** on
...
`start_web_server()`; and the 5-route Custom Properties API (7.6) —
`/api/agents/:id/properties[/:key]`, `/api/property-schemas` — is
`custom_properties_routes.{hpp,cpp}` (#2542 PR-4), registered against the same `inline_sink`.)
...
extraction (`page_routes.{hpp,cpp}`, 25 routes), and the Custom Properties API extraction
(`custom_properties_routes.{hpp,cpp}`, 5 routes, #2542 PR-4) — both registered against
`inline_sink` — `mcp_server.cpp` is the only remaining route owner registering directly on a raw
`svr.{Get,Post,Delete}` (3 registrations) — plus `server.cpp`'s own 74 inline routes, which are not
a route-owner class and are untouched by this migration; 77 registrations remain outside the sink
...
page-shell extraction (independently re-verified during that extraction), 79 immediately before
the Custom Properties API extraction, and is 74 after it.
