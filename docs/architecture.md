...
startup. `server.cpp` wires those owners, *and* registers a further **69 routes inline** on
...
inline routes, so none of them is reachable from the in-process test harness. (Four surfaces this
...
`page_routes.{hpp,cpp}` (#2542), and a #2542 follow-up split 10 further scattered routes into two
...
`/api/nvd/{status,sync,match}` (3 routes) are `nvd_routes.{hpp,cpp}` — both constructed in
`start_web_server()`.)
...
extraction (`page_routes.{hpp,cpp}`, 25 routes registered against `inline_sink`), and a #2542
follow-up (`dashboard_api_routes.{hpp,cpp}` + `nvd_routes.{hpp,cpp}`, 10 more scattered routes
registered against the same `inline_sink`), `mcp_server.cpp` is the only remaining route owner
registering directly on a raw `svr.{Get,Post,Delete}` (3 registrations) — plus `server.cpp`'s own
69 inline routes, which are not a route-owner class and are untouched by this migration; 72
registrations remain outside the sink in total. Count these with the anchored pattern
...
page-shell extraction (independently re-verified during that extraction), was 79 after it, and is
69 after the `dashboard_api_routes`/`nvd_routes` follow-up.
