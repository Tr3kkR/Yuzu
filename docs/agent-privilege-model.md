...
| `installed_apps.list_per_user` | n/a | n/a | Machine-scope actions above need no privilege. `list_per_user` walks each local profile's registry hive (#2771): loaded `HKU\<SID>` hives are read first with **no privilege**; the offline `NTUSER.DAT` mount fallback rides **`SeBackupPrivilege` + `SeRestorePrivilege`**, already held by the agent account (see the `license_scan.list` row below) — **no new privilege** is introduced. On a hardened install that strips them, the machine-scope actions above still report; only the per-user apps for a *logged-out* profile go unreported, surfaced honestly via `warning\|privilege_missing`. |
