...
|---|---|---|---|
| `quarantine.isolate`, `quarantine.release`, `quarantine.whitelist` | `/sbin/pfctl` | `/usr/sbin/iptables`, `/usr/sbin/ip6tables` | service account is in `Administrators` group OR has `SeAssignPrimaryTokenPrivilege`; uses `netsh.exe advfirewall` directly (no `Set-NetFirewallRule` cmdlet) |
