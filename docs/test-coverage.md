...
| `test_guardian_journal_heartbeat.cpp` | Journal fleet telemetry (item 7 PR-Ag) | Sparse-emit rule (quiescent journal emits no tags), pinned key names + non-zero-only emission, every field has a distinct key (26, designated initialisers so a mid-struct insertion is a compile error, not a silent coverage loss), the capacity/size gauges emit under their keys, and a doc/emitter cross-check binding every `yuzu.guardian_*` name in `docs/user-manual/metrics.md` to a name the emitter actually emits |
