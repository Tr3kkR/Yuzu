...
| `test_viz_routes.cpp` | VizRoutes (`/api/v1/viz/fleet/topology` + `/fragments/...`) | 200 JSON envelope shape, fragment wraps same JSON in `<script type="application/json" id="viz-data">`, 403 perm-denied short-circuit, 503 kill-switch with `denied`/`kill_switch` audit row (DEP-1), 503 store-null with `error`/`store_null` audit, 413 + `oversize` audit + metric increment when materialised count exceeds `machines_max` (M-1), 400 on non-numeric / out-of-range / zero `machines_max`, `?fresh=1` emits separate `viz.fleet_topology.invalidate` audit before the get, cache-miss/cache-hit metric increments observed across two sequential calls, `include_vuln=1` flips JSON `include_vuln` field (13 cases) |
