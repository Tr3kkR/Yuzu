...
| `test_guardian_outbox_drain_worker.cpp` | Guardian outbox drain worker (ADR-0021 rung 7, F6) | drain_once ships everything pending; a throwing send is counted, not fatal (item 4); wake-on-enqueue beats the periodic bound and the bound drains without a wake; stop() idempotent + destructor-driven; a copied waker outliving the worker is a safe no-op. Journal maintenance relocated onto this worker (C0 #2298): paging a durable batch and shipping it in ONE wake (the post-page re-drain), `notify()` as the reconnect kick (prompt, and inert after stop()), the TIME-based prune cadence (20 passes under a one-hour interval prune zero times - the regression guard against a per-wake rule), `request_stop()` making a whole pass a no-op, stop()-during-maintenance joining cleanly, the drain and maintenance firewalls not starving each other, and a `[tsan]` checkpoint racing worker maintenance against a persister (the heartbeat's surviving phase-1) and reconnect kicks |
