...
| `test_mcp_server.cpp` | MCP server | JSON-RPC parsing, tier policy enforcement, token integration, tool dispatch, store interactions, #2405 pre-approval input-schema gate (subset-compiler keyword semantics, mint/consume blocking, boot-validator schema offences, gated-set tether, concurrent validate) |
