...
| `test_kv_store.cpp` | KV storage | Set/get/delete, namespace isolation, list with prefix, clear, persistence across reopens; the item-7 durable-journal primitives — `list_entries` (byte-exact/NUL-preserving/empty-vs-error), `insert_if_absent` (Inserted vs Exists), `rename_key` (Renamed/NotFound/Conflict), `del_keys` (count via RETURNING, all-or-nothing), `pragma_synchronous`, and the #2299 O(work) scan pair — `list_keys_sized` (key order, octet_length byte parity with `list_entries`, empty-vs-error) and `get_entry` (absent vs failed, byte-exact), plus the fallible-read contract itself: a CLOSED handle and a REAL SQLite prepare failure (table dropped via a second raw connection) must both surface as an error rather than as an empty result or a clean absent - the property that distinguishes these two from fail-open `list`/`get`, both mutation-verified |
...
| `test_guardian_lifecycle_journal.cpp` | Engine-owned durable journal (item 7 PR-Ag) | persist() batching + round-trip + circuit-break + committed-prefix-on-failure; retention (age/count/byte caps, oldest-by-(ts_ms,key)) with fact-set clock-guard dedup (#2573 GJ half — a distinct anomaly declines again even while a prior decline's fact set is still recorded, an identical repeat stays suppressed, the recorded set clears once the backlog drains, an idle journal's clock jump is not reported at all), quarantine of unparseable batches + bounded quarantine set, sent-label GC, paging token bucket; fault injection (injected read failure → fail-safe/counted/retried; injected delete failure → M4 intact-and-retried); hard write ceiling refuses runaway growth + resumes after prune (UP-1); over-cap quarantine eviction counted (UP-7); live size gauges track persist + prune; #2299 O(work) pins — a malformed KEY is quarantined by prune while a corrupt VALUE under a good key SURVIVES prune (the lazy-parse guard) and is still bounded by the ceilings, the eviction classifier's per-key fallback when the label scan fails, and an UNREADABLE label counted as neither sent nor unsent; per-call persist bounds - a BATCH cap (remainder persists next call, provenance survives the cap, counts batches on a byte-split push) and a RECORD cap that floors the drain rate when the byte limit splits batches; and the #2299 UP-1 pin that an implausibly-sized value is quarantined by size rather than being allowed to blow the byte ceiling and evict the journal |
| `test_guardian_outbox_drain_worker.cpp` | Guardian outbox drain worker (ADR-0021 rung 7, F6) | drain_once ships everything pending; a throwing send is counted, not fatal (item 4); wake-on-enqueue beats the periodic bound and the bound drains without a wake; stop() idempotent + destructor-driven; a copied waker outliving the worker is a safe no-op. Journal maintenance relocated onto this worker (C0 #2298): paging a durable batch and shipping it in ONE wake (the post-page re-drain), `notify()` as the reconnect kick (prompt, and inert after stop()), the TIME-based maintenance cadence (real journal scans counted through the pre-scan hook while a running worker is hammered with 200 wakes - the regression guard against tying a full-journal scan to wake count), an enqueue-driven wake shipping entries WITHOUT rescanning the journal, `request_stop()` making a whole pass a no-op, a stop fired from INSIDE a scan being honoured mid-pass (not merely at the entry gate), stop()-during-maintenance joining cleanly, the drain and maintenance firewalls not starving each other, maintenance-pass exceptions counted and survived, a bounded drain's truncation/re-drain and its guaranteed compliance share, a drain starting no further sends once stop is requested, a prune failure not swallowing a forced reconnect page, the joined-thread role marker the mtx_ abort keys off, a jammed lifecycle head plus live compliance traffic NOT re-arming the page (the scan-amplification guard), REAL Retain send semantics across a link-down/reconnect cycle and a mid-drain link drop, and a `[tsan]` checkpoint racing worker maintenance against the production phase-1 persist path and reconnect kicks. `[chaos]` (#2345), each observed RED against the unfixed code and each targeting a way a durably-written audit record is deleted without ever being sent: required headroom counted as net-new rather than raw batch size (and a fully-windowed batch reporting neither work nor a blockage, including at the paging pre-check), a forward wall-clock step neither wiping the trail in one pass nor ageing it out faster than a capped rate, a BACKWARD step retaining evidence while the clock-independent count ceiling still bounds the journal, a page-side read failure counted separately from retention's, a rate-limiter-deferred pass reporting WHY it did nothing so a reconnect kick is not dropped, an over-long journal row rejected at the parse boundary, a batch repeating an event_id sized by its distinct records, a full window reporting the SMALLEST pending requirement rather than the first seen, and erase_persisted_prefix identifying the prefix it wrote (both the under-drop and over-drop ends) rather than trusting an index a concurrent overflow drop can shift. CH-10 a delivered batch is not re-offered on an ordinary cadence pass but IS on a forced one (boot/reconnect), so steady-state redelivery of what the server already has is gone; CH-11 a failed sent-label scan falls back to re-offering everything and counts that it did, so the fallback is not silent. Item-12 jitter: the offset is uniform over [0, interval) and safe at zero, a jittered forced page is DEFERRED rather than dropped (observed through the deadline seam, never a wall-clock upper bound) and still runs once the deadline passes, and jitter is OFF unless explicitly enabled so every other cadence test stays deterministic |
| `test_guardian_engine_spark_reconcile.cpp` | Guardian engine spark reconcile + durable journal wiring | Per-rule spark-vs-legacy mutual exclusion, disable/re-enable re-arm, prefer_spark inertness (no journal write, no paging pass, all-zero telemetry), the maintenance tick retrying a failed persist, stop()'s final flush, `page_journal` KICKING the drain worker (backstop pinned out via the timing seam so only the kick can page), PRODUCTION boot order (wire_spark_engine before start_local, racing the boot maintenance scan against the pre-network re-arm), and a forked-child DEATH TEST proving a worker-thread `mtx_` acquisition aborts rather than deadlocking; pending records reaching disk BEFORE `stop()` joins the drain worker, observed while the join is still parked on a blocked send (asserting after `stop()` returns cannot tell the two orderings apart). F7 (#2298): the `Unsupported` terminal state - neither backend armed, `unsupported_rules_` per-type counts (repeat/aggregate/type-change/clear-on-Arm/clear-on-disabled/clear-on-invalid), full_sync omission-clears and retention-stays-idempotent, a production-order restart reconstructing the map from cached KV with no push, `prefer_spark=false`/`SparkDisabled` never populating it, the full transition matrix extended with the Unsupported step, a macOS real-platform-factory test (not the fixture's fake) proving all three mechanisms read unsupported there, and an all-unsupported full_sync push still advancing `policy_generation` rather than holding it (#2797 is a distinct, pre-existing bug class - only a thrown reconcile holds the generation, and Unsupported returns `false` without throwing). #2238 (rung 7.7b test seams): `start_local()`'s per-rule re-arm degrade - a fault hook forces exactly one cached rule's re-arm to throw `std::system_error`, proving the loop still returns success, the OTHER cached rules still arm via spark, and the failure is logged with the rule id (read via `last_rearm_degrade_message_for_test()`, an `mtx_`-locked accessor returning by value over a plain object-member written at the point of emission, rather than a captured-logger swap - the latter proved cross-image-unsafe on a macOS dylib build, where a test-binary-side `spdlog::set_default_logger()` swap never reached the `spdlog::error` call inside `libyuzu_agent_core`; mutation-verified RED with the try/catch removed); `wire_spark_engine()`'s prefer_spark start gate - the convergence scheduler and drain worker are constructed regardless of the flag but started only under it, proven via `started_for_test()` introspection on both classes (mutation-verified RED both ways: always-start, and never-start); and a `[tsan]` checkpoint starting the drain worker with a real `this`-capturing send and tearing the engine down while a send may be in flight, racing `stop()`'s join. |
| `test_guardian_journal_heartbeat.cpp` | Journal fleet telemetry (item 7 PR-Ag) | Sparse-emit rule (quiescent journal emits no tags), pinned key names + non-zero-only emission, every field has a distinct key (26; designated initialisers make a field REORDER a compile error - an omitted designator is legal and value-initialised, so an INSERTION still requires bumping the expected count by hand), the capacity/size gauges emit under their keys, and a doc/emitter cross-check binding every `yuzu.guardian_*` name in `docs/user-manual/metrics.md` to a name the emitter actually emits |
| `test_guardian_unsupported_heartbeat.cpp` | `mech_unsupported_total` per-mechanism fleet gauge (F7, #2298) | Empty counts emit no tags; a zero-count entry stays omitted (sparse, belt-and-braces); the pinned `yuzu.spark_<mechanism>_unsupported` key format for all three mechanisms; a 2→1→0 shrink sequence proving the emitter appends rather than clears (a fresh tag map is required per simulated call - reusing one map would falsely look latched) |
| `test_guardian_backend_heartbeat.cpp` | `yuzu.guardian_backend` heartbeat tag (F7, #2298, #2240 item 1) | `guardian_backend_from_state`'s full 8-combination `(prefer_spark, SparkAvailability)` matrix (not just representative cases); pinned label strings for all four `GuardianBackend` values; always-present emission (never sparse, unlike the counter families) |
...
| `test_audit_store.cpp` | Audit store (PostgreSQL, ADR-0040) | Log/query, filter by principal/action/target, pagination, timestamp ordering; action-prefix scoping + fail-closed wildcard prefixes; a degraded read distinguishable from an empty one; detail sanitisation (read-time and during the copy); the mandatory legacy-`audit.db` backfill — streamed, idempotent, resumable, reconcile shortfall, prefix proof incl. the contiguous-ids foreign table, WAL/SHM sidecars, the sourceless exits over a non-empty table, and the no-native-row-before-the-marker order contract; the retention clock guard (#2360/#2579) on a single-sweeper advisory lease with durable dedup — bootstrap decline, per-pass cap, corrupt/implausible readings, forward skew, restart via durable meta, sibling-lease skip; the retention-liveness gauge seed (#2854) — restores from the durable anchor at construction and after the legacy backfill on the same instance with no restart, negative dead-CMOS readings seeded unclamped, corrupt/implausible readings seeded to a distinct anomaly sentinel rather than laundered to 0 (the seed's own connection/query-failure branches are untested — #2854 follow-up) |
...
| `test_mcp_server.cpp` | MCP server | JSON-RPC parsing, tier policy enforcement, token integration, tool dispatch, store interactions, #2405 pre-approval input-schema gate (subset-compiler keyword semantics, mint/consume blocking, boot-validator schema offences, gated-set tether, concurrent validate), #2437 `execute_instruction` input bounds (per-field reject + exactly-at-cap accept on the **operator** tier, where no approval gate runs; `{tool,reason}` counter delta per bound; served-schema↔handler-constant cross-check; transport-cap sizing figures pinned to the published docs number; supervised-tier pre-mint enforcement of the two schema-inexpressible bounds, asserting `pending_count()` does not move), #2444 gated-tool schema tightening (pattern/maxLength bounds compile-validated against the real served schemas for `revoke_certificate`/engine-principal/`confirm_engine_rotation`/`quarantine_device`; a `minLength:1` sweep across ~30 required-string arguments, data-driven-sampled across every distinct tool family; `execute_bundle` empty nested step rejected pre-mint; `yuzu_mcp_approval_burned_total{tool,reason}` fires on a genuine post-consume handler reject and stays at 0 for schema-invalid/success) |
| `test_api_token_store.cpp` | API token store | Token CRUD, expiration, MCP tier assignment |
| `test_agent_health_store.cpp` | Agent health store | Health status tracking, query, TTL; spark fleet rollup against the REAL store + recompute_metrics (four postures, inert exclusion, absent-not-zero — mutation-proved: deleting the disabled/failed emit loops goes red while the mirror-based cases stay green); Guardian journal fleet rollup against the REAL store (sum across agents, absent-not-zero, rogue-value rejection, meta-signals published at 0, explicit-zero publishes as 0, staleness prune drops a reporter, one agent mixing parseable and rejected tags (length gate and ceiling through the store) - mutation-proved: deleting the publish loop reddens all three sum cases, deleting the clear loop reddens the absence and staleness cases). F7 (#2298): `yuzu_fleet_spark_unsupported` against the REAL store - fleet sum across two agents, current-gauge decrease (not latched, unlike its cumulative siblings), an explicit-"0" report publishing a PRESENT zero-valued series (contrasted with sparse-emit omission, which leaves the series absent), a never-reported mechanism staying absent, staleness dropping a reporter, and the four per-mechanism spark gauges (`watch_rejected`/`quarantined`/`slow_op`/`unsupported`) never cross-attributing a value from one metric onto another |
| `test_guardian_journal_fleet_tags.cpp` | Guardian journal fleet-tag contract (server↔agent pin) | Structural `static_assert` pinning `sizeof(GuardianJournalStats)` to the table row count plus `has_unique_object_representations_v` (no-padding premise compiler-checked); bidirectional writer↔reader key bind against the agent's real emitter; per-key expected-VALUE bind catching a field/key swap the key-set checks cannot (mutation-proved); sparse-emit contract; mechanical tag→gauge name rule; count-parse forged-value posture (full-token, negative/fractional, 1e9 plausibility ceiling and the 10-digit length gate - the gate runs FIRST, so it is what rejects a UINT64_MAX token (20 digits, refused before parsing) and also what makes true from_chars overflow unreachable; the ceiling only rejects 10-digit values above 1e9) |
| `test_spark_fleet_tags.cpp` | Spark fleet-tag contract (server↔agent pin) | static_assert key pins (all 7 fixed keys), writer↔reader runtime bind against the agent's real emitter (spark_heartbeat.hpp), four-posture wire contract, mechanism-CSV closed-set/dedup/64-byte cap, count-parse forged-value posture (full-token, negative/overflow/1e9-plausibility, 10-digit length cap incl. a 4 MiB token). F7 (#2298): `kSparkMetricUnsupported` key pin + `spark_type_metric_tag` composition, and a SEPARATE writer↔reader bind against the real `emit_guardian_unsupported_heartbeat_tags` (a distinct data source from the other three metrics, not folded into the SparkEngine-stats bind case) |
...
| `test_notification_store.cpp` | `NotificationStore` (born-on-PG, ADR-0046) | In-app notification CRUD, read/unread status; ADR-0009 backfill idempotency, id-preserving insert + sequence advance, holder-side fingerprint verification (matching-fingerprint retry, HOLDER-SIDE mismatch refusal, sourceless-sibling refusal), pre-backfill emptiness guard against a conflicting-row regression (live PostgreSQL) |
...
| `test_webhook_store.cpp` | Webhooks (Postgres, ADR-0057) | Subscription CRUD, HMAC-SHA256 signing, delivery, the ADR-0010 secrets seam (has_secret invariant, tamper→skip-not-unsigned-fire, no-secret→unsigned fire preserved), ADR-0009 legacy-SQLite backfill (id preservation, decrypt-and-compare verification, orphan/fingerprint handling, WAL/SHM sidecar 0600 restriction at both read- and move-time, sidecar-rename-failure resilience) |
| `test_webhook_routes.cpp` | Webhooks REST surface (`/api/webhooks`, ADR-0057) | 400-vs-503-vs-404 status classification per route, audit_fn coverage on every success AND failure branch (create/delete), the pre-delete URL snapshot in the delete audit detail, 403 on permission denial, 503 on a null/unopened store |
