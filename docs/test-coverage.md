...
| `test_win_str_utils.cpp` _(Windows-only)_ | Shared `yuzu::win` wide<->UTF-8 helpers (#1681): `agents/plugins/shared/win_str.hpp` | `to_wide`/`from_wide` round-trip preserving "Café"; empty + null input; 512-wchar value with no terminator (#652); lone surrogate → U+FFFD; `reg_sz_to_utf8` trailing-NUL strip (none/one/two); non-wchar-multiple size floor; null buffer + empty payload |
