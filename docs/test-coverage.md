...
| event_logs | full `event_logs_parsers.hpp` surface (rendered-event XML walk, journal rows, short-iso parse — `test_event_logs_parsers.cpp`) + `event_logs_macos.hpp` `log show` classifier (`test_event_logs_macos.cpp`); real-plugin dispatch runs in `test_event_logs_{posix,win}_actions.cpp` | wevtapi EvtQuery/EvtNext, sd_journal reads, `log show` invocation |
