...
| `test_agent_health_store.cpp` | Agent health store | Health status tracking, query, TTL; spark fleet rollup against the REAL store + recompute_metrics (four postures, inert exclusion, absent-not-zero — mutation-proved: deleting the disabled/failed emit loops goes red while the mirror-based cases stay green); Guardian journal fleet rollup against the REAL store (sum across agents, absent-not-zero, rogue-value rejection, meta-signals published at 0, explicit-zero publishes as 0, staleness prune drops a reporter - mutation-proved: deleting the publish loop reddens all three sum cases, deleting the clear loop reddens only the absence case) |
