...
| `test_guardian_journal_heartbeat.cpp` | Journal fleet telemetry (item 7 PR-Ag) | Sparse-emit rule (quiescent journal emits no tags), pinned key names + non-zero-only emission, every field has a distinct key (22), the capacity/size gauges emit under their keys |
