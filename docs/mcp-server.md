...
- **MCP Streamable HTTP transport** (`Mcp-Session-Id` session lifecycle, GET SSE channel with `Last-Event-ID` resume, `notifications/progress` for long-running tools) — committed platform direction, ADR-1005 execution plan **Decision 15 / track 2f** (five-rung PR ladder; design commitments including the security pre-commitments live in that decision entry; chaos merge-gate map: `docs/mcp-streamable-http-chaos-design.md`). Session/stream state will be in-memory only and deliberately does not gate `/readyz`.
