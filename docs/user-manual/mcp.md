...
| 107 | `create_result_set_from_inventory_query` (#2146 Batch B2) | Create a result set from a SYNCHRONOUS inventory query — evaluates `conditions` against every agent's stored inventory server-side; membership is every match, optionally narrowed to an owned `parent_id` set's CURRENT members. Gated on a REAL `Inventory:Read` `perm_fn` check (same securable `query_installed_software` uses) — not a dispatch, so no #1788 confinement applies; matches the REST twin's own unconfined posture exactly (deliberately not "fixed" here, to avoid a false `confined` claim the REST twin does not back). A capped/truncated inventory read is refused (503) rather than materialised as a partial set (#2500/#2492 class). Mirrors `POST /api/v1/result-sets/from-inventory-query`. | `Inventory:Read` |
