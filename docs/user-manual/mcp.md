...
| 51 | `discover_plugins` | A2 discovery: plugin/action catalog observed across currently-connected agents. NOT a build-time manifest. Catalog `version: 3` (v3 adds a per-plugin `docs` summary `{summary, kind, platforms, readme, resource}`, or `null`, pointing at `yuzu://plugin-docs`): each action carries an inline `parameter_schema` when it has a published `InstructionDefinition` (matched on plugin+action) **and** the caller holds `InstructionDefinition:Read`; otherwise name+description only (an `Infrastructure:Read`-only caller gets no schemas). A top-level `actions_enriched_with_schema` counts the enriched actions. Mirrors `GET /api/v1/discover/plugins`. | `Infrastructure:Read` |
