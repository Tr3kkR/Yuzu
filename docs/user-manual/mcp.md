...
| 105 | `get_pending_approval_count` (#2146 A2-R4) | Count pending approval requests. Mirrors `GET /api/v1/approvals/pending/count` and the legacy `GET /api/approvals/pending/count` — all three share `ApprovalManager::pending_count_checked`, so a genuine store failure answers with a retryable error rather than a false zero count. | `Approval:Read` |
