...
| 115 | `unlock_account` (#2146 Batch B4) | Clear a local account's failed-login lockout counter (SOC 2 CC6.3 operability path - the lockout also auto-expires on its own). Self-target is permitted. Requires the Postgres auth store (AuthDB) - answers "lockout subsystem unavailable" if not wired, same as REST. Mirrors `POST /api/v1/users/{username}/unlock`. **Approval-gated at the supervised MCP tier - same cookie-session bypass as `create_api_token` above (#4309).** Destructive: overwrites the account's existing lockout/failed-login state. | `UserManagement:Write` |
