...
| 74 | `get_dex_app` (#4035) | App blast-radius drill: crash/hang summary, faulting modules, exception codes, and the affected-device list for one application (required `name`). The `devices[]` array names agent IDs fleet-wide (individual-identifying), so a service-scoped API token is denied outright and every call emits a fail-closed `dex.app.view` audit (`audit_persisted:false` surfaced on a dropped row — MCP's set-and-proceed equivalent of the REST twin's `Sec-Audit-Failed` header). Mirrors `GET /api/v1/dex/app`. | `GuaranteedState:Read` |
