...
| 52 | `query_software_licenses` | SLE (ADR-0024): fleet software-**licence** posture rows from the precomputed rollup — one row per product with per-state counts (`licensed`/`subscription_active`/`trial`/`grace`/`expired`/`unlicensed`/`unknown`), device/install counts, and expiry outlook. Filters: `state` (incl. `lapsed` = expired ∪ unlicensed), `q`, `expiring_within_days`, `limit` (1..1000, default 100; truncation flagged via `result_truncated_by_cap`). **Pinned global** (rows carry no agent id — a management-group-confined principal is denied at the gate, never served a partial rollup; the per-device drill is REST `GET /api/v1/sle/agents/{id}`). A degraded store is an **error**, never an empty success. Distinct from `query_installed_software` (the installed-software catalogue) and from Yuzu's own product licence (`License:*`). Mirrors `GET /api/v1/sle/licenses`. | `SoftwareLicensing:Read` |
| 53 | `get_license_compliance_summary` | SLE (ADR-0024): the fleet licence-compliance headline — `products`, `installs`, `lapsed_products`, `expiring_soon` (30-day window), `next_expiry_at`, plus the posture `as_of` stamp. `evaluated:false, as_of:0` = the compliance evaluator has not produced a rollup yet (honest empty, not a zero estate). Same numbers as `GET /api/v1/sle/summary` (shared aggregation — the two surfaces cannot drift). Pinned global; degrade → error. | `SoftwareLicensing:Read` |
