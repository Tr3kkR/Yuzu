...
| 18 | `list_executions` | List recent command executions (filterable by definition, status). **Confined (#1634):** gated by `require_fleet_read`; execution rows carry no single `agent_id` to filter by, so a confined caller sees only executions they themselves dispatched, not a full per-row visible-agent check. | `Execution:Read` |
