...
| 37 | `query_installed_software` | Query the typed installed-software inventory from the agent daily-sync framework (ADR-0016): machine-wide packages (name, version, publisher, install_date) per device, fleet-wide. Filter by `name` and/or `agent_id`; returns up to `limit` rows (max 1000). **Results are management-group scoped** — out-of-scope devices are omitted (and the omission audited `result=denied`); `result_truncated_by_cap:true` flags a partial page (keyset follow-up). **Distinct from `query_inventory`/`get_agent_inventory`** (generic blob store, `Infrastructure:Read`). | `Inventory:Read` |
