...
| 19 | `list_schedules` | List scheduled (recurring) instructions — a service-scoped API token is denied outright (`schedule.list`). **(#4030):** rows now also carry `execution_count`, matching the dashboard fragment and `GET /api/v1/schedules`. **(#2146 A2-R1):** optional `definition_id`/`enabled_only` filters, matching `GET /api/v1/schedules` exactly (`enabled_only` is a real boolean on both — unlike the legacy unversioned `GET /api/schedules` route, where any presence of `enabled_only` is treated as true regardless of value). | `Schedule:Read` |
