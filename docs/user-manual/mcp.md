...
| 157 | `get_pending_approval_count` (#2146 A2-R4) | Count pending approval requests. Mirrors `GET /api/v1/approvals/pending/count`, which shares `ApprovalManager::pending_count_checked` with this tool, so a genuine store failure answers with a retryable error rather than a false zero count. The legacy `GET /api/approvals/pending/count` is NOT migrated onto the checked path - it still returns a bare `{"count":0}` on a degraded store; prefer this tool or the v1 route for failure-honest polling. | `Approval:Read` |
