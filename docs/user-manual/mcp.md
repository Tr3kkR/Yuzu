...
| 51 | `discover_plugins` | A2 discovery: plugin/action catalog observed across currently-connected agents. NOT a build-time manifest; per-action parameter schemas are not available here (see `discover_instructions` for actions that also have a published definition). Mirrors `GET /api/v1/discover/plugins`. | `Infrastructure:Read` |
