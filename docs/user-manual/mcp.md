...
| 110 | `get_host_topology` (api-parity #2146 Batch B3) | Per-host slice of the fleet topology — one machine's processes, connections, and listening sockets, keyed by `agent_id`. Mirrors `GET /api/v1/viz/host/{id}/topology` exactly, reading the same `FleetTopologyStore` cache `get_fleet_topology` uses. Unlike `get_fleet_topology`, this tool does NOT fall back to a durable stale placeholder for a host that aged out of the cache (matches the REST twin, which never consults the offline-endpoint store) — `kInvalidParams` ("host not found") means "not in the live snapshot right now", not "never existed". Same kill-switch/audit posture (`viz.host_topology`) as `get_fleet_topology`. | `Response:Read` |
