...
| 72 | `list_tar_process_tree_devices` (#4027) | List the operator-scoped device picker for the TAR process-tree viewer (online and offline devices; each row carries `online`). Read through the SAME per-operator management-group-scoped provider as the REST twin and the `/fragments/tar/process-tree` HTML picker — not `list_agents`' fleet-wide `agents_fn`. Mirrors `GET /api/v1/tar/process-tree`. | `Infrastructure:Read` |
