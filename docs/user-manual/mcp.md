...
| 19 | `list_schedules` | List scheduled (recurring) instructions — a service-scoped API token is denied outright (`schedule.list`). **(#4030):** rows now also carry `execution_count`, matching the dashboard fragment and `GET /api/v1/schedules`. **(#2146 A2-R1):** optional `definition_id`/`enabled_only` filters, matching the legacy `GET /api/schedules` route and threaded into the same `ScheduleQuery` as `GET /api/v1/schedules`. | `Schedule:Read` |
