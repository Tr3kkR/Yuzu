...
- `/healthz` (authenticated) reports `stores.guaranteed_state` **and `stores.baselines`** alongside the other store-health entries. Both stores are on the shared Postgres substrate (schemas `guaranteed_state_store` / `baseline_store`, ADR-0038 / ADR-0055); if either fails to migrate/open, the server reports `status = "degraded"`.
