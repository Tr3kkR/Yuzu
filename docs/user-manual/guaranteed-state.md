...
Runtime inert is a state the agent enters and leaves by itself, and the failed-pass counters are test-only (nothing on the box or the server exposes them), so the agent log is the on-box evidence and `yuzu.spark_mechs` (with its fleet gauge) is the only signal the server sees. The agent log is not flushed per line, so read the second bullet before trusting a quiet log.

- **Where the log is.** The shipped Windows installer registers the service with `--log-file {app}\logs\yuzu-agent.log` (`{app}` is the install directory); `sc qc YuzuAgent` shows the command line, and so the log path, actually in effect. A service registered by hand without `--log-file` writes `<data_dir>\yuzu-agent.log`, and an interactive (non-service) agent logs to the console unless `--log-file` is set. Rotation and the other log flags are in the agent flag table in [Device Management](device-management.md) (`--log-file`, `--log-max-size`, `--log-max-files`).
- **The log is not flushed per line.** The agent sets no flush level or flush interval on its logger, so lines pass through the sink's write buffer and can lag behind the event; they are guaranteed to be written at rotation and when the service stops (`service_win.cpp` flushes once `run()` returns), and a crash or a hard exit can lose the buffered tail. An empty search on a quiet agent therefore proves nothing: the absence of `worker pass failed` lines does not show a boot-time inert. The fleet series and a service stop or restart are the reliable signals.
