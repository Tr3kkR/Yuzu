...
- **A blocked log sink stalls the worker.** The failure and recovery lines are written on the File worker's (or Registry sweeper's) own thread, so a blocked or slow log sink (a stalled pipe, a hung log volume) stalls that worker, and with it IOCP draining, for as long as the sink blocks. Only agent shutdown is bounded: a blocked sink there ends in a forced exit after about 20 s. This is a known limit, recorded in [R5.7 (g)(4) of the Guardian and Spark design](../spark-stage2-guardian-consumer-design.md).
