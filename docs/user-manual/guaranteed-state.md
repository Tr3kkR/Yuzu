...
- `Guardian T_server` (server), `Guardian T_wire` and `Guardian T_detect` (agent) are `info`-level diagnostic log lines (#4606): per-event latency waypoints for the Spark cutover benchmark, correlated by `event_id`. They are plain log lines, not metrics or an audit trail - no alert or dashboard consumes them. Volume tracks Guardian event volume; `--log-level warn` suppresses them (and every other `info` line). The fields, the rule that a correlator must join on `event_id` and never on log-file order, and how to read an orphaned `T_detect` line are in the header comment of `agents/core/src/guardian_spark_timing.hpp`; the operator-facing summary is the Upgrade note in [server-admin.md](server-admin.md).
