...
| **Guard** | The agent-side component that watches a kernel signal (Windows registry change, service SCM transition, ETW event; Linux systemd unit-state via sd-bus) and evaluates the rule. The `registry` guard is Windows-only and live; the `file` guard is live on **Windows** (`ReadDirectoryChangesW`) and **macOS** (FSEvents); the `service` (run-state) guard is live on **Windows** (SCM `NotifyServiceStatusChange`) and **Linux** (systemd sd-bus `ActiveState` watch, **observe-only** — enforce is Windows-only for now). Service start-type, ETW, further macOS guards (Endpoint Security), and non-systemd Linux guards are on the roadmap. |
...
Three guard types ship today: the **registry** guard (above, Windows-only), the **file** guard (below, Windows + macOS), and the **service** guard (below, Windows + Linux/systemd). On Linux the service guard is **observe-only** — it detects and reports drift but does not start/stop units; an `enforce`-mode rule on a Linux agent degrades to observe (drift is reported, never remediated), with no error. macOS Endpoint Security guards, the remaining Linux event guards (inotify/netlink), and non-systemd Linux fallback are on the roadmap.

## File guards — detect a file changed or deleted, in realtime

A `file-change` spark watches a file via `ReadDirectoryChangesW` on its parent directory (Windows) or an FSEvents stream on the parent (macOS) — kernel-notified, **no polling**, so detection is realtime. The watch is resilient: it survives the parent directory (and its whole ancestor chain) being deleted and recreated. One macOS caveat: FSEvents does not report **remote-origin** changes to targets on network volumes (SMB/NFS); the guard re-verifies real state every 15 minutes as a liveness backstop, so such drift is detected on that cadence rather than in realtime. Two assertions decide what counts as drift:
