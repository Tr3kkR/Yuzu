...
A `file-change` spark watches a file via `ReadDirectoryChangesW` on its parent directory — kernel-notified, **no polling**, so detection is realtime. The watch is resilient: it survives the directory holding the guarded file, or its nearest existing ancestor, being deleted and recreated anywhere in that ancestor chain. A rename or move of the watched directory itself is detected separately, via a watch on that directory's own immediate parent — but nothing watches that parent's own parent, so a rename or move of the parent itself, or of anything above it, is not detected in real time. Recovery in that case depends on the *relocated* directory's own content next changing (not on any activity at the original path) — a clean substitution that recreates a new directory at the original path, while leaving the relocated original untouched, can go undetected indefinitely. A rename or move of the watched directory is evaluated like any other change to the guarded path: a guarded file that is no longer at its original path is reported as absent under a `file-exists` rule that expects it present and under a `file-hash-equals` rule, and changes made after the original path is recreated are detected. This behaviour is validated on local NTFS; it also functions over SMB in testing, via the plain name-match fallback used when the extended, FileId-bearing notify API is unavailable, but has not been validated against a production Windows/NAS file server.

**Known degraded mode.** The watch on the directory's own parent is retained across ordinary re-arms and only rebuilt when the watched directory's identity actually changes; if a rebuild's teardown of the outgoing watch cannot be confirmed within a bounded wait (normally only a wedged filesystem driver, minifilter, or sustained packet loss on a network share), that watch generation is abandoned rather than reused. This condition has been reproduced in testing: holding a watch on an SMB share and inducing packet loss on that connection, a frozen SMB server or a fully severed connection both still confirmed cancellation well under the bound (both fail fast, locally), but sustained mid-path packet loss — a blackhole window on an otherwise-established connection, not a crash — reliably exceeded it, consistent with the client falling back to TCP's own retransmission timeout with no faster local signal available. After 3 such abandons for one rule, this parent-level rename detection is **permanently disabled for that rule** — logged once, at `error` level, naming the rule and path (`"parent-directory watch for {rule} ... permanently disabled after {n} drain failures"`) — until the rule is next re-armed (a policy re-push or an agent restart). Each level of a directory chain created one level at a time beneath the guarded path, and each replacement of the watched directory that is observed mid-change, performs one bounded teardown of the outgoing watch; where cancellation takes longer than the bound (sustained packet loss on a network share is the realistic case, per the measurement above), each one counts toward the limit, so it can be reached within a single such episode rather than only over three unrelated ones. This does not affect the guard's primary detection (the file's own presence or content, evaluated on every wake); only the specific ability to detect a rename or move of the watched directory itself is lost. **On the transition into this state the rule reports itself unhealthy** — the device page's per-rule compliance census shows it `errored` (the same red state as an ordinary unresolved drift), with an operator-facing detail explaining the cause and the fix ("Rename/move detection for this rule is disabled after repeated failures to reset its file-system watch (often caused by a slow or unreliable network share). This rule's file presence/content checks are unaffected. To restore rename detection: re-push this rule's policy, or restart the agent." — also retrievable via [`GET /api/v1/guaranteed-state/events`](#5-query-events-and-status)), and the agent re-sends that report every 5 minutes for as long as the agent keeps running and the rule stays disabled, so a single lost report does not leave the census stuck showing an earlier, stale state (an agent crash or restart of this specific watch is a separate condition, corrected the same way any other agent restart is — a fresh policy sync). While disabled, a compliant evaluation of the file's own presence/content is never published as `guard.compliant` — it is replaced by the same unhealthy report — so the rule cannot show 100% compliant while rename detection is dead; ordinary drift on the file's own presence/content is still reported normally. **To recover:** [re-push the rule's policy](#4-push-to-agents) (`POST /api/v1/guaranteed-state/push`), or [re-deploy the Baseline it belongs to](#baselines--how-guards-reach-agents) from the dashboard, or restart the agent — the rule's first verdict after any of these clears the state.

**Other known limitations.** A forced dismount of the volume holding the watched directory, and a change to a mount point/reparse point along the watched path that does not itself rename any watched directory, are not guaranteed to be detected by the mechanisms above; both are pre-existing, architectural gaps distinct from the rename-detection behaviour described above.

Two assertions decide what counts as drift:
...
| `file-hash-equals` | the file's **content** differs from a baseline | `path` (required), `expected_hash` (64-char SHA-256 hex; **empty → baseline captured on arm**), `max_bytes` (hashing cap, default 64 MiB → larger files report `<oversize>`; **hard ceiling, currently 1 GiB** — a larger value, in either JSON wire form, is rejected at authoring time (#2233 item 6); a rule already stored above the ceiling before this check existed is not retroactively rejected — the agent silently clamps it on arm, with a warning re-logged on each arm/reconcile while it stays over the ceiling), `settle_ms` (quiescence window before hashing, default 750) |
...
A baseline captured on arm (empty `expected_hash`) is **persisted per rule** on the agent and re-asserted on every later arm — a fleet-wide policy change elsewhere, or an agent restart, does not re-capture the target's then-current content as a new baseline (#4021). The original baseline holds until the rule's `path` is deliberately changed (which is treated as a genuinely new target). Authoring an explicit `expected_hash` *shadows* the persisted baseline rather than replacing it — the rule compares against the authored value while it's set, but the original captured baseline is neither deleted nor recomputed; removing the authored `expected_hash` later restores that original persisted baseline, not a fresh capture of whatever the target holds at that point. If the agent's local KV store is unreadable at the moment a rule arms, that single arm falls back to capturing current content as if no baseline existed (logged as an error). The persisted record itself is protected against that fresh capture overwriting it *if* the store is readable again by the time the capture completes — the agent re-checks before writing and refuses to replace an existing record for the same target. A KV outage spanning BOTH the arm-time read and the capture-time write is a narrower case where the fresh capture is written anyway (also logged), since the alternative would leave the rule permanently without a persisted baseline; a sustained, not merely momentary, local KV outage can therefore still record a new baseline over a genuinely drifted one.
...
>
> **Named exception (#4855): the per-device DEX experience-score read fails CLOSED.** `dex_device_signal_summary` — the read behind the device-page DEX lens, `GET /api/v1/dex/devices/{id}`, and MCP `get_dex_device_score` — is the ONE DEX/analytics read that does NOT follow the "#2659 interim, degrade to empty" posture above: a degraded read now surfaces as a degraded placeholder on the lens ("DEX store degraded."), a `503` (`retry_after_ms: 2000`) on REST, and a retryable error on MCP — never a fabricated healthy score of 100 with no signals. Every OTHER `/dex` analytic read (crash/signal/app aggregations, the fleet overview/health composites) still degrades to empty per the paragraph above; the `/dex` overview and health panels additionally now COUNT connected devices whose per-device score could not be computed ("N device(s) could not be scored") rather than silently thinning the great/fair/poor population.
...
`GET /api/v1/guaranteed-state/rules`, `.../events`, and `.../status` each have an MCP twin (`list_guardian_rules`, `list_guardian_events`, `get_guardian_status`, #4037) calling the exact same store/builder as their REST sibling — an agentic worker sees the identical rule catalogue, event stream, and rollup a human operator sees over REST or the dashboard. Two more read surfaces are new with #4037 and have no dashboard-fragment sibling of their own shape: `GET /api/v1/guaranteed-state/rules/{rule_id}/status` / MCP `get_guardian_rule_status` (the per-guard fleet-wide agent-status drilldown — every agent's state for ONE rule) and `GET /api/v1/guaranteed-state/agents/{agent_id}/rules` / MCP `get_guardian_device_guards` (the per-device all-guards view — every guard's state for ONE device, unscoped to any Baseline; contrast with `device-compliance`, which is scoped to one named Baseline). **#2146 Batch B1 adds MCP twins for the remaining write and per-device surfaces**: rule create/get/update/delete (`create_guardian_rule`/`get_guardian_rule`/`update_guardian_rule`/`delete_guardian_rule`), push (`push_guardian_rules`), per-agent status (`get_guardian_agent_status`), and device-compliance (`get_guardian_device_compliance`) — every REST route on this page now has an MCP twin. See [`docs/user-manual/rest-api.md`](rest-api.md#guaranteed-state) and [`docs/user-manual/mcp.md`](mcp.md) for the full contract of each.
...
>
> **The same deny applies on the write side.** Creating a Guard, flipping its enable/disable toggle, and creating, editing, deploying, or deleting a Baseline are the same 403 for a service-scoped token, regardless of the `GuaranteedState:Write`/`Push`/`Delete` grant it carries — a service-scoped token cannot deploy a Baseline (the fleet-wide operation that determines what every agent enforces). Each mutation's own denial reuses that action's existing success-verb audit row with `result=denied` (`guaranteed_state.rule.create`, `.rule.update`, `.baseline.create`, `.baseline.update`, `.baseline.deploy`, `.baseline.delete`) rather than a separate generic verb — see [Audit log](audit-log.md).
>
> **Offline convergence:** an agent that was offline or unreachable when the deployed set changed re-converges automatically on reconnect. It reports its applied policy generation (`yuzu.guardian_generation`) in each heartbeat; the server re-pushes when that generation is behind the current store generation (rate-limited per agent). No manual re-push is needed after a network partition. Once the Spark-backed detection path is live, the same convergence can take one or two additional heartbeat ticks under a slow-arming rule — an accepted arm is tracked by an internal acknowledgment ledger and the generation only advances once the arm is confirmed, instead of the agent blocking on it synchronously. This has no visible effect today: a generation advances exactly as it always has while the Spark path stays off by default.
...
The Baseline surface is on the dashboard (`/guardian` → Baselines). Its **read side has a first agentic entry**: `GET /api/v1/guaranteed-state/device-compliance?baseline={name}&agent_id={id}` looks a Baseline up by **name** and returns the Guards applicable to a device plus that device's per-Guard verdict (see [REST API](rest-api.md) → Guaranteed State) — built for embedding compliance into an external CMDB/ITSM CI record (the integration pins one stable Baseline **name** such as `ServiceNow Compliance`, so there is no per-environment `baseline_id` to track; a scripted baseline-list endpoint is a tracked follow-up). **Create, edit, deploy, and delete remain dashboard-only** (tracked as an agentic-first parity gap). Scripted/agentic clients can **author** individual Guards via REST or MCP (#2146 Batch B1 added the MCP twins for rule create/get/update/delete and push), but **enforcement requires those Guards to be members of a Baseline an operator deploys** — authoring a Guard (or calling `POST /push`/`push_guardian_rules`) never enforces a standalone Guard, and the deployable-collection authoring model itself is GUI-only for now.
...
All four actions above additionally deny a service-scoped API token outright (403), regardless of the securable grant it carries — see the note in [§6](#6-enabledisable-a-guard-from-the-dashboard).
...
The same rollup also folds in a synthetic **not implemented** count: an (agent, rule) pair whose platform can't arm that Guard's *type* — a per-guard-type matrix (`service-status-change` arms, observe-only, on Windows **and** Linux; `registry-change`/`file-change` are Windows-only; nothing arms on macOS today), never a blanket "non-Windows" rule — and which doesn't already own a real status row. It counts toward the denominator, never toward compliant, so a fleet with unenforceable pairs never silently reads as 100%. The Fleet view's honesty banner names the agents affected **pair-level**, not platform-level — an agent whose only deployed Guard type it *does* support is never flagged just for being on a less-covered platform. Two known, accepted edge cases of this fold (#4252):
- A Service guard that never actually arms on a genuinely-supported platform (no system D-Bus — every containerized/compose Linux agent — a disabled build flag, or an invalid unit name) reports nothing and so is indistinguishable from an unreported pair: it silently drops out of the denominator rather than showing "not implemented". If a compliance number looks better than expected for a Linux fleet, check whether its Service guards are actually arming (D-Bus reachability on the endpoint), not just whether the dashboard flags them.
- A status row surviving a guard-*type* revision on the same rule id (e.g. re-authoring a Service rule as Registry) can render a now-unenforced pair as compliant with no "not implemented" marker on that rule — though the `yuzu_server_guardian_platform_matrix_stale_total` counter below still fires on it. Tracked as #4263, not yet fixed.
...
**This is `prefer_spark`-gated and currently inert in production (`prefer_spark=false`).** It is documented here ahead of that flag flipping so a pilot's network monitoring is not surprised by it later; see [Behaviour changes](upgrading.md) for how you'll be told when that flag actually flips.

Once active: on every agent reconnect (a new `Subscribe` stream), the agent re-sends its full backlog of not-yet-expired lifecycle-journal entries to the server. There is no delivery cursor, so an entry keeps being re-sent - on every reconnect, and periodically while connected - until it ages out of the retention window below. If your SOC or NDR tooling watches for reconnect-correlated outbound bursts, this is genuine, deliberate traffic, not beaconing: it rides the **existing agent-server gRPC/mTLS `Subscribe` stream** (the same stream carrying commands and other Guardian events); there is no new port, protocol, or egress destination.

There is no dedicated on/off switch for this traffic specifically. Once `prefer_spark` is active, its only lever is **`--spark-disable`** (see [SparkEngine](#sparkengine--the-next-generation-detection-engine-observe-only) below) - a boot-time agent setting that also turns off spark-based detection, not just journal replay.

**Registry detection under spark (also `prefer_spark`-gated, inert today).** Documented ahead of the flip for the same reason. Once active, a change to a watched registry key produces the usual `drift.detected` evaluation immediately and one further re-evaluation a few tens of milliseconds later, when the mechanism has re-armed its notification (two invalidation hints for one change; the second re-reads the key and, with the default 1000 ms `event_debounce_ms`, folds into the next drift row's `collapsed_count` rather than producing a second row - a rule debounced below roughly 200 ms shows a second, uncollapsed drift row instead). A key that takes longer than 50 ms to arm or re-arm (a slow or contended hive) is reported `guard.unhealthy` and then `guard.healthy` when it establishes, so Registry rules on such hosts can show short health flaps that the legacy backend never reported. Neither behaviour needs an operator action; both are recorded so dashboards and alert thresholds can be read correctly when the flag flips.

**Windows Service detection under spark (also `prefer_spark`-gated, inert today).** Documented ahead of the flip for the same reason. `watch()` itself is instantaneous (a queue push); establishment - opening the service against the Service Control Manager - happens asynchronously on a bounded worker. Registering for change notification and delivering the first observed state then happen on the mechanism's own thread, as before - that registration step is not isolated by this change (a Win32 requirement ties it to the mechanism thread), so a service whose registration itself hangs can still delay sibling services on the host, a narrower, still-open residual of the original hazard this restructure otherwise closes. Expect that delay to be dominated not by the underlying Windows API cost (well under a millisecond per call) but by the mechanism's own poll cadence: end-to-end, arm to first observed state runs roughly 50-65 ms on measured hardware (idle or under SCM load - the two are within noise of each other), because the mechanism only checks for a completed lookup once per its fixed 50 ms tick rather than being woken the instant one finishes. This is a structural property of the current design, not a fault condition, and not something an operator needs to act on - but a pilot's own tooling expecting sub-millisecond Service Spark delivery (matching the raw API cost) should be calibrated to this instead. A genuinely absent service resolves to an ordinary `Stopped` state, never a fault; a service the agent cannot query (permissions, a backend SCM error) is reported `guard.unhealthy` until it resolves, mirroring Registry's health-flap behaviour above but driven by SCM query failures rather than hive contention. Retried lookups re-poll on a fixed cadence and do not extend or reset an already-in-flight health window once the mechanism has a definitive answer for that attempt, so a persistently-absent or persistently-unreachable service settles into a stable, non-flapping state rather than repeatedly re-arming its own grace period.

The traffic is bounded and small by construction - and every number below is something your own network capture can check directly, independent of any Yuzu-side tooling:

- **Rate:** a per-agent token bucket refills at 0.1 batches/sec (1 batch every ~10 seconds), so **one reconnected agent adds at most 1 batch/10s of steady-state traffic**. A fleet of N agents therefore adds at most `N * 0.1` batches/sec server-wide in steady state. At an average retained batch size of roughly 32 KiB (derived from the 32 MiB / 1000-batch retention cap below - most batches in practice run smaller), that steady-state rate works out to on the order of 3 KB/s per agent.
- **Startup burst:** each agent process gets a one-time allowance of 5 batches it can send immediately on its first reconnect, before the steady-state rate kicks in. This allowance is a single **process-lifetime** budget, not reset on every reconnect - a flapping agent reconnecting every few seconds does not get a fresh 5-batch burst each time; it draws down the same slowly-refilling budget.
- **Fleet-wide reconnects:** because each agent's burst is independent and tied to its own process lifetime, a mass-reconnect event - a scheduled fleet restart, or many agents recovering together from a brief network blip - can see every freshly-started agent contribute its own up-to-5-batch burst at roughly the same time: up to `5 * N` batches for N freshly-started agents, before the fleet settles to the steady `N * 0.1`/sec rate. This is still bounded (5 batches is small per agent) and one-time per agent process, not a sustained multiplier.
- **Per-pass cap:** a single reconnect (or periodic tick) scans at most 128 batches of backlog per pass, regardless of how much unexpired backlog exists - this bounds the CPU cost of each pass to a fixed ceiling. The token-bucket rate above separately bounds how many of those scanned batches actually go out on the wire; the per-pass cap limits work done, not just bytes sent.
- **Retention window:** the journal itself keeps at most 7 days of not-yet-expired entries, evicted oldest-first (also capped at 1000 batches / 32 MiB per agent). This is a **separate, fixed, non-configurable** window from the Guardian *event* retention described in [Retention](#retention) just above - that one is the 30-day, operator-configurable server-side event store; this is the agent-side journal that feeds it, and the two numbers are unrelated. Replay can therefore never cover more than the last 7 days since an agent's last arm/disarm activity, never unbounded history. Worst case - an agent reconnecting with a full 1000-batch backlog - drains at the steady 1-batch-per-10s rate in a little under 3 hours, not all at once.
  - This 7-day clock runs on the agent's own local schedule, independent of whether it can reach the server. An agent that is network-partitioned (but still running) for most of a week can age out and lose entries on schedule, before it ever gets a chance to reconnect and replay them - "reconnects within 7 days" is not quite the right mental model; "has had a chance to attempt a send within 7 days of each entry" is closer. A loss like this is still a rare, counted event, not silent - see the eviction-loss note in [Guardian metrics](metrics.md#guardian-metrics) - but the count itself has the same visibility limits described immediately below.

**Observability status:** a fleet-wide Prometheus rollup for this traffic EXISTS today - the `yuzu_fleet_guardian_journal_*` gauge family on the server's `/metrics` endpoint (see [metrics.md](metrics.md)): fleet sums of every `yuzu.guardian_journal_*` heartbeat counter (pages, records paged, the eviction-loss counters, and the rest), plus three `*_seconds_max` age gauges (worker staleness and the headroom-blocked episode age). Like everything in this section it is live only once the Spark detection path is the authoritative backend (`prefer_spark`); pre-cutover the family is deliberately absent. What is genuinely still missing: a dashboard panel, a per-agent REST/MCP view of these counters (fleet aggregates only today - no per-endpoint attribution axis), and durability (the counters are in-memory per agent process and reset on restart, so they are telemetry, not an audit trail). Those remain tracked under #2298 and the per-agent-axis work. Until the dashboard lands, read the rollup directly from `/metrics`, and treat the bounds above as falsifiable predictions to check against your own capture.

## Observability

Guardian surfaces readiness and event counts via the standard observability endpoints:

- `/healthz` (authenticated) reports `stores.guaranteed_state` **and `stores.baselines`** alongside the other store-health entries. Both stores are on the shared Postgres substrate (schemas `guaranteed_state_store` / `baseline_store`, ADR-0038 / ADR-0055). A RUNTIME open failure (post-boot) reports `status = "degraded"`; construction/backfill failing at boot instead refuses to start (non-zero exit) rather than serving `/healthz` at all — a distinct failure mode, not a degrade the endpoint can report on.
...
- `yuzu_guardian_push_rule_excluded_total{reason}` (currently only `reason="depth_exceeded"`, json-dump-depth-guard fix) counts a rule whose stored spec was excluded from a push (both the periodic heartbeat reconcile and the on-demand push fan-out triggered by a rule create/update/dashboard toggle) because it could not be safely parsed. A rule this happens to keeps looking like an ordinary rule everywhere except its own detail page (which shows an "invalid data" state) - if a Guard appears not to be enforcing on any agent, check this counter and that page before assuming a scope/targeting problem. This can only happen to a row poisoned before the #4478 depth-guard fix shipped, or by direct database manipulation; the create/update routes reject an over-deep body outright. **The counter increments on every exclusion, unconditionally, but its paired log line does not** (#4497/#4499): a rule's first exclusion logs immediately, then that same rule logs again at most once per 60 seconds no matter how many further exclusions it has in between - so a counter that keeps climbing with no matching log line is expected behavior, not evidence of a separate bug. Rate-limiting is now tracked per rule (a bounded 256-rule LRU cache), so a second, distinct poisoned rule always gets its own immediate first log line rather than being masked by an unrelated rule's ongoing suppression window - but the cache is a per-rule pacing bound, not a fleet-wide log-rate ceiling: above 256 simultaneously-poisoned rules cycling through the cache in a repeating reconcile order, pacing for that whole set collapses to a hard cliff - every rule logs on every reconcile pass (verified: 257 rules cycling produced 257/257 logged on every pass, a 0% suppression rate), not merely "somewhat more often than once per 60 seconds" - and a burst of many distinct rules newly poisoned at once each still logs immediately (by design - a genuinely new rule is never suppressed). In that regime the log line fires once per exclusion, so its rate equals the reconcile rate (heartbeat rate times connected agents): bounded, not runaway, but with no pacing at all - worse than the pre-#4497 shared sampler, which floored the same regime at roughly one line per hundred exclusions. The underlying exclusion EVENT rate (how often build_agent_push encounters the row at all) is unchanged by either fix; only the LOG LINE's rate in this specific regime is worse than before. Whichever rule(s) are affected, the reliable way to identify them is each rule's own detail page (the "invalid data" state above), not the log stream.
- `yuzu_server_guardian_platform_matrix_stale_total{spark_type}` (server) fires when the dashboard's not-implemented fold above (Compliance overview) finds a pair the server's guard-type/platform matrix calls unsupported, but the agent already reports real status for it — the matrix disagreeing with what an agent has actually observed. Render-time only, no dedicated alert (a diagnostic signal for future matrix drift, checked on incident, not paged on) — see [metrics.md](metrics.md).
- `yuzu_fleet_agents_dex_observer_disarmed` (server) counts agents — DEX enabled, on any platform with an observer (Windows/macOS/Linux) — whose signal observer is **not currently healthy**: it failed to arm at startup, or its health flipped at runtime (Windows: a channel subscription died — an EventLog-service restart or channel ACL change; Linux: persistent `/proc/stat` read failure). `> 0` means reliability telemetry is off or degraded on that many endpoints; investigate. Agents started with `--dex-disable`, or on a platform without an observer, are excluded, so this is a genuine fault count, not an opt-out count. **Caveat:** it does *not* detect a host where the underlying reporter is disabled (e.g. Windows Error Reporting off → no Event 1000, observer still armed) — verify coverage with a synthetic event on a representative host. (Agents report arm-state in each heartbeat; the server rolls it up — the agent has no `/metrics` endpoint.)
...
- `Guardian T_server` (server), `Guardian T_wire` and `Guardian T_detect` (agent) are `info`-level diagnostic log lines (#4606): per-event latency waypoints for the Spark cutover benchmark, correlated by `event_id`. They are plain log lines, not metrics or an audit trail - no alert or dashboard consumes them. Volume tracks Guardian event volume; `--log-level warn` suppresses them (and every other `info` line). If you consume them, join on the agent and `event_id` (the agent-side lines carry no agent field) and never on log-file line order (a `T_wire` line can precede its own `T_detect` line), and read `sent=0` per path: on the Spark path it means the local write failed and the entry is normally retried under the same `event_id` (unless a newer observation replaced it or the rule was dropped meanwhile; a down stream logs no line at all), on `domain=legacy` it means the link was down or the write failed and the event was dropped. The fields are described under "Reading the `Guardian T_*` diagnostic lines" below; how to read an orphaned line (one with no partner) is in the header comment of `agents/core/src/guardian_spark_timing.hpp`; the operator-facing summary is the Upgrade note in [server-admin.md](server-admin.md).
...
### Reading the `Guardian T_*` diagnostic lines

All `*_ns` fields are nanoseconds. `*_wall_ns`, `recv_ns`, `committed_ns` and `agent_ns` are wall-clock instants (nanoseconds since the Unix epoch); `*_mono_ns` come from a steady clock with an arbitrary origin and are only for intervals within one process. A difference between two instants taken on different hosts includes their clock skew. Every line is best-effort: one can be missing because logging failed, the log level was raised, a file rotated or the process stopped.

| Line | Where and when | Fields |
|---|---|---|
| `Guardian T_server` | Server, once per Guardian event it stores for an ordinary rule (not a replay, a conflict or an event the store failed to write, and not a ruleless DEX observation) | `event_id`, `agent`, `rule`; `recv_ns` (when the server received the response); `committed_ns` (after the database commit); `agent_ns` (the event's own timestamp from the agent, `-1` when it is absent or invalid); `store_ms` (elapsed time of the whole store call, which includes work after the commit, so it is not `committed_ns - recv_ns`) |
| `Guardian T_wire` | Agent, once per send attempt of a Guardian event. On the Spark outbox path a down stream logs nothing (the entry is retained and retried); on the legacy path a down stream logs `sent=0` (the event is dropped); a legacy detection made before the sink is wired logs nothing | `event_id`; `domain` (`compliance`, `health` or `lifecycle` on the Spark outbox path, `legacy` on the drift path); `sent` (`1` means the local write succeeded, which is not server receipt); `wire_wall_ns` (after the write returned) |
| `Guardian T_detect` | Agent, Spark path only (`prefer_spark`), once per outbox entry an evaluation pass stages | `event_id`, `domain`; `detect_wall_ns` and `detect_mono_ns` (just after the rule was evaluated); `accepted` (`1` when the entry was queued, `0` when a full outbox rejected the batch); `fire_wall_ns` and `fire_mono_ns` (when it was queued, `-1` when `accepted=0`); `trigger_present` (`1` when a detection event started the pass, `0` for a scheduled re-evaluation); `mechanism_wall_ns` (when the detection engine received the mechanism's report, which is not the moment of the underlying change), `handler_wall_ns` and `handler_mono_ns` (when the agent's handler received the event) and `seq` (the engine's per-armed-spark counter for that event), all `-1` when `trigger_present=0` |

The ids in all three lines are neutralised and length-limited the same way (the Upgrade note in [server-admin.md](server-admin.md) gives the rule, and lists the other log lines that still print a rule id as authored).

The trigger fields describe the event that started the pass and are repeated on every entry that pass stages, including an entry for a rule that event did not change (a refresh or a first verdict), so do not treat every `trigger_present=1` line as a detection of its own rule. On the agent the order mechanism, handler, detect, fire holds in program order; fire before wire, and wire before `recv_ns`, are not guaranteed (they are stamped on different threads and hosts), so a small negative difference between them is possible.
...
**`--spark-disable` / `YUZU_AGENT_SPARK_DISABLE`** is a **boot-time** agent opt-out: when set to `1`, SparkEngine is never instantiated and watches nothing — but the heartbeat **still carries the posture itself** (`spark_running=0` + `spark_disabled=1`), so the fleet can tell your deliberate opt-out apart from an engine that *failed* to start. (Emitting nothing at all is what used to make a fleet-wide boot failure invisible.) Like `--dex-disable` it takes effect only at agent start — a live change is ignored until the agent restarts. It is a deploy-time agent setting, **not** a server-side runtime toggle, and it does **not** affect the enforcing legacy path. **At rung 1, leaving it unset (the default) is recommended** — the engine is observe-only and safe; there is no rung-1 reason to disable it (if a boot exception is hit, e.g. thread exhaustion, the agent already degrades to no-spark on its own and continues). On Linux/systemd installs, persist it across restarts and package upgrades by adding
`YUZU_AGENT_SPARK_DISABLE=1` to `/etc/yuzu-agent/yuzu-agent.env` (root-owned 0600, not
shipped - deliberately NOT the shared `/etc/yuzu/`, which a co-installed `yuzu-server`
package owns and re-asserts on every upgrade; the unit loads this file via
`EnvironmentFile=-`, so absence is a no-op) - write one bare `NAME=value` line; systemd
does not strip a trailing `#` comment, and a malformed value stops the agent starting.
Restart the agent to apply; re-enable by removing the line, not by setting it to `0`.
...
**macOS note:** a macOS agent reports `spark_running=1` with an **empty** mechanism capability — all three mechanisms are Windows/Linux facilities, so SparkEngine on macOS runs with zero mechanisms **by design**. This is expected, not a fault: the absence of `yuzu_fleet_spark_mechanisms{os="darwin",...}` series *is* the correct signal. Do not file it as "spark broken on my Mac".

**Fleet observability** (all os-labelled — File/Registry are Windows-only, Service is Windows + Linux, macOS has none — so **never aggregate across `os`**; **absent** means no reporting agent of that OS/mechanism, never "0 = healthy"). An agent shows **no** spark series at all (`ABSENT`) in three cases: it pre-dates rung 1, it is dead/not heartbeating, or it is mid-graceful-shutdown (the final beats of a stopping agent deliberately ship no spark keys, so a clean restart never counts as a failure):
...
- `yuzu_fleet_spark_unsupported{os,mechanism}` (F7, #2298 rung 2) - rules currently classified `unsupported`: a known spark type with no mechanism on that host, enforced by neither backend. A live gauge, not a counter - it can legally decrease. Routine, not page-worthy (every rule on macOS reads this once spark is preferred, since macOS has no mechanisms). **Telling routine from anomalous:** a nonzero reading on an OS/mechanism combination that SHOULD have a mechanism (file/registry on Windows, service on Windows or Linux-with-systemd) means a registration failure or an inert-but-registered mechanism, not a routine cross-platform gap - cross-check `yuzu_fleet_spark_mechanisms{os,mechanism}` and the agent log (see [Diagnosing an inert File worker or Registry sweeper](#diagnosing-an-inert-file-worker-or-registry-sweeper-windows)): that mechanism's `start()` boot lines (a mechanism that could not bind its OS facility), or, on Windows, `spark_file: worker pass failed (consecutive #N)` / `spark_registry: sweeper pass failed (consecutive #N)` followed by `spark_file: worker failing persistently` / `spark_registry: sweeper failing persistently` (a File worker or Registry sweeper that is inert at runtime clears itself: look for `spark_file: worker pass recovered after N failure(s)` / `spark_registry: sweeper pass recovered after N failure(s)`). `yuzu_fleet_spark_mechanisms` counts registered-AND-functional mechanisms (rung 1 deliberately excludes inert ones from the capability CSV; clarified here per F7/#2298), so a 0 there does NOT by itself distinguish "never registered" from "registered but inert" - it is consistent with either. The agent log is what actually disambiguates the two. Once spark is preferred, a rule classified `unsupported` while a File worker or Registry sweeper was inert at runtime stays `unsupported` after that mechanism is counted again in `yuzu_fleet_spark_mechanisms`, until the next reconcile or an agent restart (a known open gap, tracked as #4685; see R5.7 (g) of `docs/spark-stage2-guardian-consumer-design.md`).
- `yuzu_fleet_spark_watch_faults{os}`, `…_queued_dropped{os}`, `…_consumer_errors{os}` — engine-level fault / drop / handler-error sums.

A spark-capable agent's wire state maps to exactly **one of four postures**, never more than one at once: **running** (`_reporting`), **failed** (`_failed`), **disabled** (`_disabled`), or **absent** — no `yuzu.spark_*` tag at all, meaning a pre-rung-1 agent, one that has stopped heartbeating, or one mid-graceful-shutdown.

At rung 1 the mechanism- and consumer-health counters (`armed_faulted`, `watch_rejected`, `quarantined`, `slow_op`, `watch_faults`, `queued_dropped`, `consumer_errors`, `unsupported`) are all 0, because nothing is armed; they go live as later rungs arm rules. **`reporting`, `disabled`, `failed` and `mechanisms` carry live signal today** - none of them depends on a consumer being armed. The agent has no `/metrics` endpoint - these are rolled up server-side from heartbeat tags.

**`yuzu.guardian_backend`** (F7, #2298 rung 2, agent heartbeat tag, not a fleet gauge): reports which detection backend a device is actually enforcing with - `spark`, `legacy`, `unwired`, or `spark_failed`. Unlike the counters above, this one IS live today: it reads `legacy` fleet-wide, since `prefer_spark` is not yet enabled anywhere in production. It exists to let the server distinguish `spark_failed` (agent-wide - nothing enforced) from a routine per-rule `unsupported` classification, which the fleet gauges above cannot do on their own.

**Alerts.** One rule ships **ACTIVE** today: `YuzuSparkBootFailed` (group `yuzu-fleet-spark-rung1` in `docs/prometheus/yuzu-alerts.yml`) — `yuzu_fleet_spark_failed > 0` for 30 minutes, severity **warning** (a ticket, not a page: the posture is agent-self-reported, so one forged or buggy agent can trip it). `_failed` is a per-sweep state gauge, not a latching counter sum, and boot failure is genuine rung-1 signal. The reviewed `yuzu-fleet-spark` **counter** alert group ships **commented-out** until rung 2: those counters are all 0 at rung 1 (an enabled rule could only fire on a forged heartbeat), and they are fleet **sums of monotonic per-agent counters** — a `> 0` alert on them **latches**: once any agent reports a non-zero value it stays firing until that agent's process **restarts** (only `armed_faulted`, a live self-healing gauge, is latch-free). The rung-2 templates use `increase(...[15m]) > 0` over counter-typed metrics to avoid this; enable them when rung 2 arms real rules.

**K-bound wedge waiver (rung 9c PR-5e, #4221 — governance Gate 6/sre finding: this operator-facing note was missing; dormant until Guardian enforces through SparkEngine, same as the rest of this section).** Once enforcement is live, a rule whose arm genuinely hangs past its backend deadline ("wedged") no longer holds its policy generation's acknowledgment forever: after 3 identical same-generation re-applies, the generation acknowledges anyway - **provided every OTHER outstanding failure in the application is also Wedged; a co-occurring non-wedge refusal (a genuine backend refusal, an admission rejection, a congestion-only expiry) still holds the whole generation's acknowledgment indefinitely regardless of this rule's own re-apply count** - while `yuzu.guardian_arm_failed`/`yuzu_fleet_guardian_arm_failed` (see `docs/user-manual/metrics.md`) stays the signal that this specific rule is still failing, for as long as the current application stands. **The "3" counts identical re-applies of the whole push, not this rule's own wedge history** - it's one shared counter, so a rule can be waived the very first time it's ever seen wedged if an unrelated sibling rule's ordinary, now-cleared failure already funded the count on earlier re-applies; don't read "3 re-applies" as "this alert fired three times before waiving." A wedged rule does not clear itself with time alone, and **restarting the agent is not always the right remediation**: it re-arms the rule, which resolves a *transient* wedge (a temporarily stuck watch or backend call) but does nothing for a *permanently* wedged target (a dead host or service) — restart simply re-arms against the same dead target and re-wedges. Check whether the underlying target is transient or permanently unreachable before restarting on a sustained `arm_failed` alert. Full mechanism: `docs/spark-stage2-guardian-consumer-design.md` §R5.3.

### Diagnosing an inert File worker or Registry sweeper (Windows)

Runtime inert is a state the agent enters and leaves by itself, and the failed-pass counters are test-only (nothing on the box or the server exposes them), so the agent log is the on-box evidence and `yuzu.spark_mechs` (with its fleet gauge) is the only signal the server sees. The agent log is not flushed per line, so read the "The log is not flushed per line" bullet before trusting a quiet log.

- **Where the log is.** The shipped Windows installer registers the service with `--log-file {app}\logs\yuzu-agent.log` (`{app}` is the install directory); `sc qc YuzuAgent` shows the command line, and so the log path, actually in effect. A service registered by hand without `--log-file` writes `<data_dir>\yuzu-agent.log`, and an interactive (non-service) agent logs to the console (stderr), and also to that file when `--log-file` is set. Rotation and the other log flags are in the agent flag table in [Device Management](device-management.md) (`--log-file`, `--log-max-size`, `--log-max-files`).
- **The log is not flushed per line.** The agent sets no flush level or flush interval on its logger, so lines pass through the sink's write buffer and can lag behind the event; they are guaranteed to be written at rotation and when the service stops (`service_win.cpp` flushes once `run()` returns), and a crash or a hard exit can lose the buffered tail. An empty search on a quiet agent therefore proves nothing: the absence of `worker pass failed` lines does not show a boot-time inert. The fleet series is the reliable signal: a service stop flushes the log, but it also ends the episode you are diagnosing.
- **Boot-time versus runtime.** A mechanism that could not bind its OS facility logs an error from `start()` (for example `spark_file: CreateIoCompletionPort failed`, `spark_registry: CreateThreadpool failed`) and has no `worker pass failed` / `sweeper pass failed` lines. A runtime-inert File worker or Registry sweeper logs `spark_file: worker pass failed (consecutive #` or `spark_registry: sweeper pass failed (consecutive #` lines, then `worker failing persistently` / `sweeper failing persistently`, and `worker pass recovered` / `sweeper pass recovered` when it clears. The failed-pass lines are logged at consecutive failures 1, 2, 4, 8 and so on, so the third failure logs only the `failing persistently` line. The failed-pass line carries no exception text (#4686), so a caught allocation failure looks like any other throw. The failed-pass and `failing persistently` lines are logged at error level and the recovered line at info level, so `--log-level warn` hides the recovery.
- **Timing.** Retries start at 50 ms and double up to a 30 s cap. The mechanism is reported inert at the third consecutive failed pass (about 150 ms of backoff after the first failure at the default cadence) and clears at the first successful pass. For File, a watched-directory event also runs a pass at once, which ends the episode if it succeeds, and a watch added during an episode has its directory read armed inline when the directory resolves within the 50 ms caller budget (a slower or missing directory also waits for a pass); its establishment report and faults are not delivered before the next pass, up to 30 s away unless such an event runs one sooner.
- **Alternating failures.** A retry pass that finds nothing due counts as a success, so a worker that alternates failing and succeeding logs `worker pass failed (consecutive #1` and `worker pass recovered` over and over and never logs `failing persistently`; `file` then stays in `yuzu.spark_mechs`. Repeated `#1` / `recovered` pairs mean an unhealthy worker even though the fleet series looks healthy.
- **A blocked log sink stalls the worker (and, on Registry, `mu_` itself).** The failure and recovery lines are written on the File worker's (or Registry sweeper's) own thread, so a blocked or slow log sink (a stalled pipe, a hung log volume) stalls that worker for as long as the sink blocks. On File this stalls only that worker's own draining; on Registry the lines run while its internal lock is held, so it also blocks arm/disarm on that mechanism. Only agent shutdown is bounded: a blocked sink there ends in a forced exit after about 20 s. This is a known limit, recorded in [R5.7 (g)(4) of the Guardian and Spark design](../spark-stage2-guardian-consumer-design.md) and tracked as [#4704](https://github.com/Tr3kkR/Yuzu/issues/4704).
- **Fleet lag.** The fleet gauge trails the agent by up to about 45 s (the 30 s default heartbeat plus the 15 s server sweep), so an episode shorter than that can pass the gauge unseen and shows only in the log. The fleet-side query is in [Metrics](metrics.md).
...
- [Spark/legacy delta registry](../spark-legacy-delta-registry.md) — every deliberate behavioral difference between the legacy `IGuard` detection path and the SparkEngine path (engineering reference).
