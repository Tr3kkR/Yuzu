...
| `file-hash-equals` | the file's **content** differs from a baseline | `path` (required), `expected_hash` (64-char SHA-256 hex; **empty → baseline captured on arm**), `max_bytes` (hashing cap, default 64 MiB → larger files report `<oversize>`; **hard agent-side ceiling, currently 1 GiB** — a larger authored value is silently clamped on the agent and logged once, not rejected at authoring time), `settle_ms` (quiescence window before hashing, default 750) |
