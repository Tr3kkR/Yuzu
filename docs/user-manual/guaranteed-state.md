...
Three guard types ship today: the **registry** guard (above, Windows-only), the **file** guard (below, Windows-only), and the **service** guard (below, Windows + Linux/systemd + macOS/launchd). On Linux and macOS the service guard is **observe-only** — it detects and reports drift but does not start/stop services; an `enforce`-mode rule on those agents degrades to observe (drift is reported, never remediated), with no error. Further macOS guards (Endpoint Security / FSEvents), the remaining Linux event guards (inotify/netlink), and non-systemd Linux fallback are on the roadmap.
...
A `service-status-change` spark watches one service in real time: on Windows via `NotifyServiceStatusChange` (kernel-notified by the Service Control Manager, ~0 ms, no polling); on Linux via the systemd `PropertiesChanged` D-Bus signal on the watched unit's `ActiveState` (sd-bus, event-driven, no polling); on macOS via a `launchctl print` state poll (30 s) overlaid with a kqueue process-exit watch on the job's live PID, so a stop is still caught in ~0 ms while starts are bounded by the poll (launchd exposes no public state-change notification API). The watch is resilient: it survives the service being deleted and recreated, re-arms automatically, and on Linux reconnects after a `systemctl daemon-reexec` / dbus restart. The assertion *type* encodes the desired run state; the only param is the service / unit / label name:
...
**macOS (observe-only, v1).** `service_name` carries the **launchd label** in the system domain (e.g. `com.apple.newsyslog` — LaunchDaemons; per-user LaunchAgents are out of scope). `service-running` / `service-stopped` detect and report drift; the guard does **not** start, stop, or unload jobs — macOS has no stable public API for controlling arbitrary launchd services (`SMJobSubmit` is deprecated; `SMAppService` manages only the calling app's own services), so enforcement is a separate, governance-gated change. An `enforcement_mode: enforce` rule on a Mac still detects drift and emits `drift.detected` (with `platform=macos`); a `drift.remediated` event is never emitted on macOS today. launchd's state vocabulary maps onto the two run-state tokens: `running` → running; `not running`, `waiting` (a loaded on-demand job idling), or a label launchd does not know → stopped; an unrecognised state is held (no event), so a state the guard does not understand can never raise a false positive. The `detected_value` on a drift event carries the launchd-native word (e.g. `waiting`, `absent`) where no cross-platform token maps — if you key dashboards or alerts on the raw `detected_value` string rather than the compliant/drift verdict, expect those macOS-specific values. One consequence worth noting: a **socket-activated / on-demand LaunchDaemon** sits in `waiting` when healthy but idle, so a `service-running` rule against it reports continuous drift — exactly as a socket-activated systemd unit (which idles `inactive`) drifts a `service-running` rule on Linux. Author `service-running` only against always-running daemons; for on-demand services, guard the socket/launchd job differently. A job stop is detected in ~0 ms while the agent can watch the job's PID (kqueue `NOTE_EXIT`; always true for the root agent) and within one 30 s poll otherwise; job starts and label re-appearance are detected by the poll.
...
| Assertion | Drift when | Key params |
|---|---|---|
| `service-running` | the service is not Running (stopped, paused, or absent) | `service_name` (required) — Windows: the SCM **key** name (e.g. `Spooler`, not the display name); Linux: the systemd unit (a bare name defaults to `.service`); macOS: the system-domain launchd label (e.g. `com.apple.newsyslog`) |
...
A macOS author uses the same shape in audit mode with a launchd label, e.g.
`"assertion": {"type": "service-running", "params": {"service_name": "com.apple.newsyslog"}}`.
