...
A `file-change` spark watches a file via `ReadDirectoryChangesW` on its parent directory — kernel-notified, **no polling**, so detection is realtime. The watch is resilient: it survives the directory holding the guarded file, or its nearest existing ancestor, being deleted and recreated anywhere in that ancestor chain. A rename or move of the watched directory itself is detected separately, via a watch on that directory's own immediate parent — but nothing watches that parent's own parent, so a rename or move of the parent itself, or of anything above it, is not detected in real time (it is picked up once the guarded path's own content next changes). A rename or move of the watched directory is evaluated like any other change to the guarded path: a guarded file that is no longer at its original path is reported as absent under a `file-exists` rule that expects it present and under a `file-hash-equals` rule, and changes made after the original path is recreated are detected. This behaviour is validated on local NTFS; it also functions over SMB in testing, via the plain name-match fallback used when the extended, FileId-bearing notify API is unavailable, but has not been validated against a production Windows/NAS file server. Two assertions decide what counts as drift:
