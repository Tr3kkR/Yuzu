...
| `file-hash-equals` | the file's **content** differs from a baseline | `path` (required), `expected_hash` (64-char SHA-256 hex; **empty → baseline captured on arm**), `max_bytes` (hashing cap, default 64 MiB → larger files report `<oversize>`; **hard ceiling, currently 1 GiB** — a larger value sent as a JSON string is rejected at authoring time (#2233 item 6); sent as a JSON integer it bypasses that check and is instead silently clamped on the agent, with a warning re-logged on each arm/reconcile while it stays over the ceiling), `settle_ms` (quiescence window before hashing, default 750) |
