...
- **The log is not flushed per line.** The agent sets no flush level or flush interval on its logger, so lines reach the file when the buffer fills, at rotation or when the service stops (`service_win.cpp` flushes once `run()` returns); a crash or a hard exit loses the buffered tail. An empty search on a quiet agent therefore proves nothing: the absence of `worker pass failed` lines does not show a boot-time inert. The fleet series and a service stop or restart are the reliable signals.
