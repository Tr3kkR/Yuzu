...
- **Boot-time versus runtime.** A mechanism that could not bind its OS facility logs an error from `start()` (for example `spark_file: CreateIoCompletionPort failed`, `spark_registry: CreateThreadpool failed`) and has no `worker pass failed` / `sweeper pass failed` lines. A runtime-inert File worker or Registry sweeper logs `spark_file: worker pass failed (consecutive #` or `spark_registry: sweeper pass failed (consecutive #` lines, then `worker failing persistently` / `sweeper failing persistently`, and `worker pass recovered` / `sweeper pass recovered` when it clears. The failed-pass lines are logged at consecutive failures 1, 2, 4, 8 and so on, so the third failure logs only the `failing persistently` line. The failed-pass line carries no exception text, so a caught allocation failure looks like any other throw. The failed-pass and `failing persistently` lines are logged at error level and the recovered line at info level, so `--log-level warn` hides the recovery.
