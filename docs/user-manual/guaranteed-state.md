...
**Observability gap:** today there is **no operator-facing surface at all** for any of this - no dashboard panel, REST endpoint, or MCP tool exposes it, fleet-wide or even for a single agent. The underlying counters (`yuzu.guardian_journal_pages`, `yuzu.guardian_journal_records_paged`, and the related `yuzu.guardian_journal_*` keys, including the eviction-loss counter) exist only inside each agent's live heartbeat `status_tags`, which the server receives but does not currently render anywhere - and that counter itself is in-memory only and resets on agent restart, so it isn't a durable audit trail either. Closing this gap with a fleet-wide Prometheus rollup and dashboard panel is tracked under issue #2298 and is being built separately; this note ships ahead of it so the traffic is documented even before Yuzu's own tooling can independently show it to you. Until then, treat the bounds above as falsifiable predictions to check against your own capture, not as something you have to take on faith.
