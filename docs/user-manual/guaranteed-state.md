...
|---|---|---|
| `Guardian T_server` | Server, once per Guardian event it stores for an ordinary rule (not a replay, a conflict or a rejected event, and not a ruleless DEX observation) | `event_id`, `agent`, `rule` (neutralised, see the Upgrade note in [server-admin.md](server-admin.md)); `recv_ns` (when the server received the response); `committed_ns` (after the database commit); `agent_ns` (the event's own timestamp from the agent, `-1` when it is absent or invalid); `store_ms` (elapsed time of the whole store call, which includes work after the commit, so it is not `committed_ns - recv_ns`) |
| `Guardian T_wire` | Agent, once per send attempt of a Guardian event | `event_id`; `domain` (`compliance`, `health` or `lifecycle` on the Spark outbox path, `legacy` on the drift path); `sent` (`1` means the local write succeeded, which is not server receipt); `wire_wall_ns` (after the write returned) |
| `Guardian T_detect` | Agent, Spark path only (`prefer_spark`), once per outbox entry an evaluation pass stages | `event_id`, `domain`; `detect_wall_ns` and `detect_mono_ns` (just after the rule was evaluated); `accepted` (`1` when the entry was queued, `0` when a full outbox rejected the batch); `fire_wall_ns` and `fire_mono_ns` (when it was queued, `-1` when `accepted=0`); `trigger_present` (`1` when a detection event started the pass, `0` for a scheduled re-evaluation); `mechanism_wall_ns` (when the detection mechanism observed the change), `handler_wall_ns` and `handler_mono_ns` (when the agent's handler received it) and `seq` (the mechanism's sequence number for that event), all `-1` when `trigger_present=0` |
