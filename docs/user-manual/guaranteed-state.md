...
- `Guardian T_server` (server), `Guardian T_wire` and `Guardian T_detect` (agent) are `info`-level diagnostic log lines (#4606): per-event latency waypoints for the Spark cutover benchmark, correlated by `event_id`. They are plain log lines, not metrics or an audit trail - no alert or dashboard consumes them. Volume tracks Guardian event volume; `--log-level warn` suppresses them (and every other `info` line). If you consume them, join on `event_id` and never on log-file line order (a `T_wire` line can precede its own `T_detect` line), and read `sent=0` per path: on the Spark path it means the local write failed and the entry is normally retried under the same `event_id` (unless a newer observation replaced it or the rule was dropped meanwhile; a down stream logs no line at all), on `domain=legacy` it means the link was down or the write failed and the event was dropped. The full field list and how to read an orphaned line are in the header comment of `agents/core/src/guardian_spark_timing.hpp`; the operator-facing summary is the Upgrade note in [server-admin.md](server-admin.md).
