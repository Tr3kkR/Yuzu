...
- `yuzu_fleet_spark_unsupported{os,mechanism}` (F7, #2298 rung 2) - rules currently classified `unsupported`: a known spark type with no mechanism on that host, enforced by neither backend. A live gauge, not a counter - it can legally decrease. Routine, not page-worthy (every rule on macOS reads this once spark is preferred, since macOS has no mechanisms). **Telling routine from anomalous:** a nonzero reading on an OS/mechanism combination that SHOULD have a mechanism (file/registry on Windows, service on Windows or Linux-with-systemd) means a registration failure or an inert-but-registered mechanism, not a routine cross-platform gap - cross-check `yuzu_fleet_spark_mechanisms{os,mechanism}` and that mechanism's `start()` boot logs for the inert bit. Governance finding (sre, F7/#2298): `yuzu_fleet_spark_mechanisms` counts registered-AND-functional mechanisms (rung 1 deliberately excludes inert ones from the capability CSV), so a 0 there does NOT by itself distinguish "never registered" from "registered but inert" - it is consistent with either. The boot logs are what actually disambiguate the two.
