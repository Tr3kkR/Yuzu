...
**Known degraded mode.** The watch on the directory's own parent is retained across ordinary re-arms and only rebuilt when the watched directory's identity actually changes; if a rebuild's teardown of the outgoing watch cannot be confirmed within a bounded wait (an unusual condition — normally only a wedged filesystem driver, minifilter, or a slow/disconnected network share), that watch generation is abandoned rather than reused. After 3 such abandons for one rule, this parent-level rename detection is **permanently disabled for that rule** — logged once, at `error` level, naming the rule and path (`"parent-directory watch for {rule} ... permanently disabled after {n} drain failures"`) — until the rule is next re-armed (a policy re-push or an agent restart). This does not affect the guard's primary detection (the file's own presence or content, evaluated on every wake); only the specific ability to detect a rename or move of the watched directory itself is lost. There is currently no operator-facing signal for this state beyond the agent log line — watch for it if rename detection appears to stop working for a specific rule on a specific endpoint.
