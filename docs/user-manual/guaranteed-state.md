...
**Known degraded mode.** The watch on the directory's own parent is retained across ordinary re-arms and only rebuilt when the watched directory's identity actually changes; if a rebuild's teardown of the outgoing watch cannot be confirmed within a bounded wait (normally only a wedged filesystem driver, minifilter, or a lossy/degraded network share), that watch generation is abandoned rather than reused. This is a measured, not merely theoretical, condition: a private test rig reproduced it by holding a watch on an SMB share and inducing packet loss on that connection — a frozen SMB server or a fully severed network connection both still confirmed cancellation in well under the bound (both fail fast, locally), but genuine mid-path packet loss (the ordinary shape of a degraded or congested WAN link, not a crash) reliably exceeded it, since the client falls back to TCP's own retransmission timeout with no faster local signal available. After 3 such abandons for one rule, this parent-level rename detection is **permanently disabled for that rule** — logged once, at `error` level, naming the rule and path (`"parent-directory watch for {rule} ... permanently disabled after {n} drain failures"`) — until the rule is next re-armed (a policy re-push or an agent restart). Each level of a directory chain created one level at a time beneath the guarded path, and each replacement of the watched directory that is observed mid-change, performs one bounded teardown of the outgoing watch; where cancellation takes longer than the bound (a lossy or congested network share is the realistic case, per the measurement above), each one counts toward the limit, so it can be reached within a single such episode rather than only over three unrelated ones. This does not affect the guard's primary detection (the file's own presence or content, evaluated on every wake); only the specific ability to detect a rename or move of the watched directory itself is lost. There is currently no operator-facing signal for this state beyond the agent log line — watch for it if rename detection appears to stop working for a specific rule on a specific endpoint (tracked: this should become a queryable per-rule field once the guard-health status surface referenced in `guard_file.cpp` gains real per-guard reporting, rather than the placeholder every rule currently gets).

**Other known limitations.** A forced dismount of the volume holding the watched directory, and a change to a mount point/reparse point along the watched path that does not itself rename any watched directory, are not guaranteed to be detected by the mechanisms above; both are pre-existing, measurement-derived gaps distinct from the rename-detection behaviour described above.
