...
- `/healthz` (authenticated) reports `stores.guaranteed_state` **and `stores.baselines`** alongside the other store-health entries. Both stores are on the shared Postgres substrate (schemas `guaranteed_state_store` / `baseline_store`, ADR-0038 / ADR-0055). These entries report whether each store opened at startup (a store's open state does not change afterwards); construction/backfill failing at boot refuses to start (non-zero exit) rather than serving `/healthz` at all. A Postgres outage while the server runs shows as `stores.pg_reachable` = `error` and `status = "degraded"` — the one runtime check (HA WS-8).
- `/readyz` includes both `guaranteed_state_store` and `baseline_store` in the per-store readiness conjunction (startup state), and `pg_reachable` for Postgres reachability at runtime. Either failing takes the replica out of rotation.
