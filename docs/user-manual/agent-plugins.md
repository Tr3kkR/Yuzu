...
6. Document the plugin (`docs/plugin-readme-standard.md`): write `agents/plugins/<your_plugin>/README.md` from `docs/templates/plugin-README.md`; add the optional `description`, `values`, `example` and `platforms` keys to the definition's `result.columns`; on every OS where an action is supported or constrained, capture a sample with the built `tools/plugin-capture` (`plugin-capture <plugin library> --os <os> --host-class <bare-metal|vm|container> --action <name> … --out agents/plugins/<your_plugin>/docs/samples/<os>.txt`), then `python3 tools/plugin-doc-gen/plugin_doc_gen.py --stamp <your_plugin> <os>` to write the leg-hash; finally `python3 tools/plugin-doc-gen/plugin_doc_gen.py --all` (or `meson compile -C <builddir> docs-regen`) so the generated fences, this index, the site navigation and the plugin's `content/plugin-docs` manifest are current, and `--check` to confirm. The `docs` test suite and the docs-lint touch rule enforce this.
