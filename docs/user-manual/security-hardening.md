...
- **Terminal-outcome replay.** When a command's outcome has been recorded, a redelivery replays the **original terminal result** (its real status, exit code, and output) rather than a generic rejection. This closes the "effect-once, result-maybe-lost" gap where a dropped acknowledgement of a completed command would otherwise discard its real result. (A command whose output exceeds 64 KiB is stored with its status and exit code but a truncated output, to bound on-disk growth; a replay of such a command returns the real status, not the full output bytes.)
