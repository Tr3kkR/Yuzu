...
**Degraded mode is fail-open.** If the store cannot be opened or written (e.g. a full disk), the agent keeps executing commands **undeduplicated** rather than refusing them, and raises the `yuzu.dedup_degraded` heartbeat signal so the condition is visible. A per-destructive-instruction-class fail-closed option is planned but not in this release.
