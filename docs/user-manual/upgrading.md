...
| 0.12.x (next) | 0.12.0 | 0.12.0 | **Build-time content auto-import.** All YAML files in `content/definitions/` (217 InstructionDefinitions) and `content/packs/` (10 InstructionSets at this version) are now embedded in the server binary and auto-imported on every startup. Existing operator-customised definitions with matching IDs are NEVER overwritten — conflicts are silently skipped. **Behaviour change for upgrades:** definitions that an operator previously DELETED via the REST API or dashboard will reappear after upgrade because the auto-import treats a missing row as "needs creation". To permanently suppress a shipped definition, set `enabled: false` via the dashboard or `PATCH /api/v1/definitions/{id}` rather than DELETE-ing the row. Each auto-import write emits an `audit_events.action="content.bundled_import"` row with `principal=system` so operators can audit which definitions were inserted at boot. **Yuzu dark navy palette + Inter webfont** (visual change every operator sees) and **Apache ECharts chart renderer** (replaces bespoke SVG; same payload contract — no operator migration required) ship in the same release. **Agent transport consolidation (#376 PR 1c-4):** all five agent RPCs (Register, Heartbeat, CheckForUpdate, Subscribe, DownloadUpdate) now share a single HTTP/2-multiplexed TLS session per agent instead of two independent channels. Wire protocol is unchanged and no operator migration is required, but expect approximately half the per-agent kernel socket count visible in network monitoring tools post-upgrade. The agent additionally enforces a 30-second per-chunk idle deadline on `DownloadUpdate` reads as a symmetric guard against stalled servers; the agent retries on the next heartbeat cycle if it fires (no operator action required). **OTA hardening (#911 / #913 / #934 / SRE-3):** the server now also enforces a per-chunk **write** deadline on `DownloadUpdate` (default 30 s; **operator-tunable via `--ota-chunk-write-deadline-secs` / `YUZU_OTA_CHUNK_WRITE_DEADLINE_SECS`** since #934 — raise for satellite / 4G/LTE fleets that need more headroom, but stay well below half the heartbeat deadline of 30 s to avoid starving Subscribe/heartbeat traffic) and a per-peer token-bucket rate limit (5 tokens / ~90 min refill — automation that hammers OTA checks may see `ResourceExhausted` after 5 attempts; the limit resets on server restart). **Slow-link agents that hit the chunk-write deadline have the consumed token refunded** (#934 / UP-206), so a legitimately-slow peer cannot compound deadline failures into a 7.5h lockout — the new metric label `yuzu_grpc_requests_total{method="DownloadUpdate",status="rate_limit_token_refunded"}` should track `chunk_write_deadline_exceeded` 1:1. The `yuzu_agent_reconnections_total` metric gains a `reason` label (channel_fault / peer_halfclose / stream_open_failed / enrollment_pending) — existing bare-counter PromQL continues to work with no migration; queries with the label require 0.12.x agent. Three new Grafana alerts ship (`YuzuBidiPoolSaturationRejects`, `YuzuBidiPoolNearSaturation`, `YuzuAgentChannelFaultRate`); operators upgrading should silence pre-deploy or accept first-day noise — see `docs/ops-runbooks/transport-saturation.md` for the runbook. New metric families `yuzu_server_transport_*` (8) and `yuzu_agent_ota_chunk_deadline_total{phase}` are documented in `docs/user-manual/metrics.md`. **Gateway `/readyz` JSON shape change (#376 PR 1c-6, #896):** the gateway's `/readyz` endpoint (port 8081) now includes two additional role-named keys in the `checks` object — `"agent_listener"` (the `:50051` agent-facing grpcbox listener) and `"mgmt_listener"` (the `:50063` operator-facing grpcbox listener). Both report `true` when their grpcbox supervisor is alive and `false` when crashed. Kubernetes readiness probes that read only the HTTP status code are unaffected (the 200 / 503 semantics are unchanged), but operators with body-parsing dashboards or Prometheus blackbox rules will see additive keys. See `docs/user-manual/gateway.md` "Health and Readiness" for the documented response shape. **Server TLS pre-flight error messages refined (#376 PR 1c-6):** the single startup error `"TLS is enabled but agent-listener credentials are invalid; refusing to start"` is replaced by five distinct messages — paths empty, key permissions invalid, cert/key files unreadable or empty, CA cert unreadable or empty, or CA missing without `--insecure-skip-client-verify`. SRE alerting rules that grep for the old message text need updating. **Trailing-metadata symmetric scrub at the wire boundary (#376 PR 1c-6, #897):** keys and values in `CallResult::trailing_metadata` / `BidiStream::trailing_metadata()` are now scrubbed to printable ASCII with a 1024-byte cap (`...[truncated]` marker). No operator-visible behaviour change for the gRPC backend (gRPC core already blocks the same bytes on outbound CHECK); the contract is forward-looking for the msquic backend in PR 3. |
