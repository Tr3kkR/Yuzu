...
| 400 | `RESULT_SET_BAD_REQUEST`: on `from-instruction-result` or `re-eval`, `instruction_id` exceeds 256 bytes, or `params` exceeds 32 keys / a key exceeds 256 bytes / a value exceeds 64 KiB. On `re-eval` only, the original's `sql` may also exceed 100 KiB (#4373) |
