...
Every request is checked against a per-route body-size cap **before authentication runs and before the body is read** — a single pre-routing chokepoint (`server.cpp`) resolves the caller's `(method, path)` against the policy table in `body_cap_policy.hpp` and rejects an oversized or unmeasurable body without buffering it. **The four health-probe paths (`/livez`, `/readyz`, `/health`, `/api/health`) are NOT exempt from this gate** — the cap check runs ABOVE their unauthenticated early-return in the pre-routing handler (`server.cpp:7354` vs. the probe return at `:7680`; pinned by `tests/unit/server/test_body_cap_policy.cpp`'s "POST /health ... is rejected (D3)" case). Before this fix those four paths' early-return sat above the cap, making them the server's last unauthenticated 100 MiB buffer — closing that was the point of moving the check. The probes remain exempt from the on-behalf-of guard and the rate limiter that run further below (a header-stamping proxy or orchestrator must not be able to crash-loop the server by tripping either on its own liveness check) — that exemption is unchanged; only the body cap now also reaches them. Three status codes:
