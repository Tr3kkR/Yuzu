...
Every request is checked against a per-route body-size cap **before authentication runs and before the body is read** — a single pre-routing chokepoint (`server.cpp`) resolves the caller's `(method, path)` against the policy table in `body_cap_policy.hpp` and rejects an oversized or unmeasurable body without buffering it. The sole exception is the four health-probe paths (`/livez`, `/readyz`, `/health`, `/api/health`), which return before this gate — or any other pre-routing gate — runs at all (a header-stamping proxy or orchestrator must not be able to crash-loop the server by tripping a gate on its own liveness check). Two status codes:
...
Both responses use the standard [A4 error envelope](#json-envelope) with a `remediation` hint (no `permission` field — the request is rejected before any principal is resolved). This gate is separate from, and runs *before*, any route-local body check a handler may also carry (e.g. the SCIM/response-template 64 KiB checks below) — those still exist for defense-in-depth if a future edit widens this table's entry, but on the current table this pre-routing gate rejects first. **This is not purely a timing change for every affected class.** For most (response templates, CA import), an earlier rejection at the same byte count is the entire effect. SCIM is an exception: a pre-routing 413/411 is the generic A4 JSON envelope above, not SCIM's own RFC-defined `application/scim+json` error schema, so a strict SCIM client that only recognizes the latter will not parse a pre-routing rejection the way it parses the handler's own 64 KiB check — that content-type/schema gap is tracked separately and is not closed by this change.
...
‡ = a generous, explicit, judgment-call bound (still at least two orders of magnitude below httplib's 100 MiB backstop) because no aggregate size contract exists for that class yet, reasoned against that class's OWN realistic scale rather than copy-pasted from a sibling. Do not read either footnote as license to invent a number for a different route — see the header block of `body_cap_policy.hpp`. Ten of the table's entries are reasoned (five †, five ‡) rather than exactly measured; every other entry mirrors a cited, decoded-equals-raw byte count exactly. A third, unmarked category (`ota_upload`, `json_to_csv_export`) is pinned at httplib's own 100 MiB backstop as an explicit, reviewed decision rather than squeezed or given a judgment-call number — neither is "reasoned" in the † /‡ sense, since there is no smaller number to reason toward.
