...
| One credential is active but still carries **unresolved rotation metadata** (a best-effort pair-resolve failed, or its partner expired before cleanup) | `409` — `one active credential with unresolved rotation metadata - do not rotate; revoke or inspect the credential state`. Rotating from here would strand a malformed pair. |
