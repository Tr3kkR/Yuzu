...
**Tier classification heuristic.** `classifyTier` reads `listeners[]` port hints (`DB_PORTS = {3306, 5432, 6379, 27017, 1521, 1433, 9042, 9200, 5984, 8086, 11211}` / `WEB_PORTS = {80, 443, 8080, 8443, 3000, 4200, 5173, 8000, 8088}`) plus process-category strings (`'database'`, `'web'`). Priority is **db > web > app**: a host with any DB signal lands on the database tier regardless of whether it also serves web traffic. A host with no DB or web signal defaults to the application tier. **Known limitation:** databases on non-standard ports (e.g. Postgres on 5431 for a sharded cluster) and custom-port web servers fall through to the application tier unless one of the host's processes is classified as `database`/`web` by `process_category.hpp` — there is no operator-accessible override today; this is tracked as a follow-up.
