...
| `--viz-disable` | off | Disable fleet visualization API endpoints. When set, both `GET /api/v1/viz/fleet/topology` and `GET /fragments/viz/fleet/topology` return `503` with a structured envelope and audit `viz.fleet_topology` / `denied` / `kill_switch`. Tier-before-permission ordering: the kill switch takes effect even for callers who would otherwise fail RBAC. The flag's startup state is logged as `[VIZ] viz endpoint disabled by configuration` so operators have explicit evidence that the kill switch took effect. **The `/viz/fleet` page shell is NOT disabled by this flag** — it continues to serve the renderer scaffold; the `503` surfaces in the browser console when the page's JS attempts to fetch topology. To fully hide the feature from operators, gate the page route with a reverse-proxy rule. Env: `YUZU_VIZ_DISABLE`. |
...
The fleet-visualization feature ladder lands across PRs 1–11 of the `feat/viz-engine` branch. PRs 1–8 are shipped: agent collector + server store + REST/fragment routes + page scaffold + camera controls + cube renderer + Sprite labels + hover tooltip + interior process nodes coloured by category + intra-cube localhost edges. PRs 9–10 add cross-machine + external edges and the vulnerability overlay; PR 11 ships polish (LOD, edge bundling, a11y, perf).

**Operator-visible state today (PRs 1–8).** Navigating to `/viz/fleet` shows one translucent cube per fleet machine on a deterministic grid (FNV-1a hash on `agent_id`). Live agents render at opacity `0.18`; stale agents (no `tar.fleet_snapshot` response within the 5 s deadline) drop to `0.08`. Per-OS palette: Linux `#f0c674`, macOS/Darwin `#a0a0a0`, Windows `#5294e2`. Hostname `Sprite` labels appear above each cube and truncate at 24 characters with an ellipsis. Inside each cube, one `SphereGeometry` dot per process is laid out deterministically (`hash(pid|ppid)`-mod-bucket inside the cube's interior) and coloured from a six-category palette: system `#6e7681`, browser `#58a6ff`, database `#d29922`, web `#56d364`, runtime `#bc8cff`, other `#8b949e`. Faint white lines (`LineSegments`, opacity `0.3`) connect process dots that have an active loopback (127.0.0.1 / ::1) socket pair on the same machine — the renderer's first view of in-host IPC. Hovering a cube shows a tooltip with hostname / OS / process / connection count; hovering a dot shows the process tooltip (pid / name / user / category). WASD pans, drag rotates, wheel zooms.
...
**Kill switch behaviour.** `--viz-disable` / `YUZU_VIZ_DISABLE` disables only the API endpoint. The page shell continues to load (the `503` from the disabled API surfaces as a "Fleet visualization is currently disabled by an administrator" overlay on the canvas where machines would have rendered). To fully hide the feature, gate `/viz/fleet` and `/static/yuzu-viz.js` at a reverse-proxy level.

**Permission model.** The page route is auth-gated only (`require_auth`); the data fetch at `GET /api/v1/viz/fleet/topology` enforces `Response:Read`. An operator with a session but without `Response:Read` can land on the page (sees the grid + axes + the "Access denied" overlay when the JS fetch fires) but every JSON fetch returns 403. The `viz.fleet_topology` audit row is emitted on the API path, not the page path — auditors querying "who accessed fleet visualization?" should filter on the `FleetTopology` `target_type` (covered in detail in `audit-log.md`).
