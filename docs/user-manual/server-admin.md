...
|---|---|---|
| `YuzuOtaConcurrencyRejections` | A peer is repeatedly refused for holding too many parallel transfers. | Find the peer in the server log — the rejection line carries `key=` and `mode=`. **That line is sampled 1-in-100** (the first always logs, then every hundredth, on a counter shared with the server-wide rejections), so a short burst may leave a single line or none; the counter is the complete record and the log is for attribution only. There is deliberately no audit row for admission rejections. |
| `YuzuOtaIdentityRejections` | A peer presented no usable certificate, one from a foreign CA, or an `agent_id` that does not match it. | Check the `reason` label first — the counter is complete. Then the `session.ota_identity_rejected` audit rows for attribution, bearing in mind **two** reasons they under-count: `no_client_identity` is metric-only and writes none, and the row is rate-limited to ~2/s per (peer, RPC, reason), so a flood is deliberately sampled, and `yuzu_ota_identity_audit_suppressed_total` counts what was dropped. Never read row volume as rejection volume. |
...
| `YuzuOtaServerCapacityRejections` | The server-wide ceiling is saturated, so agents are refused regardless of their own per-peer budget. | Two very different causes look identical in the metric; the rejection log's `cert_keyed=` field separates them — but **that line is sampled 1-in-100**, so on a small burst expect one line or none, and correlate with `yuzu_ota_admission_key_mode_total` instead when the log is thin. All-`true` means a genuine rollout wants more concurrency — raise `--ota-max-concurrent-total`, sizing roughly as `devices x transfer_seconds / rollout_seconds`. `false` concentrated on a few addresses is a denial attempt against the shared ceiling; the `--ota-cert-reserve-pct` share is what keeps it off the enrolled fleet. |
