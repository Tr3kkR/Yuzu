...
- **Two new `503` (retry) responses** where the route previously answered `200` with a value it could not stand behind: a `runtime_config_store` outage backing the `required` flag (previously silently reported as `required:false`), and a concurrent trust-bundle upload/clear racing this request's PEM re-read (previously fell through to `enabled:true` with an empty `trust_bundle_pem`).
