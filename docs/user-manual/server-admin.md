...
| `--grpc-max-concurrent-streams` | `128` | **gRPC server bound (#913).** Maximum concurrent HTTP/2 streams per gRPC connection. Before this setting the server's one `ServerBuilder` carried keepalive/ping arguments and nothing else — no stream cap and no `ResourceQuota` existed anywhere — which is what made an unbounded per-peer OTA path a capacity-monopolisation issue rather than a theoretical one. **Gateway-upstream caveat:** this same one `ServerBuilder`/channel argument also bounds the `--gateway-upstream` listener, by construction — but that connection is architecturally different from a single agent's: one gateway connection proxies `ProxyRegister`/`ProxyInventory`/`BatchHeartbeat` (and the OTA RPCs) for a whole fleet of devices behind it, and `GRPC_ARG_MAX_CONCURRENT_STREAMS` is per-CONNECTION, so a cap sized for "one agent, ~1 stream" is also what throttles a gateway's fan-out of many devices' concurrent RPCs over its single upstream connection. If you run agents behind a gateway, size this from the expected concurrent-RPC count of the largest fleet-behind-one-gateway deployment, not from a single agent's needs — analogous to `--grpc-max-threads`'s `expected_agents x 1.5` sizing rule above. Env: `YUZU_GRPC_MAX_CONCURRENT_STREAMS`. |
