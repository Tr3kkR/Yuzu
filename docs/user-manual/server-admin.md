...
- **Multi-host DSNs.** Use `host=n1,n2,n3` **with `target_session_attrs=read-write`**. The probe tries the hosts in the DSN's order — the same order the server's pool uses — each under its own 5 s deadline, so a frozen host costs one deadline instead of holding the probe forever. Without `target_session_attrs`, libpq (and so the pool) takes the *first host that accepts a connection, even a standby*: the probe then reports `read_only`, truthfully, because the pool's writes on that host fail — and pool connections already opened on a read-only host keep failing writes until they are recycled. The probe deliberately does not go looking for a writable host the pool would not use. Exceptions, where libpq's own behaviour applies (a silent first address holds the probe for the whole deadline): one host *name* that resolves to several addresses, and a host list supplied through `service=` or the `PGHOST` environment variable — list the hosts explicitly in the DSN instead. With `target_session_attrs=read-write`, libpq itself refuses a host that is read-only, so a cluster with no writable host reads `unreachable` rather than `read_only` (the server log says why).
