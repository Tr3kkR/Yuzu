...
- **They are written synchronously on the server; the agent side is now asynchronous (#4666 PR-2).** On the server, the thread that reads the agent's stream, or the `ForwardGuardianMessage` handler when the agent is behind a gateway, still writes `T_server` inline, and a blocked sink (an undrained pipe, a stalled network mount) still blocks that thread. On the agent, a guard worker (legacy path) or the Spark consumer, convergence and send threads call the same bare `spdlog::` functions as before, but main.cpp now installs the async hand-off logger (see "Agent logging is now asynchronous" above) as the process's spdlog default logger, so every one of those calls, including the Spark runtime's own arm-committed/late-arm/sweep-residue lines, now enqueues and returns rather than blocking on sink I/O, on any platform where the agent image and `libyuzu_agent_core` share one spdlog registry (confirmed for Linux and Windows; see "spdlog registry identity across images" in `docs/darwin-compat.md` for the macOS caveat). This is the mechanism, not a dedicated per-line change: the `docs/spark-flip-gate.md` section 7 precondition (synchronous benchmark and Spark-runtime log writes) is addressed by this global default-logger swap, not by threading a bounded-wait call through each individual `T_detect`/`T_wire`/arm-committed site.
...
**What changed.** The agent no longer writes log lines synchronously on the thread that produced them; `main.cpp` installs a bounded async hand-off logger as the process's spdlog default (see "Agent logging is now asynchronous" under *Stopping a wedged agent*, above, for the queue-size/memory-cost/overrun details). Agent shutdown gains one more possible self-exit code, **5**, distinct from the existing 1/3/4, fired if tearing down that logger (flushing its queue, joining its worker thread) does not complete within an internal 2-second watchdog.

**Impact.** Not a breaking change: no flag, wire format, API, or default behavior changes, and no operator action is required. Log output looks the same (same pattern/JSON formatting, same `--log-file`/rotation behavior). The only externally-visible differences are: (1) under sustained log-sink overload, the agent can now silently drop older queued lines (`overrun_oldest`) rather than blocking, so a very bursty logger under a stuck sink may show gaps instead of a stall; (2) a shutdown wedge that used to hang or need `SIGKILL` before this and the related #2233 watchdogs landed can now self-exit with code 5 specifically, in addition to the pre-existing 1/3/4.
...
including a `hard_exit()`-driven one. `Restart=always` restarts on ANY exit the process
makes (0 included; it does not distinguish success from failure for the *restart*
decision), but any exit codes 1, 3, 4, or 5 from a shutdown wedge, even one that happened
during an operator-initiated `systemctl stop`, still count as a "failure" result and can
leave the unit reporting `failed` in `systemctl status` rather than `inactive`. All
four also count toward the `StartLimitBurst=5` ceiling above regardless of whether a human or
the crash-loop path triggered them.
