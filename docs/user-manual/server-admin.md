...
- **Multi-host DSNs.** With `host=n1,n2,n3` (plus `target_session_attrs=read-write`), the probe tries each host in turn under its own 5 s deadline, so a frozen first host costs one deadline, not every probe. One host *name* that resolves to several addresses is the exception: libpq walks those addresses itself, and a silent first address holds the probe for the whole deadline — list the hosts explicitly instead.
...
the shipped 210s grace period), then store teardown. A `--shutdown-drain-seconds`
