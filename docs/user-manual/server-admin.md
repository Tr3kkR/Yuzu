...
- **Multi-host DSNs.** With `host=n1,n2,n3` (plus `target_session_attrs=read-write`), the probe tries each host in turn under its own 5 s deadline, starting from the host that last worked, so a frozen host costs one deadline when the probe has to move, not every probe. Exceptions, where libpq's own behaviour applies (a silent first address holds the probe for the whole deadline): one host *name* that resolves to several addresses, and a host list supplied through `service=` or the `PGHOST` environment variable — list the hosts explicitly in the DSN instead. With `target_session_attrs=read-write`, libpq itself refuses a host that is read-only, so a cluster with no writable host reads `unreachable` rather than `read_only` (the server log says why).
...
background-thread joins block in their own Postgres calls until it answers. A `--shutdown-drain-seconds`
grace of N seconds runs before all of this and adds to it — raise the stop
timeout by N when you set it. (Those figures do not include the two background
roll-up joins, app-perf and catalogue, which can each wait out an in-flight
recompute — up to 120 s and 60 s; the 210 s budget was sized for them.)
