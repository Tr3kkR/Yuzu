...
- **What `/readyz` does not see: the pool's existing connections.** The probe holds one connection of its own, re-checks it every 2 s, and reconnects — walking the host list as a new pool connection does — only when that check fails; it answers "can this replica reach a writable primary", not "is every pooled connection healthy". Precisely: `pg_reachable` is red when this replica, connecting exactly as its pool would at that moment, cannot establish a session to a server that accepts writes, or when the probe's own session stops answering or turns read-only. It does not observe the pool's other held connections, nor settings re-resolved after the probe last connected. Known gaps, each tracked: pooled connections to a server demoted in place (below); service-file or environment edits until the probe's next reconnect; a host name with several addresses, one of them silent; `max_connections` exhaustion (the probe is the connection refused first); and about ±1 s of timing difference from the pool's own connect. The pool does not re-check the connections it already holds, and never retires them by age, so a connection whose server turns read-only *in place* keeps failing writes until that connection breaks. If that server is the one a new connection reaches, `/readyz` goes red as well; it stays green only when a new connection reaches a different, writable host. Connecting to Postgres directly, a real demotion restarts the old primary and breaks those connections; the gap needs a server that flips read-only while staying up (for example `default_transaction_read_only` set on a running primary), or a pooler in between that keeps its server connections across the demotion.
