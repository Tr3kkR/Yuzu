...
- **Multi-host DSNs need `target_session_attrs=read-write`, and the server enforces it.** Without it, libpq — and so the server's pool — takes the *first host that accepts a connection, even a standby*, and writes fail on connections nothing can see. So when `--postgres-dsn` lists several hosts (or a `PGHOST`/`PGHOSTADDR` list, or sets `load_balance_hosts`) and has no `target_session_attrs`, the server appends `target_session_attrs=read-write` and logs a warning; `primary` is accepted as well; any other explicit value (`any`, `read-only`, `standby`, `prefer-standby`) makes the server **refuse to start**, naming the value. A host list supplied through a `service=` entry cannot be checked this way — spell the hosts out in the DSN. The probe then tries the hosts in the DSN's order, like the pool, each under its own 5 s deadline, so a frozen host costs one deadline instead of holding the probe; like libpq, it stops at the first host that answers with an error of its own (a failed login, too many clients, a missing database — everything except "cannot connect now"), because the pool stops there too.
- **What `/readyz` does not see: the pool's existing connections.** The probe checks that a *new* connection from this replica reaches a writable primary. The pool does not re-check the connections it already holds, and never retires them by age, so a connection whose server turns read-only *in place* keeps failing writes until that connection breaks. In a real primary/standby cluster a demotion restarts the old primary, which breaks those connections; the gap needs a server that flips read-only while staying up (for example `default_transaction_read_only` set on a running primary). Exceptions, where libpq's own behaviour applies (a silent first address holds the probe for the whole deadline): one host *name* that resolves to several addresses, and a host list supplied through `service=` or the `PGHOST` environment variable — list the hosts explicitly in the DSN instead. With `target_session_attrs=read-write`, libpq itself refuses a host that is read-only, so a cluster with no writable host reads `unreachable` rather than `read_only` (the server log says why).
