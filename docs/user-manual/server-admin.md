...
| `--log-level` | `info` | Logging verbosity: `trace`, `debug`, `info`, `warn`, `error`. Applies to stdout and to the `--log-file` sink. Raising it to `warn` also suppresses the diagnostic `Guardian T_*` lines (see the Upgrade note "new `Guardian T_*` diagnostic log lines") along with every other `info` line. Env: `YUZU_LOG_LEVEL`. |
...
The agent writes `Guardian T_wire event_id=… domain=… sent=… wire_wall_ns=…` for every Guardian event it sends over the Subscribe stream on the legacy detection path. Only once the Spark path is the live backend (`prefer_spark`, off by default) does it also write `Guardian T_detect …` per evaluated event, plus the same `T_wire` line for the Spark outbox path. They exist to measure detect-to-deliver latency for the Spark cutover benchmark (#4606) and are correlated by `event_id`; they are plain log lines, not metrics or audit events, and nothing consumes them.

**Impact.** Log volume only: one server line and one agent line per Guardian event, so it tracks your Guardian event volume. There is no API, schema, metric, alert or wire change, and no change to any detection, dispatch or ingest decision. Nothing to do on upgrade.

**If log volume matters.** There is no dedicated switch. The blunt lever is `--log-level warn` (env `YUZU_LOG_LEVEL`) on the server and/or the agent; it also suppresses every other `info` line (for example the Guardian arm/commit messages), so weigh it before applying fleet-wide. Retiring or gating these lines once the benchmark concludes is tracked in `docs/spark-flip-gate.md` §7.
