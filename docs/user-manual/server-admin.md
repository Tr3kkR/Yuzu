...
- **Multi-host DSNs.** Use `host=n1,n2,n3` **with `target_session_attrs=read-write`** (the pool relies on it to find the primary too). The probe tries each host in turn under its own 5 s deadline, starting from the host that last worked, so a frozen host costs one deadline when the probe has to move, not every probe; a host that answers read-only moves the starting point on to the next host, so the probe cannot stay on a standby. Exceptions, where libpq's own behaviour applies (a silent first address holds the probe for the whole deadline): one host *name* that resolves to several addresses, and a host list supplied through `service=` or the `PGHOST` environment variable — list the hosts explicitly in the DSN instead. With `target_session_attrs=read-write`, libpq itself refuses a host that is read-only, so a cluster with no writable host reads `unreachable` rather than `read_only` (the server log says why).
