...
| `--viz-disable` | off | Disable fleet visualization endpoints. When set, both `GET /api/v1/viz/fleet/topology` and `GET /fragments/viz/fleet/topology` return `503` with a structured envelope and audit `viz.fleet_topology` / `denied` / `kill_switch`. Tier-before-permission ordering: the kill switch takes effect even for callers who would otherwise fail RBAC. The flag's startup state is logged as `[VIZ] viz endpoint disabled by configuration` so operators have explicit evidence that the kill switch took effect. Env: `YUZU_VIZ_DISABLE`. |
