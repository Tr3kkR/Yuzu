...
- **They are written synchronously** by the threads that handle the event (on the server, the thread that reads the agent's stream), so a log sink that blocks, such as an undrained pipe or a stalled network mount, blocks those threads too. The existing per-event log lines already carry that exposure, and for the Spark path it is a precondition of the `prefer_spark` flip recorded in `docs/spark-flip-gate.md` section 7.
