...
**Dedicated coordination connection (HA WS-3, ADR-2002 §10).** Beyond the pool, each server holds **one** additional, never-recycled Postgres connection for background-work leader election, **plus one** for the `/readyz` reachability probe (HA WS-8) — so budget `N_servers × 2` connections against Postgres `max_connections` on top of the pool size below. Per §10 it is deliberately outside the pool and must reach the primary directly (an HAProxy-to-primary front is fine; a *transaction-mode* pooler is not, as it breaks the session advisory lock leadership rests on).
...
- **`pg_reachable` — the runtime signal.** Each replica runs a small probe on its **own dedicated Postgres connection** (not the pool) every 2 s: `SELECT pg_is_in_recovery()`, with client-side deadlines of 5 s to connect and 2 s per query. `/readyz` reports not ready after **two consecutive failed probes** (`unreachable`), **at once** if the probe reached a standby (`read_only` — the server writes to Postgres on nearly every request, so a replica pointed at a standby cannot serve), or when no probe has succeeded for **15 s** (`stale`). One successful probe makes it ready again; your load balancer's own healthy/unhealthy thresholds add any hysteresis you want. Measured end to end: a stopped database turns `/readyz` red in about 3 s, a frozen one (`docker pause`, a black-holed host) in about 11 s, and recovery takes about 2 s (`scripts/ha/ha-readyz-scenarios.sh`). The body never carries host names, DSNs or error text — the libpq detail goes to the server log (`[readyz] Postgres not reachable ...`, logged on each change and every 60 s while not ready).
- **During a Postgres failover every replica goes red at once**, for roughly the failover time (Patroni: 10–30 s). That is correct — nothing can serve writes until the new primary is up. Many load balancers fail open when every backend is unhealthy and keep forwarding; the replicas then answer `503` per request until Postgres is back, which is the same outcome.
...
2. Give the orchestrator a stop timeout longer than the whole shutdown: the shipped compose `stop_grace_period` and systemd `TimeoutStopSec` are **210 s**, which covers the maximum drain (see `docs/user-manual/upgrading.md` for the stacked worst case). Kubernetes' default `terminationGracePeriodSeconds` of 30 s does not.
...
the shipped 210s grace period), then store teardown. `--shutdown-drain-seconds`
above 30 lengthens the first stage by the difference: at its 60s maximum the
worst cases become ~145s and ~205s — the latter only 5s inside the 210s grace,
so raise your stop timeout if you set the maximum. **A long-seeming wait can
