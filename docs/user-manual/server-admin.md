...
| `--mfa-enforcement` | `optional` | MFA enforcement mode: `optional` (users may enroll voluntarily), `admin-only` (admins must be enrolled before login succeeds), or `required` (all users must be enrolled). **This release ships `optional` semantics only** — the parser accepts `admin-only` and `required` for forward compatibility so service files can be staged, but those values are no-ops until the enforcement PR lands and the server emits a startup `WARN` so the silent no-op is loud. See `docs/user-manual/authentication.md` § Multi-Factor Authentication and `docs/auth-mfa-design.md`. Env: `YUZU_MFA_ENFORCEMENT`. |
| `--mfa-step-up-window-secs` | `300` | Seconds after a successful TOTP proof during which high-risk endpoints accept the session as "stepped up" without re-prompting. Consumed by the step-up flow that ships in a follow-up release. Env: `YUZU_MFA_STEP_UP_WINDOW_SECS`. |
...
Three new CLI flags ship in this release: `--mfa-enforcement` (default `optional`; the parser accepts `admin-only` / `required` for forward-compat but those values are silent no-ops in this release and emit a startup `WARN`), `--mfa-step-up-window-secs` (default `300`), and `--mfa-login-pending-secs` (default `120`). Recovery code format changed from `XXXXX-XXXXX` (50 bits) to `XXXX-XXXX-XXXX-XXXX` (80 bits) — codes printed by earlier PR1 commits remain valid until consumed or regenerated. The break-glass procedure for a user who has lost both their authenticator and all recovery codes lives at `docs/ops-runbooks/auth-db-recovery.md` § Emergency MFA disable.
