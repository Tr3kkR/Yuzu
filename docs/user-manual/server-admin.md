...
The fleet-visualization feature ladder lands across PRs 1–11 of the `feat/viz-engine` branch. PRs 1–6 are shipped: agent collector + server store + REST/fragment routes + page scaffold + camera controls + cube renderer + Sprite labels + hover tooltip. PRs 7–10 add interior process nodes / connection edges / vuln overlay; PR 11 ships polish (LOD, edge bundling, a11y, perf).

**Operator-visible state today (PRs 1–6).** Navigating to `/viz/fleet` shows one translucent cube per fleet machine on a deterministic grid (FNV-1a hash on `agent_id`). Live agents render at opacity `0.18`; stale agents (no `tar.fleet_snapshot` response within the 5 s deadline) drop to `0.08`. Per-OS palette: Linux `#f0c674`, macOS/Darwin `#a0a0a0`, Windows `#5294e2`. Hostname `Sprite` labels appear above each cube and truncate at 24 characters with an ellipsis (full hostname in tooltip). Hovering a cube shows a tooltip with hostname / OS / process count / connection count. WASD pans, drag rotates, wheel zooms.
