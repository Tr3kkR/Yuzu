...
**Per-chunk write deadline (#911 / UP-101 + #934 / UP-206):** in addition to the request-frame read deadline, the server enforces a per-chunk write deadline on `DownloadUpdate` (default 30 s; `ota_chunk_write_deadline_secs_` member of `AgentServiceImpl`). A peer whose TCP receive window has collapsed for >deadline — typical causes: a stuck NAT/firewall, HTTP/2 flow-control window collapse, a transparent proxy buffering badly — sees `DeadlineExceeded` and the server log emits `DownloadUpdate: chunk-write deadline expired for peer=<peer> at offset <N> (pool slot released; rate-limit token refunded)`. The pool slot is reclaimed within the deadline; the agent retries OTA on its next heartbeat cycle. The counter `yuzu_grpc_requests_total{method="DownloadUpdate", status="chunk_write_deadline_exceeded"}` increments per fired deadline.

The deadline is **operator-tunable** via `--ota-chunk-write-deadline-secs <n>` or the `YUZU_OTA_CHUNK_WRITE_DEADLINE_SECS` env var (post-#934). Operators with deployments behind genuinely slow links (satellite, residential 4G/LTE, congested corp WAN) can raise this to 120 or higher to avoid spurious deadlines. Setting `<= 0` is clamped to the 30 s default — the deadline is the only thing freeing the bidi pool slot held by a stalled write, so disabling it is not a supported configuration. Pick the smallest value that works for your slowest legitimate link; the deadline directly bounds how long a malicious peer can pin a pool slot during the chunk-streaming phase.
...
**Token refund on chunk-write deadline (#934 / UP-206):** when the per-chunk write deadline fires, the consumed rate-limit token is refunded so a slow-link peer cannot compound the deadline + the rate-limit into a 7.5h lockout. The refund is recorded under a distinct metric label `yuzu_grpc_requests_total{method="DownloadUpdate", status="rate_limit_token_refunded"}`; this counter should track the `chunk_write_deadline_exceeded` counter exactly. A divergence (chunk-write-deadline > token-refunded) signals a regression in the refund wiring. Refund caps at bucket capacity (5 tokens), so a future caller that double-refunds cannot grant a peer excess credit. Threat model: a fast-valid monopoliser still drains the bucket because their calls succeed (no chunk-write-deadline → no refund); only the failing-but-not-monopolising peer benefits from refunds.
...
* A peer that holds open 5+ DownloadUpdate streams in parallel and lets each chunk-write-deadline simultaneously — every stream consumes a pool slot for the deadline duration. The bidi pool's bound (#904) caps the absolute concurrency; if a single peer can exhaust the pool, raise `--bidi-dispatcher-pool-size` or front the listener with the gateway.
