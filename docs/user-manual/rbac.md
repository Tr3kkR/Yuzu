...
- **A caller may not revoke the fleet's last remaining `Administrator` grant through this surface.** The store-level guard runs inside the same transaction as the delete, so two concurrent revokes cannot both succeed and leave zero administrators. It counts only `principal_type='user'` rows that ALSO name a currently-active `auth.users` account — a grant naming a nonexistent, deactivated, or (soft-)deleted username is never treated as a survivor.
  ...
  - **The `auth.users` check is local-account-only, which is a usability limitation for SSO fleets, not a security gap.** An SSO (OIDC/SAML) Administrator never has an `auth.users` row (their session is minted fresh from IdP group membership each login, `AuthManager::create_oidc_session`/`create_saml_session`), so in an SSO-only deployment every `principal_roles` Administrator grant reads as zero survivors to this guard — a last-Administrator-role unassign is permanently refused even when live SSO admins remain. This is over-restrictive, never permissive: it never lets a real lockout through, it just can't yet be used to clean up a stale grant on an SSO-only fleet. Also tracked in [#4966](https://github.com/Tr3kkR/Yuzu/issues/4966).
