...
- **A caller may not revoke their own `Administrator` assignment.** The fleet's **last** remaining `Administrator` grant held by a human user cannot be revoked **through this surface**, by anyone, even another admin — the store-level guard runs inside the same transaction as the delete, so two concurrent revokes cannot both succeed and leave zero administrators. The guard counts only `principal_type='user'` rows that ALSO name a currently-active `auth.users` account — a grant naming a nonexistent, deactivated, or (soft-)deleted username is never treated as a survivor, matching the `is_rbac_administrator` gate's own definition of "administrator" exactly (a grant that satisfies neither could never itself have passed that gate). This guard covers only THIS route's own unassign path — it does not, on its own, guarantee the fleet always has a *usable* administrator: deactivating or deleting the account behind the fleet's last Administrator grant is a **separate, unguarded** path (account lifecycle management, not role-grant management) that can still leave zero authenticatable administrators. The `auth.users` check above is also local-account-only — an SSO (OIDC/SAML) Administrator never has an `auth.users` row (their session is minted fresh from IdP group membership each login, `AuthManager::create_oidc_session`/`create_saml_session`), so in an SSO-only deployment every `principal_roles` Administrator grant reads as zero survivors to this guard, making a last-Administrator-role unassign permanently refused even when live SSO admins remain — over-restrictive, not permissive, but a real gap for SSO-only fleets. Both gaps are tracked in [#4966](https://github.com/Tr3kkR/Yuzu/issues/4966).
