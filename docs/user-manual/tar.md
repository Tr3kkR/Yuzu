...
| `dns_enabled` | `true` / `false` | **`false`** | Toggle the DNS resolver-cache capture source (`dns` source → `$DNS_Live`/`$DNS_Hourly`) on this host (ADR-0015). **Off by default** — the DNS cache reveals which domains a host resolved (**usage-class telemetry under the works-council posture; enabling is audited**). **Device-level state only — no per-process attribution** (the cache carries no PID). **Windows only today** (`DnsGetCacheDataTable`, cache-only — never issues a wire query); Linux planned, **macOS unsupported** (no userspace mechanism enumerates the resolver cache on modern macOS — see `docs/darwin-compat.md`). Set to `true` to opt in; collected at `fast_interval`. |
...
| **dns** | supported (`dnsapi`), **opt-in (off by default)** — `DnsGetCacheDataTable` + cache-only `DnsQuery_W`: name/record_type/data/TTL (ADR-0015). Device-level resolver-cache state; **no per-process attribution**. | planned (`systemd-resolved`) — resolve1 D-Bus / `/etc/hosts` fallback (constrained). Records nothing until wired. | **unsupported** — no userspace mechanism enumerates the DNS resolver cache on modern macOS (`dscacheutil -cachedump` is defunct; `scutil --dns` is config-only; the mDNSResponder cache is dumpable only as root to the redacted system log). Records nothing; no follow-up planned — see `docs/darwin-compat.md`. |
...
host's OS ships; likewise `arp` is **Windows-only today** (planned on
Linux/macOS) and `dns` is **Windows-only today** (planned on Linux;
**unsupported on macOS** — no userspace resolver-cache enumeration exists), so
on a Linux or macOS agent `arp_enabled|true` / `dns_enabled|true` still report
`*_live_rows|0` — run the `compatibility` action to distinguish a
supported-but-empty source from a planned-but-unimplemented one (or, for macOS
DNS, a permanently unsupported one).
...
> empty until you opt in (Windows-only collectors today; ARP is planned on
> Linux/macOS; DNS is planned on Linux but **unsupported on macOS** — see
