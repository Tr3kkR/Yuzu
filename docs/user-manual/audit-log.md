...
| `preflight.run` | Scope | An operator ran a `/auto` pre-flight check via `POST /fragments/auto/run`. `target_id` is the management-group id scoped to the run, or `all-visible` when no group filter was applied; `detail` carries `run=<id> checks=<N> devices=<N>`. `result=success`, or `no_devices` when the chosen scope resolved to no visible devices (a no-op, not a denial). **A service-scoped API token is denied outright** (`result=denied`, `target_id=` empty) before its device cohort is even resolved — the SEC-2/SEC-3 confinement-gap class: `devices_fn` is username-keyed and does not confine a service-scoped token, and unlike the read-only fixes elsewhere in this branch this route MUTATES (creates a run and dispatches the configured checks), so the deny is unconditional rather than data-dependent. Permission gates: `Infrastructure:Read` + `Execution:Execute`. |
