...
| `instruction.approval_required` | instruction | An approval-gated instruction execution was held pending review, and a ticket was minted. `target_id` is the definition id; `detail` carries `approval_id=<id> mode=<approval_mode>` from the REST instruction gate, or `approval_id=<id> mode=scheduled schedule_id=<id>` from a scheduled occurrence. `result=pending`. This is the mint record for the REST and scheduler surfaces; the MCP gate records its own mint under `mcp.<tool>` instead — see that row. Not to be confused with the `approval.created` **webhook/event** type emitted alongside it (`EmitEventFn`, not the audit store) — that is an event-bus notification and is not audit evidence. |
| `approval.approve` | approval | A pending approval was **granted** via `POST /api/approvals/{id}/approve`. `target_id` is the approval id. `result=success` on the grant; `result=denied` when the review was refused — including the self-approval segregation-of-duties block — with `detail` carrying the store's reason. Permission gate: `Approval:Approve`. **Best-effort:** the audit write is not checked, and this route sets no `Sec-Audit-Failed` header and no body flag — unlike `instruction.import` above — so a grant can succeed with no row. **REST/dashboard surface only:** the MCP twins record their grants under `mcp.approve_request` / `mcp.reject_request`, so a rule keyed on this verb alone misses every approval granted over MCP. See the gap note below the table for what is **not** recorded at all. |
| `approval.reject` | approval | A pending approval was **refused** via `POST /api/approvals/{id}/reject`. Same shape and the same `Approval:Approve` gate as `approval.approve` — there is no separate Reject operation. Both routes act on a **pending** row only; neither can act on one already approved. |
| `approval.read` | Approval | A single approval was read via `GET /api/v1/approvals/{id}`. `detail` carries `status=<status>`. Permission gate: `Approval:Read`. **Success branch only** — the 404 and 503 paths return before the audit call, so probing this route with `Approval:Read` to learn which ids exist leaves no `approval.read` trace. (A 403 is separately recorded by the permission gate as `auth.permission_required` / `denied`.) The legacy list route `GET /api/approvals` emits **no** audit row on any path, which matters because that is the route disclosing approval ids in full — see #1803. |
...
| `mcp.<tool>` (approval gate) | mcp_tool | The supervised-tier approval gate on an MCP tool call records its outcome under the tool's own action (`target_type=mcp_tool`, `target_id=<tool>`) — there is no separate `approval.*` verb for this surface. `detail` carries a stable SIEM-keyable token. **Mint branch** (`result=pending`): `[agent_id=<id> ]approval_id=<id>`, with ` (deduped)` appended when an equivalent pending ticket already existed and was reused rather than a second one minted. **Refusals** (`result=denied`): `per-submitter pending-approval cap reached`; `approval_id does not match this request` (the supplied id is not the ticket for these canonical arguments); `approval <id> status=<status>` (a ticket found but not `approved`). **Redemption refusals** (`result=denied`, all of the form `approval <id> <token>`): `already used` — a spent ticket, the ordinary replay/double-click; `refused: minted by a non-MCP surface (#2442)` — a cross-surface redemption attempt, the event the origin control exists to stop, and the one to alert on. This token is **one constant for every non-MCP origin**, so the audit row does not say which surface minted the ticket — see [Metrics](metrics.md#mcp-approval-gate-metrics) for the two surfaces that do; `refused by pre-consume recheck` (#2443); `not consumed: approvals store read failed` — a store read failure, emitted with `result=denied` although the ticket was **not** consumed and remains valid. **Submit failure** (`result=failure`): `approval submit failed: <store error>`. **Successful redemption** (`result=approved`): `consumed approval_id=<id>` — the record that a gated tool actually ran, and the row a misuse of the residual described in [Server Administration](server-admin.md) would appear on. **Best-effort, like every row in this family:** `mcp_audit`'s return is discarded and MCP has no `Sec-Audit-Failed` channel, so a store failure loses the row and the call still succeeds. Absence of a row is therefore not evidence that a redemption did not happen. **Degraded path** (`result=denied`): `approval-gated; approval manager unavailable`. **One branch emits no row at all, so do not read this family as complete:** a recall against a still-`pending` ticket returns without auditing, so an exact-args probe of a harvested approval id against a not-yet-approved ticket is unrecorded. Note also that `pending` and `approved` are `result` values of this family, so the "filter on `result != \"success\"`" advice below sweeps in every mint and every successful redemption — scope that filter by action. The four redemption tokens are deliberately distinct here while the **client-facing** response for all but the store-read failure is one constant string — the recall must not be an oracle for which approval ids exist, but a CC7.2 reviewer must be able to tell a forgery attempt from a benign replay, so the distinction lives only in the audit row and the server log. Pair with the metrics for alerting; the audit row is the per-event evidence, the counter is the signal. `yuzu_mcp_approval_denied_total{tool,reason}` is the operational breakdown; the `foreign_origin` case ALSO increments the SIEM-routed `yuzu_mcp_approval_forgery_total{tool,event="security"}`, which is the one to alert on. See [Metrics](metrics.md#mcp-approval-gate-metrics) for the closed label sets, the one-to-one caveat and the alert form. **Note:** `detail` here carries the **full** approval id, unredacted, while server logs truncate the same value via `redact_id` — so an approval id is recoverable from the audit store by anyone holding `AuditLog:Read`. Among the **seeded** roles that is not a widening: each seeded role that holds `AuditLog:Read` is granted `Approval:Read` too (`rbac_store.cpp` — the four role arrays pair them, and `Operator` holds both at `:522`/`:531`); `Reviewer` and `ApiTokenManager` hold neither, so they are not counterexamples. **It is a widening for any role holding `AuditLog:Read` without a paired `Approval:Read` allow**, and if you rely on `Approval:Read` as the boundary for approval-id disclosure you must account for that. Four ways to reach it, and the list is not asserted to be exhaustive: a **custom role** granted `AuditLog:Read` alone (`create_role` + `set_permission` accept any tuple, and nothing pairs the two); a **post-boot grant onto a seeded role** that lacks `Approval:Read` — `set_permission` has no `is_system` guard, and `rbac_store.cpp` names `Reviewer:AuditLog:Read` as a re-grant an operator may deliberately make; a **`deny` row** on `Approval:Read` for a role that keeps `AuditLog:Read` — deny beats allow ([RBAC](rbac.md)) and, unlike a plain grant removal, survives the boot reseed, which is why it is the durable way to narrow a seeded grant; and **engine principals**, which `RbacStore::assign_role` forbids from holding any built-in system role, so their authority is custom-role-only and seeded parity says nothing about them. Treat a principal holding `AuditLog:Read` as able to read approval ids. To check your own deployment, enumerate roles with `GET /api/v1/rbac/roles` and their grants with `GET /api/v1/rbac/roles/{role_name}/permissions`, and look for `AuditLog:Read` without a paired `Approval:Read` allow. This audit-store sink is tracked on #2776, which carries the redact-or-keep decision; #1803 covers the two `Approval:Read` REST routes, a different reader set. |
| `mcp.bridge.*` | Execution | Server-side lifecycle events of the MCP progress bridge (track 2f PR 3a), NOT operator actions - emitted from the background sweep/projector (and, for the two arbitration verbs `mcp.bridge.cancel` and `mcp.bridge.dispatch_failure`, from the request thread that lost or unwound), so `principal=system` for the bridge's own background work (sweep/projector) - **except** the two client-driven verbs, EVERY `mcp.bridge.cancel` row (whether the cancel detached a live streamed response or was consumed at `arm()` - as of the SSE-on-`POST` rung both carry the canceller; the session gate binds the record's principal to the authenticated caller, so the two cannot diverge) and `mcp.bridge.dispatch_failure`, which carry the authenticated caller as `principal` so a cancel is attributable to the client that asked for it (Decision 15(j) non-repudiation) - and `target_id` is the `execution_id` - **except** for a record reaped by the arming reaper before `subscribe()` assigned an execution id, where `target_id` is empty (that row is therefore not joinable to an execution; the `detail` still identifies the disposition). `result` is normally `success`, but every verb that routes through the claimed-teardown path (all of them except `mcp.bridge.cancel` and `mcp.bridge.dispatch_failure`, which are transitions rather than teardowns) emits **`result=failure`** when the teardown could not settle one of the three resources it owns (`detail` begins `teardown incomplete: ...` and names what is retained - see `yuzu_mcp_bridge_teardown_incomplete_total{reason}` and `docs/ops-runbooks/mcp-bridge-teardown-recovery.md`), or when the terminal-frame publish ladder poisoned the session, threw, or was never reached (`detail` states which, and never asserts a delivery or a poisoning that did not happen - #2506). **Upgrade note:** before v0.13.0 these rows were unconditionally stamped `result=success` regardless of outcome; a SIEM rule that assumed the constant needs updating. The verb family: `mcp.bridge.done_reap` (a settled GET-only record was reaped), `mcp.bridge.session_dead` (a record torn down because its session idle-expired), `mcp.bridge.arming_reaped` (an orphaned reserved-but-never-armed record was reclaimed by the age reaper), `mcp.bridge.cancel` (a cancel took effect - either consumed at arm, or detaching a LIVE streamed response), `mcp.bridge.dispatch_failure` (a request failed AFTER its work was dispatched, so its record was PARKED rather than abandoned - the execution keeps running and its result stays fetchable by `execution_id`; unlike the pre-dispatch failure paths this deliberately does NOT cancel the execution), `mcp.bridge.pin_acked` and `mcp.bridge.forced_expire` (parked-result teardown). All of these are LIVE as of the SSE-on-`POST` rung (2f PR 3b), which wired their callers. The operator action that *starts* progress streaming (`execute_instruction`) is separately audited under its own `mcp.execute_instruction` verb with the real principal - the bridge does not replace it. |

**Gap — an approval leaving `approved` is not audited.** `approval.approve` / `approval.reject` record the grant decision on the REST and dashboard surfaces, `mcp.approve_request` / `mcp.reject_request` record it over MCP, and the `mcp.<tool>` row above records redemption attempts. Nothing records an approval *losing* its grant. The 7-day TTL sweep moves a row to `expired` from inside `ApprovalManager::submit()` with no audit row and no principal, and there is no audited operation that retires a granted approval at all — so a SIEM cannot answer "who withdrew this authorisation" from the audit store today, and neither can an auditor. Any withdrawal performed directly against the database is invisible here and indistinguishable from the sweep, which is one reason this manual no longer publishes such a procedure. Tracked as #2761 (an audited `Approval:Revoke` operation) and #2738 (the TTL's own defects).
