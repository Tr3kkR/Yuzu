...
| `yuzu_server_audit_cleanup_failed_total` | A pass did not fully do its job: an unreadable probe, a failed delete, a refused implausible clock, a closed store, or an exception caught at the thread boundary. On PostgreSQL every site rolls the pass back, INCLUDING the post-delete backlog probe: the whole pass runs in one transaction, so a probe failure after the delete undoes the delete too. Read this as "the pass did not complete and nothing was removed". (On the retired SQLite store the backlog probe was a warning after a committed delete, and this row said so; the substrate change inverted it.) |
