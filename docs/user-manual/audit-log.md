...
| `guaranteed_state.rule.create` | GuaranteedState | A Guaranteed State rule is created. On 400 (missing required fields), 409 (rule_id/name conflict), or RBAC rejection, emits `result=denied` with the store error in `detail`. **The dashboard fragment twin (`POST /fragments/guardian/guards`) now also denies a service-scoped API token outright** (`target_id=` empty) — same confinement-gap class as the read fragments in this file's `guaranteed_state.fragment.access_denied` row, but on the create path: `perm_fn_`'s service-token branch checks only the `ITServiceOwner` role, never the token's own service-tag scope. |
| `guaranteed_state.rule.update` | GuaranteedState | A Guaranteed State rule is updated (version is incremented on every successful update). On 404 (rule not found), 400 (invalid JSON body — `detail` is the literal string `"invalid JSON body"`), 409 (name conflict), or RBAC rejection, emits `result=denied`. **The dashboard fragment twin (`POST /fragments/guardian/guard/{id}/enabled`) now also denies a service-scoped API token outright** (`target_id=<rule_id>`) — same confinement-gap class as `.rule.create` above. |
