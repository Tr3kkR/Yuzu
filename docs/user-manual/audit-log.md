...
| `sle.summary` | SoftwareLicensing | The SLE fleet posture headline was read (`GET /api/v1/sle/summary`). Fleet aggregate, machine-scope (no per-user data) — **set-and-proceed**. `target_id` is `fleet`; `result=success` carries `products=<n>`; a posture-rollup/meta-stamp degrade emits `result=failure` (REST returns `503`, never a silent empty). Gate: global `SoftwareLicensing:Read` (pinned global-only, ADR-0017). |
| `sle.licenses.query` | SoftwareLicensing | The per-product licence posture list was read (`GET /api/v1/sle/licenses`). Same tier/gating as `sle.summary`. `target_id` is `fleet` or `state=<filter>`; `result=success` carries `rows=<n>`; a degrade emits `result=failure`. |
| `sle.licenses.view` | SoftwareLicensing | An operator opened the **SLE page's Licences view** (`GET /fragments/sle/licenses`) — the dashboard twin of `sle.licenses.query`. Machine-scope aggregate (the `inventory.software.catalog` tier), set-and-proceed. `target_id` is `fleet` or `state=<filter>`; `result=failure` = the posture rollup degraded (the page renders the honesty banner, never an empty table). |
| `sle.licenses.devices` | SoftwareLicensing | The devices-carrying-a-product fan-out was read (`GET /api/v1/sle/licenses/{key}/devices`). `target_id` is `key=<product_key>`; `result=success` carries `rows=<n>`; a degrade emits `result=failure`. Global-gated (an ADR-0017 PR-A flip-wave consumer). |
| `sle.agent.view` | Agent | An operator opened **one device's detected-licence drill** (`GET /api/v1/sle/agents/{id}`) — renders `user_scope`/`user_ref` (personal data, ADR-0024 Decision 11), so this is the **behavioural-PII audit tier** and **fails closed**: if the row cannot persist, the route returns `503` + `Sec-Audit-Failed: true` and serves no licence PII. Emitted per-open, **before** the store read. `target_id` is the `agent_id`. Gated per device on `SoftwareLicensing:Read` (ancestor-aware scope). |
| `sle.agent.surfaces` | Agent | An operator requested **live detection-surface diagnostics** for one device (`POST /api/v1/sle/agents/{id}/surfaces` → a `license_scan surfaces` command dispatch). Machine-health metadata — the payload carries **no per-user data** (per-user outcomes aggregate per surface; the server whitelist-parses `probe_status` lines only), so set-and-proceed (`result=requested`, emitted **before** dispatch — the deliberate contrast with `sle.agent.view`'s fail-closed tier). `target_id` is the `agent_id`; `detail` carries the correlation id. Gated per device on `SoftwareLicensing:Read` **and** `Execution:Execute`. |
| `sle.agent.decommission` | Agent | An operator triggered the **durable erasure cascade** for one device (`DELETE /api/v1/sle/agents/{id}`) — the GDPR Art. 17 evidence chain, **two rows per request**: an `result=attempt` row persists **fail-closed BEFORE anything is erased** (persist failure → `503` + `Sec-Audit-Failed`, no erasure — an unaudited erasure would destroy the very data evidencing what was erased; a mid-cascade crash still leaves this row), then a `result=success`/`result=partial` outcome row whose `detail` carries the per-store breakdown (`<store>=deleted|skipped|failed ... deleted=<n> skipped=<n> failed=<n>`). `partial` pairs with an HTTP `500` — the cascade is idempotent, re-issue the DELETE. `target_id` is the `agent_id`. Gated per device on `SoftwareLicensing:Delete` (Administrator / IT Service Owner only). |
