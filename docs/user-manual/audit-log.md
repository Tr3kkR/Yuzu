...
| `device.live.uptime` | Agent | An operator requested live uptime for a device (the dashboard **Get live info** panel, or REST `POST /api/v1/dex/devices/{id}/live?kind=uptime`), dispatching a live `os_info/uptime` instruction. Execute-gated (`Execution:Execute`, beyond `GuaranteedState:Read`); `target_id` is the `agent_id`. Machine-health telemetry. **Two emitters, by design:** the dashboard panel audits **post-dispatch** (`result=dispatched`/`no_agents`, `detail` carries the agent count + `command_id`); the REST endpoint audits **pre-dispatch** (`result=requested`, `detail` carries `cid=<correlation_id>`) and fails closed (`503` + `Sec-Audit-Failed`, no dispatch) if the row cannot persist. Aligning the dashboard emitter to pre-dispatch is a tracked follow-up. |
| `device.live.processes` | Agent | An operator clicked **Get live info** and the running-process panel dispatched a live `processes/list_hashed` instruction (PID, name, and the SHA-256 of each on-disk executable image). **Usage-class behavioral telemetry** — reveals which programs a person is running. Deliberately a **separate verb** from `device.live.uptime` so usage-class reads stay separately countable for works-council access-audit purposes (same rationale as `dex.device.procperf.query`). Execute-gated; `target_id` is the `agent_id`. No hash/process data leaves the device until an operator explicitly dispatches this panel. **Two emitters** (same as `device.live.uptime`): the dashboard panel audits post-dispatch (`result=dispatched`/`no_agents`, `detail` = agent count + `command_id`); the REST endpoint audits pre-dispatch (`result=requested`, `detail` = `cid=<correlation_id>`) and fails closed if the row cannot persist. **Persistence:** the dispatched result (process names, paths, SHA-256) is stored in `responses.db` for `response_retention_days` like any command result, so it is also a DSAR/data-inventory source and is retrievable via the standard response-store API under `Execution:Read` — the works-council "separately countable" guarantee covers the *dispatch* (this verb), not a later read-back of the persisted row. |
