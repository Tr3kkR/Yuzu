...
| `quarantine.reapply` | Security | A **system-initiated** re-application of endpoint containment for a device that reconnected while an active quarantine record existed (#3425), distinct from the operator-initiated `quarantine.enable` above. `principal="system"`. `target_id` is the device. Three shapes, distinguished by `result`/`detail`: **(1)** `result=success`, `detail=command_id=<id> trigger=tick\|heartbeat` — the reconciler re-dispatched the stored whitelist and the plugin registry accepted it (not yet proof of endpoint containment). **(2)** `result=success`, `detail=CONFIRMED command_id=<id>` — a follow-up `quarantine.status` read reported `state\|active`, the only signal this repo treats as proof the endpoint firewall is actually applied. This is the target agent's own self-report of its own containment state (cert-bound to the correct agent identity, so a DIFFERENT agent cannot forge it — but not independently corroborated by any network-side signal), the same trust boundary #3127's MCP `already_active` retry path already relies on for its own confirmation check; see `docs/user-manual/security-hardening.md`'s "Reconnect re-application" section. **(3)** `result=failure`, `detail=<what> command_id=<id> but the durable stamp failed: <store error>` — the dispatch WAS accepted (or the status read DID confirm `state\|active`), a real containment-affecting action already happened, but the durable `last_applied_at`/`last_confirmed_at` stamp write failed (a store outage, or a concurrent release/requarantine racing the write) — recorded so a transient store failure never produces a silent gap in exactly the evidence this verb exists to provide. A re-application that never reaches a dispatch attempt at all (device offline, stored whitelist invalid, degraded read BEFORE any dispatch) is still counted via `yuzu_server_quarantine_reapply_total{result}` rather than audited, the same "counted, not audited" posture `yuzu_server_system_reserved_push_total` documents for its own bypass channels, chosen for the identical reason: an unreachable device produces this outcome on every tick for as long as it stays offline, and an audit row per tick per offline device is an unbounded write rate the counter already answers without it. Shape (3) is the one exception — a genuine agent-facing action already happened, so it is not merely counted. |
