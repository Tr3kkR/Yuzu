...
| `instruction.execute` | Instruction | An instruction is dispatched to one or more agents. On `result=denied` the `detail` begins with a stable SIEM-keyable `reason=<token>`; known tokens are `reason=reserved_definition_id` (#2442 — the definition id is under the reserved `mcp.` prefix, so it can never mint an approval ticket; the request is refused with 400 before any mint is attempted), the route-level `reason=body_type` (the request body was not a JSON object), and the `check_targeting_shape` tokens (#2500). Note `body_type` is a ROUTE-level reason and deliberately sits outside `kTargetingShapeReasons` — resolving "the check_targeting_shape tokens" against that array alone drops `body_type` rows. |
