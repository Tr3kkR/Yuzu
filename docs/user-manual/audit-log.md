...
| `approval.read` | Approval | A single approval was read via `GET /api/v1/approvals/{id}`. `detail` carries `status=<status>`. Permission gate: `Approval:Read`. **Success branch only** — the 403, 404 and 503 paths emit nothing, so id-probing against this route leaves no trace. The legacy list route `GET /api/approvals` emits **no** audit row on any path, which matters because that is the route disclosing approval ids in full — see #1803. |
