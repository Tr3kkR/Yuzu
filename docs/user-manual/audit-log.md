...
| `session.peer_mismatch` | Session | A gRPC Subscribe was rejected because the agent's peer IP did not match the IP recorded at Register time (#1059 — a stolen-session signal). `principal=agent:<agent_id>`; `target_id` is the `session_id`; `detail` carries `agent_id=<id> register_ip=<ip> subscribe_ip=<ip> gateway_mode=<bool>` (plus `raw_peer=<peer>` when the presented peer was malformed and `subscribe_ip` is therefore empty). `result=denied`. SOC 2 CC7.2: high-signal security event — a SIEM alert on this action is recommended. If the audit-store write itself fails, the gRPC response carries trailing metadata `x-yuzu-audit-failed: true` (see **gRPC audit-failure signal** below). |
