...
| `response.read` | Execution | A management-group scope filter dropped one or more out-of-scope agents' rows from a legacy response read — `GET /api/responses/{id}` (`surface=get`), `GET /api/responses/{id}/aggregate` (`surface=aggregate`), or `GET /api/responses/{id}/export` (`surface=export`) (#1634) — or from the dashboard `POST /api/dashboard/group-from-results` materialiser dropping out-of-scope agent ids before they enter the new management group (`surface=group_from_results`, #1712). Emitted as `result=denied` ONLY when a drop occurred (the happy, nothing-dropped path is not separately audited on these legacy handlers); `target_id` is the `instruction_id` on the legacy surfaces or the `command_id` on `group_from_results`; `detail` carries `scope_dropped=<N> surface=<get\|aggregate\|export\|group_from_results>` where `<N>` is the count of DISTINCT out-of-scope agents filtered out. The MCP `aggregate_responses` tool records the equivalent drop as its own `mcp.aggregate_responses` `result=denied` row; the visualization endpoint records it inline via `scope_dropped=<N>` on its `execution.visualization.fetch` success detail (above). Permission gate: `Response:Read` on the legacy `get`/`aggregate`/`export` surfaces — **but `group_from_results` gates on `ManagementGroup:Write`, with `Response:Read` supplying only the visibility filter**, a different combination from the other surfaces this row covers. **Current trigger scope (#1634, partial) — differs by surface:** on `get`/`aggregate`/`export`, per-management-group scoping is not yet effective for normal operators (a confined operator is 403'd at the global gate before the filter runs), so under normal RBAC operation this verb fires only in the fail-closed corrupt-`rbac.db` path (all distinct agents dropped at once). `group_from_results` is the exception (#1712): its admission gate (`ManagementGroup:Write`) is a *different* permission from the one supplying its scope (`Response:Read`), so a caller admitted via a global `ManagementGroup:Write` grant whose `Response:Read` is only management-group-scoped is a normal, reachable case — this verb fires under ordinary operation on that surface, not only on store corruption. A SIEM alert keyed on it should not expect it during normal scoped reads on the legacy surfaces until the #1634 gate lands, but should expect it routinely on `group_from_results`. |
...
| `product_pack.install` | ProductPack | A product pack install was attempted via `POST /api/product-packs`. On success: `target_id` is the new pack id, `detail` is empty, `result=success`. On rejection: `target_id` is empty (install failed pre-id-generation; pack name is attacker-controlled YAML and is deliberately not echoed into `target_id`), `detail` is the operator-facing error message, `result=denied`. Two rejection classes share `result=denied`: a **signature/policy rejection** (#802 / W7.4 — `"pack '<name>' is unsigned and signature enforcement is enabled (set --allow-unsigned-packs / ... to bypass)"` / `"signature verification failed for pack '<name>' — content may have been tampered with"` / `"pack '<name>' has signature but no publicKey — cannot verify"`), and, since the ADR-0054 Postgres migration, a **mid-install database failure** (store unreachable, or the pack/item persist transaction failed) — `detail` carries a generic message in the latter case, never the raw database error (ADR-0054). SOC 2 CC6.7: every install attempt is logged regardless of outcome. Permission gate: `ProductPack:Write`. |
