...
| `role.elevation.denied` | User | `POST /api/v1/elevate` was refused because the caller is not eligible (`users.elevation_eligible=0`) or the eligibility read failed (fail-closed). `result=denied`, `detail` names the reason. |
...
| `user.elevation_eligibility.set` | User | An admin set/cleared a user's JIT-elevation eligibility via `POST /api/v1/users/<name>/elevation-eligibility` (admin + MFA-step-up gated). `result=ok`, `detail=eligible=<bool>`; `result=error` on a store failure. |
