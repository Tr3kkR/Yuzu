...
| `dex.signal.view` | ObsType | An operator opened a **DEX signal drill-down** for one `obs_type` (`GET /api/v1/dex/signals/{obs_type}`) — top subjects, per-OS split, per-day trend, and a **most-affected devices** list naming `agent_id`s exhibiting the signal (behavioral, individual-identifying). `target_id` is the `obs_type`; permission gate `GuaranteedState:Read`, and a service-scoped API token is denied outright (`result=denied`, `target_id=` empty — the route has no per-device shape to confine a per-target check against; the empty `target_id` on denial is deliberate, not a bug: the deny fires before `obs_type` charset validation, so the raw route match is not yet safe to embed in an audit detail string). Fails closed (`503` + `Sec-Audit-Failed: true`, no data served) if the audit row cannot persist. |
