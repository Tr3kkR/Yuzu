...
| `approval.read` | Approval | A single approval was read via `GET /api/v1/approvals/{id}`. `detail` carries `status=<status>`. Permission gate: `Approval:Read`. The legacy list route `GET /api/approvals` emits **no** audit row, which matters because that is the route disclosing approval ids in full — see #1803. |
