...
| `dex.app_perf.compare` | GuaranteedState | An operator ran a `/auto` **Verify** before/after app-performance comparison — REST `GET /api/v1/dex/perf/compare`, MCP `compare_app_perf_versions`, or the dashboard VERIFY fragment. `target_id` is the cohort (management-group) id; `detail` carries `app=<name> base=<v> cand=<v> cohort=<N>` plus `view=aggregate` (the summary read) or `view=per_machine` (the per-machine drill open — the more sensitive view). **Operational, set-and-proceed**: the read is *recorded* (the accountability that stands in for the absent cohort floor — Verify deliberately has no minimum cohort size), but a dropped audit row sets the `Sec-Audit-Failed` response header rather than blocking the read. The aggregate carries no per-machine identity; the per-machine drill does. Permission gate: `GuaranteedState:Read`. |
