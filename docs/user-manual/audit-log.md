...
| `engine_principal.role.unassigned` | EnginePrincipal | A fleet-wide RBAC role was removed from an engine principal via `DELETE /api/v1/engine-principals/{id}/roles/{role}` or the MCP `unassign_engine_role` twin. `target_id` is the full `engine:<slug>` principal id; `detail=<role_name>`. `result=success` only (a not-assigned role is a `400` response with no audit row). Permission gate: `Security:Write`, plus admin + MFA step-up. |
