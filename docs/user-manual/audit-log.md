...
| `mcp.session.*` | McpSession | MCP Streamable HTTP session lifecycle (track 2f PR 1/2/3b) — `initialize`, `GET`/`DELETE /mcp/v1/`, and the streamed-POST admission path all funnel through this one action family. `mcp.session.open`: a session was minted on a successful `initialize`; `target_id` is the new session id's first 8 characters, `detail` empty (the principal is derived from `req` by the audit layer, not stamped from the minted id — the session-id prefix in `target_id` is what correlates an `open` row to its later `close`/`reject` rows, governance CONS-N2). `mcp.session.close`, `result=success`: either an explicit `DELETE /mcp/v1/` (`target_id`=session-id prefix, `detail` empty), or the server force-terminating the session because a `GET` resume's `Last-Event-ID` cursor fell outside the replay window (`detail=reason=replay_window_exceeded cid=<cid>`, #2793) — **`result=success` here describes the close itself succeeding, not the triggering request**, which still answers `404` to the client. `mcp.session.reject`, `result=failure`: every denial across all three methods and both the pre-admission and streamed-POST-internal admission checks — `Origin` not allowed, unsupported `MCP-Protocol-Version`, a missing or presented-but-invalid `Mcp-Session-Id` (unknown, expired, or another principal's — one answer for all three, no cross-principal existence oracle, Decision 15(a)/CH-8), a session or stream concurrency cap on `initialize`/`GET` (`per_principal_stream_cap`/`global_stream_cap`), `GET`-specific causes (`not_acceptable`, `stream_handover_pending`, `terminal_poisoned`), and the streamed-POST admission denials (`--mcp-enable-streamed-post`, 2f PR 3b) — `post_per_principal_cap`/`post_global_cap`/`post_pin_slots`/`post_duplicate_request_id`/`post_unknown_session`, whose `detail` additionally carries `surface=post dispatched=false` since a streamed-POST denial happens before any execution row exists. `target_id` is the session-id prefix where one was presented, else empty; `detail=reason=<X> cid=<correlation_id>` for every cause. The full closed `reason=` set (and which values are `mcp.session.close` vs `mcp.session.reject`) is in `docs/observability-conventions.md`. Every session-id prefix reaching any of these three verbs is sanitized (`sanitize_detail_value`) since the header is attacker-controlled until it validates — raw bytes could otherwise inject the `;`/`=` field separators or CR/LF a `k=v;k=v` `detail` string relies on. |
