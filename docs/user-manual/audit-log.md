...
**Gap — an approval leaving `approved` is not audited.** `instruction.approve` / `instruction.deny` record the grant decision, and the `mcp.<tool>` row above records every redemption attempt. Nothing records an approval *losing* its grant. The 7-day TTL sweep moves a row to `expired` from inside `ApprovalManager::submit()` with no audit row and no principal, and the direct-database bulk revoke documented in [Server Administration](server-admin.md) — currently the only way an administrator can retire granted approvals — is likewise invisible here, and indistinguishable from that sweep. A SIEM cannot answer "who withdrew this authorisation" from the audit store today; the answer lives in the operator's change record. Tracked as #2761 (an audited `Approval:Revoke` operation) and #2738 (the TTL's own defects).
