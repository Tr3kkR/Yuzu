...
| Row | Requirement | Verdict | Basis / plan |
|---|---|---|---|
| 22 | App-specific resource consumption (open Q: granularity) | **Partial — strong** (A2, 2026-06-12) | Top-10-by-CPU ∪ top-10-by-working-set per-APP samples every 30 s → `procperf_live`/`procperf_hourly` edge tiers (7 d raw + 31 d per-app hourly), names-only (never cmdlines), redaction-aware, own `procperf_enabled` toggle. Granularity answer: 30 s edge-side, raw stays on-device (federated). Collected + raw-SQL queryable; per-app dashboard view → F2 |
