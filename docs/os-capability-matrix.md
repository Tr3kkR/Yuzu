...
| filesystem_posture | 🟡 | ✅ | ✅ | `mounts`/`quotas`/`snapshots`, all volume-level, read-only inventory. Linux: `/proc/self/mountinfo`, `quotactl(Q_GETFMT)`, Btrfs/LVM snapshot capability (not a live inventory). macOS: `getmntinfo`, no per-user/group APFS quota equivalent, APFS snapshot enumeration. Windows: ALL THREE legs — `mounts` (`FindFirstVolumeW`/`GetVolumeInformationW`/`GetDriveTypeW`/`GetDiskFreeSpaceExW`), `quotas` (`IDiskQuotaControl`, read-only) and `snapshots` (`FSCTL_SRV_ENUMERATE_SNAPSHOTS`) — are **compile-verified only — not exercised against a live Windows host in this change**.
...
| filesystem_posture | mounts | windows | constrained | 1 | FindFirstVolumeW + GetVolumeInformationW + GetDiskFreeSpaceExW | enumerates local volumes only; a mapped network drive is not a volume and is not listed, and no per-mount option string exists so that column reads '-'; compile-verified only -- not exercised against a live Windows host in this change |
...
| filesystem_posture | snapshots | windows | constrained | 1 | FSCTL_SRV_ENUMERATE_SNAPSHOTS | reports the Previous-Versions (@GMT) shadow-copy tokens the volume exposes; a DeviceIoControl failure is reported distinctly from an empty snapshot set and degrades the result status; compile-verified only -- not exercised against a live Windows host in this change |
