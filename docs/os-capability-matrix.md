...
| filesystem_posture | 🟡 | ✅ | ✅ | `mounts`/`quotas`/`snapshots`, all volume-level, read-only inventory. Linux: `/proc/self/mountinfo`, `quotactl(Q_GETQUOTA)`, Btrfs/LVM snapshot capability (not a live inventory). macOS: `getmntinfo`, no per-user/group APFS quota equivalent, APFS snapshot enumeration. Windows: `GetVolumeInformationW` mounts; the quota (`IDiskQuotaControl`, read-only) and snapshot (VSS) legs are **compile-verified only — not exercised against a live Windows host in this change** |
