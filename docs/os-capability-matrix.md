...
| runtimes | 🔜 | ✅ | 🔜 | `dotnet`/`jvm` installed-runtime inventory, read-only and zero-subprocess (rung 1). Linux only: directory-name walks and `release` file reads over an injected root (`runtimes_linux_parsers.hpp`). macOS and Windows are declared `planned`; until those legs ship those hosts answer `status\|<action>\|unsupported\|<os>:planned`. See `agents/plugins/runtimes/README.md` |
