...
| **PII / sensitive-data discovery** (`pii_scan` — file content scan against ~190 financial/passport/driver's-licence/national-ID rules across ~45 countries; on-write realtime via the existing filesystem-trigger primitive) | ✅ | ✅ | ✅ | `agents/plugins/pii_scan/src/pii_scan_plugin.cpp` — genuinely cross-platform (`std::filesystem` walk, no per-OS branch). **On-access (on-read) scanning is out of scope, not just unported** — it needs a kernel-level hook (Windows minifilter / Linux fanotify permission events / macOS Endpoint Security AUTH events), a categorically different undertaking Yuzu is deliberately not building (EDR territory) |
