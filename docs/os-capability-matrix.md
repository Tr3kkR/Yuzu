...
| **Vulnerability identity collection** (`vuln_scan.inventory` → server-side NVD/OVAL/VEX) | 🟡 app identity (name/publisher/version) | ✅ full NEVRA (rpm adds packager + *stored* signature status) | 🟡 app identity | `vuln_scan` plugin `inventory` action → `vuln_identity.hpp` (Linux dpkg/rpm/pacman/apk + `/etc/os-release`; Win registry Uninstall 64/32/HKCU; macOS `system_profiler`+brew). **Command-path, distinct from the daily-sync inventory row above.** Windows/macOS have no NEVRA/package-signature (ADR-0018 Lane 3 — honest-empty). Matching is server-authoritative (ADR-0018) |
