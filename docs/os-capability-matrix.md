...
| **SparkEngine detection mechanisms** (ADR-0021 Stage-2; observe-only at rung 1) | ✅ file + registry + service | 🟡 service only | ⛔ none | `make_{file,registry,service}_mechanism()` in `spark_mechanism.hpp` (returns `nullptr` off-platform → type left unregistered): File/Registry Windows-only, Service Win-SCM + Linux-sd-bus, macOS none. Surfaced fleet-wide via `yuzu_fleet_spark_mechanisms{os,mechanism}`. Not yet wired to drive detection/enforcement (that is rung 2/3) |
