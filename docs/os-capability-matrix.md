...
| printing | clear_queue | windows | supported | 1 | winspool SetJobW JOB_CONTROL_CANCEL on one job id | cancel measured only under Administrators and SYSTEM (SYSTEM is today's agent identity, #1442); a least-privileged identity cancelling a job it does not own is unmeasured (PRINTER_ACCESS_USE is a projection) - see docs/agent-privilege-model.md |
