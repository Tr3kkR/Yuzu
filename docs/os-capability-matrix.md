...
| privacy_permissions | permissions | windows | supported | 1 | HKLM ProfileList enumeration, then each real profile's SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\CapabilityAccessManager\\ConsentStore via its loaded HKU\\<SID> hive or an offline NTUSER.DAT mount (RegLoadKeyW, SeBackup/SeRestore), plus the same HKLM ConsentStore path | LocalSystem's own HKCU is not read; an HKLM value overrides a profile's only when successfully read; the HKLM mirror's shape is unconfirmed on real hardware |
