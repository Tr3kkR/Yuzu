...
| runtimes | 🔜 | ✅ | 🔜 | `dotnet`/`jvm` installed-runtime inventory, read-only and zero-subprocess (rung 1). Linux only: directory-name walks, `release` file reads and a `bin/java` probe for release-less homes over an injected root (`runtimes_linux_parsers.hpp`), skipping network mounts read from `/proc/self/mountinfo`. macOS and Windows are declared `planned`; until those legs ship those hosts answer `status\|<action>\|unsupported\|<os>:planned`. See `agents/plugins/runtimes/README.md` |
