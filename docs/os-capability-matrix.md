...
| platform_security | ✅ | ✅ | 🟡 | `secure_boot` and `code_integrity`, read-only. Windows: `RegQueryValueExW` on `SecureBoot\State`, `CI\Policy`, `DeviceGuard` and `Lsa` (rung 1). Linux: efivarfs `SecureBoot-*`/`SetupMode-*` plus `/sys/kernel/security/lsm` and `lockdown` (rung 1). macOS: `secure_boot` unsupported (no public API); `code_integrity` is `spctl --status` + `csrutil status` declared rung-2 argv leaves. See `agents/plugins/platform_security/README.md` |
