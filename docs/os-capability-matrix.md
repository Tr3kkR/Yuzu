...
| execution_artifacts | ✅ | ⛔ | ⛔ | Windows-only forensics reads (ShimCache, Amcache `InventoryApplicationFile`, Prefetch). Off-Windows every action returns the fixed `unsupported\|windows_only_artefact` sentinel, rc 1 — `execution_artifacts_plugin.cpp`'s `#else` branch. `Forensics` securable, `AdminOrApproval`, single-target, default-off via the server plugin-config kill switch; see `agents/plugins/execution_artifacts/README.md` |
...
| windows_optional_features | ✅ | ⛔ | ⛔ | Windows-only. DISM `list`/`info` — optional OS feature enable/disable/pending state via `DismOpenSession(DISM_ONLINE_IMAGE)` + `DismGetFeatures`/`DismGetFeatureInfo`, resolved at runtime from `System32\DismApi.dll` (no ADK, no `dism.exe` subprocess, no WMI). Read-only — none of the eight resolved exports mutates |
