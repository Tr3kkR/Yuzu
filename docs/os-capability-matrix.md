...
| disk_actions | volumes | macos | constrained | 1 | IOKit IOMedia provider walk + getmntinfo(3) MNT_NOWAIT | one row per IOMedia object, keyed on that object's BSD name, carrying the physical whole disk that backs it and any mount points it serves; the physical disk is resolved by walking the IOKit provider chain, so a volume inside a synthesized APFS container correctly reports the underlying drive rather than the container; a media object that serves no mount point still reports its backing disk, and reports '-' for mount points rather than being omitted |
