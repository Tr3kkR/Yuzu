...
| **DEX — reliability signals** (crashes, hangs, service/boot, storage, battery, …) | ✅ | 🟡 growing | 🟡 | Win: `dex_observer.cpp`/`dex_win_poll.cpp`; Linux: `dex_linux_collector.cpp`/`dex_linux_proc.cpp`/`dex_linux_storage.cpp`; macOS: `dex_macos_collector.cpp` (DiagnosticReports/OSLog/IOKit). Catalogue: `docs/dex-signal-catalog.md` |
