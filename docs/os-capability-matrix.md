...
| **SparkEngine mechanisms** (file / registry / service; observe-only) | ✅ file + registry + service | 🟡 service only — **and NONE in a container** (see note) | ⛔ none | File/Registry are Windows-only; Service is Win-SCM + Linux-sd-bus; macOS none. **Registered ≠ functional:** a mechanism that starts but cannot bind its OS facility is marked INERT and EXCLUDED from the capability CSV (`sd_bus_open_system()` failing in `spark_service.cpp`; `OpenSCManager` denied; threadpool/IOCP failing). On Windows, a Registry sweeper or File worker that fails three consecutive passes at runtime is also marked INERT and excluded, because it cannot serve watches until a pass succeeds (its watches are still accepted), and is counted again once a pass succeeds. A Service mechanism's worker death is deliberately refuse-not-inert: it stops accepting new watches but never sets `inert`, so `service` stays in the CSV (`spark_service.cpp:974-987` Linux, `:2431-2441` Windows). **The container case is common:** `Dockerfile.agent` installs `libsystemd0` but a container has **no system bus**, so a containerised Linux agent registers Service INERT and ends up with **zero** spark capability (`spark_running=1`, empty `spark_mechs` CSV, no `{os=linux,mechanism=*}` series). Fleet view `yuzu_fleet_spark_mechanisms{os,mechanism}`; an inert mechanism shows as a per-mechanism GAP: `yuzu_fleet_spark_reporting{os}` exceeding that one mechanism's series, for an OS where every agent is expected to run it (query in `docs/user-manual/metrics.md`). Do NOT compare `_reporting` with the SUM over mechanisms: a healthy Windows agent counts once in each of file, registry and service, so the sum is about 3N against N reporting, and `file` dropped fleet-wide still sums to about 2N (it stays quiet until the average agent has lost more than two of its three mechanisms, so it cannot detect one inert mechanism on Windows; on macOS, which has no mechanism by design, it would always read as a gap) |
