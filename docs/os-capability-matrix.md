...
| runtimes | 🔜 | 🔜 | ✅ | `dotnet`/`jvm`/`python` installed-runtime inventory, read-only and zero-subprocess (rung 1). Linux only in this PR: directory-name walks and `release` file reads over an injected root (`runtimes_linux_parsers.hpp`). macOS and Windows are declared `planned` — each follows as its own PR (peripherals PR9.1a2 precedent); until then those hosts answer `status\|<action>\|unsupported\|<os>:planned`. See `agents/plugins/runtimes/README.md` |
...
<!-- TODO(integrator, runtimes): this block has no `runtimes` rows yet. Regenerate it on Linux with tools/capmatrix-gen against the built plugin .so files (or restore the untouched rows from origin/dev and add runtimes' nine rows), then run `python3 tools/plugin-doc-gen/plugin_doc_gen.py --all`. Never hand-edit the block. Remove this comment afterwards. -->
...
| runtimes | python | linux | supported | 1 | /usr/bin/python3* names + /usr/lib/python3*, /usr/local/lib/python3* directory walk | - |
| runtimes | python | macos | planned | 1 | /Library/Frameworks + CommandLineTools Python3.framework Versions + Cellar/python@3.* | follows as its own PR (peripherals PR9.1a2 precedent) |
| runtimes | python | windows | planned | 1 | PEP 514 PythonCore keys | follows as its own PR (peripherals PR9.1a2 precedent) |
