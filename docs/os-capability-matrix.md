...
| system_hardening | ✅ | ✅ | ✅ | One read-only `posture` action, rung 1 on every leg, no spawn. Linux: eleven allowlisted `/proc/sys` keys via `open`/`read` with the errno classified (absent vs unreadable). macOS: four allowlisted `sysctlbyname` keys (`kern.securelevel`, `kern.coredump`, `kern.sugid_coredump`, `kern.bootargs`). Windows: the `Session Manager\kernel` `MitigationOptions`/`MitigationAuditOptions` registry values plus `GetProcessMitigationPolicy` on the agent's own process. An absent key is its own `absent` row and never a failure; only an unreadable key downgrades the result to CONSTRAINED/PARTIAL (PERMISSION_DENIED when the read was refused), and neither fails the command |
