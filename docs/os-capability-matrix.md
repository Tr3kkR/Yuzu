...
| **━━ Agent plugins (54) — per-plugin build/availability ━━** | | | | Per-OS via platform macros / per-OS TUs (`agents/plugins/*/src/*`). 42 fully cross-platform, 6 Windows-only (`rdp_control`, `registry`, `sccm`, `wmi`, `windows_optional_features`, `execution_artifacts`), 3 uneven (`tar` — richest on Windows; `msi_packages` — Win+macOS, no Linux; `disk_actions` — Win+macOS, Linux declared unimplemented), 1 macOS-constrained (`interaction` — GUI-less daemon). "Full" = the plugin builds and its core actions work on that OS; a plugin can be cross-platform yet expose a few OS-specific actions (noted) |
