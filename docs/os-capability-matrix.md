...
_Last hand-updated: 2026-07-21._
...
| **DEX — per-app file version** (on procperf + on crash/hang signals; the `(name, version)` identity) | ✅ | ⛔ (emits `""`) | 🟡 crash/hang via `.ips` (procperf pending) | Win: procperf reads `VS_FIXEDFILEINFO` via `GetFileVersionInfo` (`tar_proc_perf.cpp`, schema v4); crash/hang read WER `AppVersion` (`dex_signal_catalog.cpp`, migration {8}); both canonicalized by `yuzu::util::canon_version`. macOS crash/hang now read `CFBundleShortVersionString` from the `.ips` (`dex_macos_signals.cpp`, via `canon_version`); Linux still emits `""` (unknown bucket) — capture is a follow-up (`COREDUMP_PACKAGE_VERSION` / ELF `.note.package`) |
...
| **Network quality** (`/network`: throughput / retransmit / RTT) | 🟡 throughput + retransmit (no RTT) | ✅ all three | 🟡 throughput only (retransmit + RTT deferred) | `net_quality_sampler.cpp`; per-OS detail in `docs/user-manual/network.md` "Platform coverage" |
...
| **TAR — TCP connections** (capture source) | ✅ | ✅ | 🟡 | `tar_schema_registry.cpp` `tcp` def (Win `kSupported` `iphlpapi`/`GetExtendedTcpTable` poll; Linux `kSupported` `/proc/net/tcp{,6}` poll; macOS `kSupportedConstrained` — connection lifecycle is now **event-driven** via `nstat` `SRC_ADDED`/`SRC_REMOVED`, with the `proc_pidfdinfo` poll retained as fallback/seed, NOT Endpoint Security — ES has no TCP/socket events); collector `tar_network_collector.cpp` (macOS leg), event client `agents/plugins/tar/src/tar_netqual_nstat.cpp` |
| **TAR — network quality** (`netqual`, capture source, opt-in) | 🟡 | ✅ | 🟡 | `tar_schema_registry.cpp` `netqual` def (Win `kSupportedConstrained` ESTATS `GetPerTcpConnectionEStats`, admin-only — ADR-0020; Linux `kSupported` netlink `INET_DIAG` `TCP_INFO`; macOS `kSupportedConstrained` — per-socket `tcp_connection_info` via the same `nstat` kernel control, **root required for system-wide flow visibility** else `netqual_capture_method=none`, layout-unstable private struct with a runtime self-check fallback); collector `tar_network_collector.cpp` (macOS leg), client `agents/plugins/tar/src/tar_netqual_nstat.cpp` (ADR-0020, opt-in) |
...
| **Live device snapshot — DNS resolver cache** | ✅ | ⛔ (no portable resolver cache) | ⛔ (OS exposes no resolver-cache contents; `dscacheutil -cachedump` defunct on macOS 26) | `network_config/dns_cache` (`DnsGetCacheDataTable` on Windows; macOS returns a `dns_cache\|unsupported` sentinel, no shell-out — see `darwin-compat.md`) |
...
| **Antivirus posture** (`antivirus` plugin: `products` + `status`) | ✅ SecurityCenter2 products + Defender status | 🟡 process/dir detection only (ClamAV/CrowdStrike/Sophos), no `status` leg | ✅ `products`: XProtect.bundle version probe (`unknown` when unreadable) + endpoint-security system extensions (`systemextensionsctl`, unprivileged) + process fallback; `status`: XProtect definition version + bundle mtime + Remediator/MRT versions | `agents/plugins/antivirus/src/antivirus_plugin.cpp` per-OS legs; pure parsers `antivirus_parsers.hpp` (tested every host) |
