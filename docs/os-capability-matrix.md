...
| **PII / sensitive-data discovery** (`pii_scan` — file content scan against ~190 financial/passport/driver's-licence/national-ID rules across 60 countries; on-write realtime via the existing filesystem-trigger primitive) | ✅ | ✅ | ✅ | `agents/plugins/pii_scan/src/pii_scan_plugin.cpp` — genuinely cross-platform (`std::filesystem` walk, no per-OS branch). **On-access (on-read) scanning is out of scope, not just unported** — it needs a kernel-level hook (Windows minifilter / Linux fanotify permission events / macOS Endpoint Security AUTH events), a categorically different undertaking Yuzu is deliberately not building (EDR territory) |
...
| pii_scan | ✅ | ✅ | ✅ | portable — `std::filesystem` walk + RE2 match, no platform macros. `enable_realtime`/`disable_realtime` register/unregister an agent `TriggerEngine` filesystem trigger (mtime-polling, itself already cross-platform). Per-action detail: the generated plugin-action table below |
