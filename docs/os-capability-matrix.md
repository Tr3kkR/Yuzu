...
| privacy_permissions | ✅ | 🟡 | 🟡 | Per-app camera/microphone/location/full-disk-access grants, read-only; `Forensics` securable, default-off behind the server plugin-config kill switch, single-target. Windows: each real profile's ConsentStore (live `HKU` hive or offline `NTUSER.DAT` mount) plus the `HKLM` mirror, measured on one non-MDM host. macOS: the system and per-user `TCC.db` read immutably in-process, `denied` until the agent has Full Disk Access. Linux: xdg-desktop-portal PermissionStore over the agent's own session bus only, normally `unavailable` under the shipped service. See the plugin README |
