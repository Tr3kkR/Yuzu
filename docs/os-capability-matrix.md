...
| pkg_inventory | 🟡 | ✅ | ✅ | Machine-scope package-manager inventory, read-only, zero subprocess. `managers`: Linux tool presence + manager config facts (identity/config only), macOS Homebrew prefix layout over both `/opt/homebrew` and `/usr/local`. `packages`: macOS Homebrew Cellar/Caskroom directory names; Linux is `unsupported` by design (`installed_apps` owns the Linux roster). Windows legs are declared `planned` (Chocolatey/winget follow as their own PR). See `agents/plugins/pkg_inventory/README.md` |
