...
| wifi | list_networks | linux | constrained | 1 | NetworkManager D-Bus (sd-bus) | returns the networks NetworkManager last observed rather than starting a new scan, so results can lag a live scan by NetworkManager's own scan interval; where NetworkManager reports no completed scan the agent falls back to nmcli, which does rescan, then to iw/iwlist. The D-Bus path is newly introduced and has not yet been validated on hardware in the field, so a host may serve results from the nmcli fallback instead; output is identical either way |
...
| wifi | connected | linux | constrained | 1 | NetworkManager D-Bus (sd-bus) | reports the network interface (for example wlan0) in the connection column, where earlier agent versions reported the NetworkManager connection profile name; falls back to nmcli, then to iwconfig. The D-Bus path is newly introduced and has not yet been validated on hardware in the field, so a host may serve results from the nmcli fallback instead; output is identical either way |
