...
- **M2/M5 — `module_enabled` must NOT default `true`** until both the §5 edge risk-filter and the rollup wiring are live (rollup is now wired as of M1; the risk-filter lands with the first high-volume collector).
