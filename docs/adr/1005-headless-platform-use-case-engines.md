...
3. Exceptions recorded here accrete into the "exception ledger" the standing review question refers to:
   ...
   - **2026-08-24 — retirement outside the deprecation cycle: the five
     `security.vuln_scan.*` instruction definitions (roadmap PR4.4).** The
     `vuln_scan` agent plugin and all five of its operator- and MCP-reachable
     instruction definitions (`scan`, `cve_scan`, `config_scan`, `summary`,
     `inventory`) were removed in a single change, without the announce →
     ≥90-day → remove cycle that `docs/api-versioning-policy.md` requires of a
     shipped operator surface. That policy states the ledger "records any surface
     retired outside it (there should be none)" — so this is that record, and it
     is an exception, not a precedent.
     - **Authority.** Maintainer decision, 2026-08-21 (Alex), on the basis of
       ADR-0018 (the agent collects, it never decides) and ADR-0028 Decision 2
       (the plugin was considered and rejected as the host for
       component-inventory collection). It supersedes ADR-0028's own
       three-phase rollout ordering; the dated corrections are recorded there,
       including in its Ratification section.
     - **Scope, and what is NOT claimed.** No versioned REST path and no MCP
       tool NAME changed — the five ids are content-plane
       `InstructionDefinition` identities, and the generic `execute_instruction`
       surface that reaches them is untouched. The narrower reading, that the
       cycle binds REST paths and MCP tool names rather than content ids, is
       available and is why this is recorded as an exception rather than
       treated as a policy breach.
     - **Cost accepted.** An interim gap with no agent-side vulnerability-scan
       entry point, and — because boot content import upserts with
       conflict-skip and never prunes — the five definitions persist in
       already-upgraded operator databases and fail at the dispatch chokepoint
       until removed by hand (`docs/user-manual/upgrading.md`).
     - **Re-creation, not restoration.** `security.vuln_scan.scan` returns when
       the ADR-0028 `component_inventory` collector lands, **re-created**
       against that collector rather than repointed. Two obligations transfer
       with it and are recorded in ADR-0028's Ratification discharge:
       fleet-wide collector adoption before the id is republished, and the
       one-time RBAC re-audit — grants naming `security.vuln_scan.*` survive
       the deletion, are inert while no such definition exists, and would
       re-attach if the id returned for a materially more invasive capability.
