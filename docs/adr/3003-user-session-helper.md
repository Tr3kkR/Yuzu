...
The Yuzu agent runs as a highly-privileged system daemon on every OS — root LaunchDaemon (macOS), LocalSystem service (Windows, #1442), dedicated unprivileged system daemon (Linux). This is the correct trust boundary for a fleet daemon, but it is, by construction, unable to reach a logged-in user's interactive session: session-0 isolation (Windows), no Aqua/GUI session or login keychain (macOS), no user D-Bus / `DISPLAY` / `XDG_RUNTIME_DIR` (Linux). A recurring class of capability is dead-ended on this and today fails *honestly* rather than working:
