...
- **Link formation forms a link only when EXACTLY ONE active SCIM resource matches** the link-claim value. Zero matches → no link (normal). More than one → **no link**, and increment an `external_id`-ambiguity signal (folds into the D2 detector family) so the misconfiguration is loud, never a silent wrong-link. This is the load-bearing safety and works on existing data.
- **Add a partial-unique index** on `external_id` where non-empty (the correct SCIM posture). The migration must **detect pre-existing duplicates and fail closed** (refuse to open, per the store's ADR-0012 posture) rather than silently skip index creation — a store that opens with a violated uniqueness assumption is exactly the mis-link hazard, deferred.
