...
|---|---|
| 0016 (agent daily-sync framework) | Mechanism reused (SyncSource/scheduler/hash-skip); a new, separately-governed source (Decision 3) — independent budget/store even though its floor cadence is daily too, plus an event-linked trigger keyed to `installed_software`'s own change detection. **Open dependency (Decision 1):** `installed_apps`/`sync_source_installed_software` needs a small addition (capture `InstallLocation`/`.app` bundle path) to anchor this ADR's walk roots — must land before or alongside implementation (recommended: a dated ADR-0016 amendment section, matching the `device_ci`/blob-v2 precedent), not after. **Update (2026-09-21): landed for the operator `list` action only — `install_location` + `bundle_id` columns (roadmap PR10.1-a, definition `crossplatform.software.inventory` 1.1.0); the `sync_source_installed_software`/blob-v3 addition remains open and is deliberately NOT folded in, so a `component_inventory` PR that anchors on the daily-sync blob must still land that amendment first.** |
