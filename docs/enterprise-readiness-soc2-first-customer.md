...
Both PostgreSQL stores fail **closed** on an unreachable database (ADR-0007 — the server refuses to boot), and are gated into `/readyz` and `/healthz`. The shared decommission/retention-purge gap (no per-device delete across either store, matching the SQLite per-agent stores) is tracked as **#1666** (platform-lifecycle follow-up; the `offline_endpoint_store` half is **#1545**); until it lands, `last_seen` is the canonical "is this device still active" filter for asset-management counts.
