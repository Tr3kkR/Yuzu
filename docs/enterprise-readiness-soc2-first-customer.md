...
|---|---|---|---|---|---|
| Audit trail | `audit.db` | Operator activity (security-relevant) | 365 days (a **floor** — see below) | `AuditStore::cleanup_once` on the `run_cleanup` thread — clock-guarded and capped (#2360): the pass declines once when it would expire every datable row or when the persisted clock reading shows an implausible step, and deletes at most 25,000 rows per pass, oldest first. `ttl_expires_at` is stamped at INSERT and never rewritten, so changing the setting does not re-date existing rows. Counters: `yuzu_server_audit_clock_anomaly_skips_total`, `..._cleanup_failed_total`, `..._retention_cap_reached_total`, `..._rows_deleted_total`. | `audit_retention_days` |
