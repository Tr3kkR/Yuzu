...
| Boot-window process trace | `procboot.etl` (kernel AutoLogger file, Windows) | Boot-window process start/stop, **names-only, no user**. Source for the one-time boot backfill into `$Process_Live`. | Configured by the dev install script only (not the production installer yet) | Circular 16 MB file (not `retention_days`-governed); removed on dev uninstall | `process_enabled` gates the backfill insert; file lifecycle via `install-agent-user.ps1` |
...
- **Aggregate-by-default, individual-by-exception.** The `/dex` headline is a fleet rollup. The per-device drill-down (the individual-identifying view) is separately permission-gated (`GuaranteedState:Read`) and **every open is audit-logged** (`dex.device.view` in `audit.db`, 365-day retention) — access transparency for the individual-level read path.
- **The machine-health audit exemption (named policy decision, F2a 2026-06-12).** Aggregate fleet-performance and cohort-benchmarking reads (the `/dex` Performance tab, `GET /api/v1/dex/perf/{fleet,cohorts,devices}`, the matching MCP tools) are **deliberately not per-read audited**: they carry no behavioral history, no event detail, no `detail_json` — only device-state telemetry (CPU/commit/disk levels) and operator tag values, which are not individually-identifying behavioral data. The audited boundary is unchanged: the individual device *behavioral* view (`dex.device.view`), the machine-health *device query* (`dex.device.perf.query`), and the usage-class per-application panel (`dex.device.procperf.query` — whose result polls require the audited dispatch's `command_id`, so no usage-class read escapes the count). This paragraph is the citable policy line for CC7.2 evidence reviews.
