...
Both PostgreSQL stores fail **closed** on an unreachable database (ADR-0007 — the server refuses to boot), and are gated into `/readyz` and `/healthz`. Beyond boot, `software_inventory_store` reads are **authoritative** (ADR-0016 §7): a transient query-time degradation after a healthy boot (pool-acquire timeout, query error) surfaces as a `kInternalError` from `query_installed_software` (and a `503` A4 envelope from the REST sibling `GET /api/v1/inventory/software`), never a successful empty result — so a fleet vulnerability query can never read a transient backend hiccup as "installed nowhere". **Both read channels (MCP + REST) enforce per-operator management-group scope at the API layer** — out-of-scope device rows are actively dropped and the drop is audited (a distinct `denied` event with the dropped-device count), so one operator cannot read another's device software (CC6.1 tenant-isolation evidence; the affirmative answer to the CAIQ confidentiality question for this surface). The shared decommission/retention-purge gap (no per-device delete across either store, matching the SQLite per-agent stores) is tracked as **#1666** (platform-lifecycle follow-up; the `offline_endpoint_store` half is **#1545**); until it lands, `last_seen` is the canonical "is this device still active" filter for asset-management counts.
...
- **Aggregate-by-default, individual-by-exception.** The `/dex` headline is a fleet rollup. The per-device drill-down (the individual-identifying view) is separately permission-gated (`GuaranteedState:Read`) and **every open is audit-logged** (`dex.device.view` in `audit.db`, 365-day retention) — access transparency for the individual-level read path. **The individual read path has multiple channels with identical gating and audit:** the `/device` dashboard DEX lens AND the machine-readable REST endpoints `GET /api/v1/dex/devices/{id}` (emits `dex.device.view`), `POST /api/v1/dex/devices/{id}/live` (emits `device.live.uptime` / `device.live.processes`, `result=requested`, audited **before** dispatch), and `GET /api/v1/dex/devices/{id}/app-perf` (the retained per-device app-performance history; emits **`dex.device.app_perf.view`** — a separate verb kept distinct from `dex.device.view` / `dex.device.procperf.query` so usage-class reads stay separately countable). All these REST endpoints are **fail-closed**: a non-persistable audit row returns `503` + `Sec-Audit-Failed: true` and serves no PII / dispatches no probe. The per-device app-perf history is **also** reachable from the `/device` dashboard DEX drill (the "Application performance over time" panel — a central B1 read, no live query, no `Execute`), which emits the **same `dex.device.app_perf.view`** verb under the dashboard **set-and-proceed** posture (a non-persistable audit row sets `Sec-Audit-Failed: true` but still renders — the same posture every dashboard PII fragment uses, distinct from the REST fail-closed surface); there is **no MCP twin** (the REST fail-closed contract cannot be expressed on MCP's set-and-proceed posture). All channels are in the data inventory for this read path, so a Workstream-E review of "what can trigger an individual behavioral read" must list both REST and the dashboard fragments, not only one. **Accepted residual risk (dashboard set-and-proceed, platform-wide; established #1647):** a dashboard per-device behavioural-PII fragment renders even when its access-audit row cannot durably persist (transient `audit.db` outage), so a strict "every access produces a durable SIEM row" guarantee — e.g. a German BetrVG §87(1)(6) works-council clause — is **not** fully met on the dashboard surface. Compensating controls: (a) the `Sec-Audit-Failed: true` response header on the failing call; (b) the `yuzu_server_audit_emit_failed_total` Prometheus counter (alertable); (c) the web-server access log (path, principal, status, timestamp). Guidance: works-council-strict deployments must treat the **REST fail-closed** endpoints (`GET /api/v1/dex/devices/{id}`, `…/app-perf`) as the canonical evidence channel — they return `503` and serve no PII when the audit row cannot persist. This posture is accepted because a transient audit outage must not blank an operator's dashboard, and the detectable-failure signals above bound the gap; it applies to **every** dashboard PII fragment, not only app-perf. This is the citable CC7.2 policy line for the dashboard read surface.
