...
**Availability posture under PostgreSQL degradation (#2703 Gate 7 merge-slice, availability-hardening pass) — assess alongside Workstream D, do not read as availability resilience.** The deny-on-degrade behaviour above is not instantaneous on every blip: for up to a **bounded ~5 s window** after the last confirmed-good cross-replica refresh, a replica continues serving authorization decisions from its last-known-good in-memory permission cache rather than dropping trust on a single transient pool hiccup — a deliberate bounded-staleness-for-continuity tradeoff, distinct from (and layered underneath) the ~1 s propagation window above. Once that 5 s bound is exceeded — or once a **fail-fast breaker** observes 2 consecutive pool-acquire/query failures, whichever comes first — the cache is invalidated and the store denies rather than continuing to serve increasingly stale data; the breaker then denies every subsequent call *without* touching the pool for a ~1 s cooldown before allowing one probe, so a sustained outage collapses to a fast, predictable deny rather than every caller separately re-discovering the same failure (each hot-path pool-acquire attempt is itself bounded to 250 ms). **The CAIQ-relevant consequence: this is a confidentiality-over-continuity design, not an availability guarantee.** During sustained PostgreSQL saturation or loss, authorization checks are denied fleet-wide once the bounds above are exceeded — every gated operation becomes unavailable, which is itself a control-plane availability impact, not evidence of resilience to it. This posture must be assessed together with Workstream D (reliability) controls and PostgreSQL's own high-availability posture, never cited on its own as an availability answer.
