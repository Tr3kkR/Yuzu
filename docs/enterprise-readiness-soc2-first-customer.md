...
> **Retention-guard coverage is partial, deliberately.** On the server, only the audit trail is clock-guarded and capped (#2360); on the agent, the TAR edge warehouse is (#2361 - see the retention note under the agent-side section below). The response store and the Guardian-event store still issue a bare `DELETE ... WHERE ttl_expires_at < now` driven by the server's wall clock, so a forward clock step can empty either in one statement. They are lower-stakes than the evidence trail (operational telemetry, not the SOC 2 control record) and are queued for the same treatment in #2508. An auditor reading this table should not infer that "clock-guarded" applies to every row in it. **Nor that "clock-guarded" means fully guarded where it does apply:** the audit trail's own guard has no missing-anchor trigger (#2579, detailed in its row above), so a pass beginning with NO stored reading on a forward-skewed host deletes without declining and keeps doing so until the rows stamped before the skew are exhausted. The honest answer to "can the audit trail be silently truncated?" is therefore **yes, under a forward-skewed clock, and not bounded to a single pass** - it drains any expired backlog and then keeps deleting rows stamped before the skew ahead of their window, with no reliable retrospective test to confirm it afterwards. It is self-limiting rather than permanent: rows written under the skew are stamped from the same clock the cutoff is read from, so they keep their full window, and a forward step of `S` on a window of `W` works through the pre-skew rows and stops about `W - S` later (a still-drifting clock is the open-ended case). That is why #2579 is tracked as a decision rather than closed.
