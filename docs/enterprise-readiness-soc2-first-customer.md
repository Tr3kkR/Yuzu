...
Both tiers are device-local; raw rows never leave the endpoint except via an operator-initiated, permission-gated query (the `/dex` device-perf panel runs a live `$Perf_Hourly` query, Execute-gated and audited `dex.device.perf.query`). Retention uses the same `RetentionType::kTimeBased` mechanism as the other TAR sources, so per-category retention controls apply.
