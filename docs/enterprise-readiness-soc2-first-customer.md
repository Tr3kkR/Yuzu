...
> **Retention-guard coverage is partial, deliberately.** On the server, the audit trail (#2360) and the **response store** (ADR-0039, this PR — the #2508 treatment for that store) are clock-guarded and capped; on the agent, the TAR edge warehouse is (#2361 - see the retention note under the agent-side section below). The **Guardian-event store** still issues a bare `DELETE ... WHERE ttl_expires_at < now` driven by the server's wall clock, so a forward clock step can empty it in one statement. It is lower-stakes than the evidence trail (operational telemetry, not the SOC 2 control record) and is queued for the same treatment in #2508. An auditor reading this table should not infer that "clock-guarded" applies to every row in it.
