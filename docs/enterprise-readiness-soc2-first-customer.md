...
Both PostgreSQL stores fail **closed** on an unreachable database (ADR-0007 — the server refuses to boot), and are gated into `/readyz` and `/healthz`. Beyond boot, `software_inventory_store` reads are **authoritative** (ADR-0016 §7): a transient query-time degradation after a healthy boot (pool-acquire timeout, query error) surfaces as a `kInternalError` from `query_installed_software`, never a successful empty result — so a fleet vulnerability query can never read a transient backend hiccup as "installed nowhere". The shared decommission/retention-purge gap (no per-device delete across either store, matching the SQLite per-agent stores) is tracked as **#1666** (platform-lifecycle follow-up; the `offline_endpoint_store` half is **#1545**); until it lands, `last_seen` is the canonical "is this device still active" filter for asset-management counts.
...
| Software install/uninstall | `$Software_Live` / `$Software_Daily` / `$Software_Monthly` | **Asset-management + limited PII.** Install/remove/upgrade *events* — app name, version, publisher. The **machine scope** (`scope=machine`, HKLM) carries no personal identity (asset/vuln-inventory data). The **per-user scope** (`scope=user`, Windows) carries the OS **profile name** in the `user` column alongside that user's installed apps → personal data, works-council-relevant, and is therefore **off by default** (`software_user_scope_enabled`). The `_Daily`/`_Monthly` rollups aggregate per `(name, scope)` and drop the username (no PII in the rollup tiers). Names/versions/publisher only — no command lines, no usage/launch data. | **Machine: on** (`software_enabled=true`); **per-user: off** (`software_user_scope_enabled=false`) | 5 000-row raw cap (row-capped; `$Software_Live` carries the per-user `user` column when per-user scope is enabled) / 31 d daily / 12 mo monthly | `software_enabled` (whole source), `software_user_scope_enabled` (per-user scope, default off), `software_interval`, `software_max_hive_mounts` |
...
- **Software install/uninstall (`software`): machine scope is asset-management class and on by default; the per-user scope carries limited PII and is opt-in.** Recording install/remove/upgrade *events* (app name, version, publisher — no command lines, no launch/usage data) is asset-management and vulnerability-relevance data, the same lower-sensitivity class as the on-by-default Services and User-session sources — distinct from `procperf`'s continuous usage sampling. The **machine scope** carries no personal identity and is therefore on by default. The **per-user scope** (Windows `scope=user`) carries the OS profile name alongside that user's installed apps → personal data, so it is gated behind a dedicated **per-scope toggle `software_user_scope_enabled` (default off)**, satisfying a works-council deployment that wants machine asset inventory without individual-identifying per-user data; `software_enabled` still disables the whole source (existing rows stay queryable). Flipping the per-scope toggle re-baselines the source (no spurious install/remove storm). The `_Daily`/`_Monthly` rollups drop the username (aggregate per `(name, scope)`), so the long-tail tiers carry no per-user PII; `responses.db` is an erasure source for software query results, same as the per-app panel (F3 DSAR path must include it).
