...
|---|---|---|---|---|---|
| Audit trail | `audit.db` | Operator activity (security-relevant) | 365 days (a **floor**, not a ceiling - deletion is paced, never retroactive) | `AuditStore::cleanup_once` on the `run_cleanup` thread - clock-guarded and capped (#2360): the pass declines once when it would expire every datable row, when the persisted clock reading shows an implausible step, or when there is no persisted reading at all so the elapsed-time check cannot run (the first pass after upgrading to a guarded build - expected once per database, and the one trigger that does not spend the decline latch), and deletes at most 25,000 rows per pass, oldest first. `ttl_expires_at` is stamped at INSERT and never rewritten, so changing the setting does not re-date existing rows. Counters: `yuzu_server_audit_clock_anomaly_skips_total`, `..._cleanup_failed_total`, `..._retention_cap_reached_total`, `..._rows_deleted_total`, `..._retention_index_ok`, `..._retention_persist_failed_total`, `..._retention_passes_total` (liveness - alerted on the ABSENCE of increase, since every other counter here moves only when something happened). | `audit_retention_days` |
