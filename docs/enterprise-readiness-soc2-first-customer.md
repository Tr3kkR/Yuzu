...
|---|---|---|---|---|---|
| Audit trail | `audit.db` | Security-relevant activity: operator actions, plus agent enrolment, fleet-topology events and background schedule execution | 365 days (a **floor**, not a ceiling - deletion is paced, never retroactive) | `AuditStore::cleanup_once` on the `run_cleanup` thread - clock-guarded and capped (#2360): the pass declines and warns when it would expire every datable row, when the persisted clock reading is unusable, or when the gap since the previous pass exceeds a fixed 7 days (a clock jump OR an outage that long - elapsed time cannot separate them), and deletes at most 25,000 rows per pass, oldest first. The decision rule is `classify()` plus the fact construction in `AuditStore::cleanup_once`, pinned by an exhaustive truth table and store-level tests; operator triage is `docs/user-manual/audit-log.md#the-retention-clock-guard`. **Closed #2579: an ABSENT stored reading is now a decline trigger when rows are already expired** - bringing it in line with the TAR guard below, which gates the same way (both require rows to be already expired; neither fires on a fresh install). The surviving differences are scope and accounting: TAR decides per warehouse TABLE, this per database, and this counts to its own series rather than the shared decline counter. Such a pass declines once, warns and anchors the reading, then proceeds - counted by `..._retention_bootstrap_declines_total`, kept separate from the clock-anomaly series because it asserts only that nothing can yet be ruled out. Previously it deleted unremarked, so a server upgraded to schema v3 while its clock was already skewed forward could lose expired rows with no decline and no counter; there is no reliable retrospective test for whether a given database was affected, and the loss is not recoverable without a backup predating it. This is the one row in this table where "clock-guarded" carries a documented exception, and it applies to the SOC 2 control record itself. `ttl_expires_at` is stamped at INSERT and never rewritten, so changing the setting does not re-date existing rows. Series (seven counters plus one gauge): `yuzu_server_audit_clock_anomaly_skips_total`, `..._retention_bootstrap_declines_total` (#2579, counted apart because it makes no claim about the clock), `..._cleanup_failed_total`, `..._retention_cap_reached_total`, `..._rows_deleted_total`, `..._retention_persist_failed_total`, plus the two LIVENESS signals `..._retention_passes_total` and `..._retention_last_pass_unixtime` - every other counter here is silence-means-healthy, so these are what distinguish a quiet healthy store from a reaper that stopped running. **Closed #2553: the alert consuming those liveness signals was itself blind to the leading cause of a stopped reaper.** `YuzuAuditRetentionNotRunning` excused any server whose uptime sat under a 3-hour grace, and a crash-looping process never accumulates uptime past it, so the sole detective control for this condition was silent for the case it most needed to catch; a second defect let an unrelated young series merely sharing an `instance` label suppress it. The failure direction is OVER-retention - rows kept past the stated 365-day floor while the reaper is down - so the exposure is data-minimisation as much as availability. Both are NARROWED AND PINNED BY PROMTOOL CASES, NOT CLOSED, and the residual gaps are stated here rather than left to be discovered: (a) a genuinely dead reaper is still undetected at restart cadences of 164-195 minutes (163-196 at a 5-minute evaluation interval) - measured by sweeping the cadence space at 1-minute resolution and recorded in `docs/prometheus/blind-band-measurement.md`, where the same sweep shows the previous expression was silent at EVERY sampled cadence from 30 to 190 minutes, so this is a strict and large narrowing; closing it requires a server-side change (the natural freshness signal is not reloaded from its persisted anchor at startup, so a rule built on it regresses the crash-loop case this one already catches) rather than a further rule edit; (b) while the condition persists the alert can resolve and re-fire, so an Alertmanager history showing fired-then-resolved does NOT by itself evidence recovery - read it with `..._retention_passes_total` to confirm a pass actually landed; (c) a server that is scraped and alive but does not export the liveness counter at all - an older build, since this rules file is applied by the operator and routinely runs ahead of the servers it points at - is covered by neither rule, and an `up`-based target-down alert does NOT cover it either, because that server's `up` is 1. A companion `YuzuAuditRetentionMetricMissing` covers the fleet-wide case where the liveness metric is not scraped at all (an absent series leaves the liveness rule unfirable by construction), but it is fleet-wide by construction and does not detect one quiet server among many. Note the alert rules ship as a file operators apply themselves, so a deployment that does not re-apply `docs/prometheus/yuzu-alerts.yml` keeps the blind rule. **Scope of "the alert rules are tested", stated plainly because it is the honest answer to that questionnaire item:** every shipped rule is PARSE-gated on each pull request (`promtool check rules`), and the retention-liveness pair above is additionally BEHAVIOUR-gated (`promtool test rules`, with each threshold, window, `for` duration and label join red against a mutation of itself). The remaining rules carry no behaviour cases, and nothing yet requires a rule change to ship one - so a green gate on those proves they parse, not that they can fire. | `audit_retention_days` |
