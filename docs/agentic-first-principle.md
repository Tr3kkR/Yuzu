...
| 2026-08-11 | `execute_instruction`'s streamed-final FALLBACK path (`McpStreamBridge::build_fallback_final()`, `mcp_stream_bridge.cpp`) | Typed output schema / `structuredContent` - the fallback final is `{execution_id, status:"unknown", detail}` with NO `content` array and NO `structuredContent` at all, a structurally different (and currently unschematized) shape from the plain/streamed-real-final response `execute_instruction`'s own `outputSchema` now describes. Deliberately NOT folded into that `oneOf` in #2712 batch 3 - real conformance means restructuring `build_fallback_final()` to emit a proper `CallToolResult` envelope, which touches `mcp_stream_bridge.cpp`, a surface owned by the still-open 3b landing (#2739/#2740), not this schema sweep. Reachable today: NOT merely a dead kRingOnly-only path (that comment describes a different, narrower trigger) - also reachable via a fault path (`build_real_final` throwing, e.g. OOM-class) on a LIVE streamed-POST client, no kill switch on that path at present. Converting this to a JSON-RPC error instead (rather than typing it) was considered and rejected: the fallback fires for a dispatched, still-running mutating fleet command, and an error response risks an agentic client re-dispatching it, i.e. double-executing the mutation - the degraded-success shape with its recovery-by-`execution_id` contract is the architecturally correct answer, it just isn't typed yet. | #2739 | when 3b lands |
