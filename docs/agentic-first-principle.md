...
| 2026-08-11 (batch 1), **narrowed same day by batch 2: DEX+network family closed, `execute_*`/writes remains** | 9 MCP tools: `execute_*`/writes family (`execute_instruction`, `execute_bundle`, `get_bundle_result`, `revoke_certificate`, `quarantine_device`, `delete_tag`, `set_tag`, `approve_request`, `reject_request`) | Typed output schema / `structuredContent` - formalized as the `kOutputSchemaExempt` closed allowlist in `test_mcp_server.cpp`'s new #2972 completeness gate (any tool NOT in this list now fails CI without one). #2712's first batch ("Phase-1 reads," 25 tools) closed the gap outside the DEX+network and writes families; #2712's second batch closed the 12-tool DEX+network family, leaving only this row. `execute_instruction` here is the SAME waiver as the row above, not a new one - listed for completeness since it's also in the code-level exemption set. | #2712 | 2026-10-31 |
