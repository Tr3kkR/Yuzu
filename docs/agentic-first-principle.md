...
|---|---|---|---|---|
| 2026-07-24, **CLOSED by #2712 batch 3 (2026-08-11)** | `execute_instruction` (MCP tool) | Typed output schema / `structuredContent` - input bounds backfilled in 2f PR 3a (enforcement is SPLIT since #2405: ENFORCED pre-mint on the approval-gated/supervised path by the C8 input-schema gate, schema-advisory on the operator/readonly path; full every-path enforcement is tracked in #2437). **CLOSED**: the plain JSON-RPC response (both the normal-dispatch and zero-agents-reached outcomes) is now a fully typed, mutually-exclusive `oneOf` - the stable output identifiers (`command_id`/`execution_id`/`agents_reached`) are no longer prose, they are the served schema. A narrower residual gap in the streamed-final fallback path is tracked separately below. | #2436 | closed |
| 2026-08-11 (batch 1), narrowed same day by batch 2 (DEX+network closed), **CLOSED by #2712 batch 3 (2026-08-11): `execute_*`/writes family typed** | 9 MCP tools: `execute_*`/writes family (`execute_instruction`, `execute_bundle`, `get_bundle_result`, `revoke_certificate`, `quarantine_device`, `delete_tag`, `set_tag`, `approve_request`, `reject_request`) | Typed output schema / `structuredContent` - formalized as the `kOutputSchemaExempt` closed allowlist in `test_mcp_server.cpp`'s new #2972 completeness gate (any tool NOT in this list now fails CI without one). #2712's three batches closed this gap for all 46 tools it originally audited. | #2712 | closed |
| 2026-08-11 | `execute_instruction`'s streamed-final FALLBACK path (`McpStreamBridge::build_fallback_final()`, `mcp_stream_bridge.cpp`) | Typed output schema / `structuredContent` - the fallback final is `{execution_id, status:"unknown", detail}` with NO `content` array and NO `structuredContent` at all, a structurally different (and currently unschematized) shape from the plain/streamed-real-final response `execute_instruction`'s own `outputSchema` now describes. Deliberately NOT folded into that `oneOf` in #2712 batch 3 - real conformance means restructuring `build_fallback_final()` (and deciding what happens to `build_real_final()`'s own additive `status`/`agents_success`/`agents_failure` keys, which sit outside `structuredContent` too) into a proper `CallToolResult` envelope, which touches `mcp_stream_bridge.cpp` - out of scope for a schema-literal sweep. **Correction:** an earlier draft of this row cited #2739/#2740 as the blocker; both are already fixed per `docs/mcp-server.md`'s own status line and are unrelated to this typing gap - filed #2990 instead, scoped to the actual work. Reachable today: NOT merely a dead kRingOnly-only path (the in-code comment near `mcp_stream_bridge.cpp:1816` claiming that is itself stale, per #2990) - also reachable via a fault path (`build_real_final` throwing, e.g. OOM-class) on a LIVE streamed-POST client, no kill switch on that path at present. Converting this to a JSON-RPC error instead (rather than typing it) was considered and rejected: the fallback fires for a dispatched, still-running mutating fleet command, and an error response risks an agentic client re-dispatching it, i.e. double-executing the mutation - the degraded-success shape with its recovery-by-`execution_id` contract is the architecturally correct answer, it just isn't typed yet. | #2990 | 2026-10-31 |
| 2026-08-11 | 8 MCP tools: A2 discovery family (`discover_permissions`, `discover_instructions`, `discover_routes`, `discover_scope_kinds`, `discover_plugins`) + agentic-demo/incident-response family (`classify_operational_question`, `get_incident_playbook`, `summarize_working_set`) | Typed output schema / `structuredContent` - these ship the generic `kObjectOutputSchema` placeholder, which satisfies the #2972 completeness gate's *presence* check (it does not check *typed-ness*) without being typed. Not in `kOutputSchemaExempt` - they pass the gate via the placeholder loophole, not exemption. Added to `kTools[]` after #2712's original 2026-08-01 audit, so its body does not name them; tracked separately rather than folded into #2712 to avoid retroactively rewriting that audit's scope. | #2986 | 2026-10-31 |
| 2026-07-24, **closed under `--mcp-enable-streamed-post` 2026-07-27; PERSISTS in the shipped default** | `execute_instruction` progress (MCP GET channel) | Progress-lifecycle ordering deviation: for the 2f 3a GET-only rung, `notifications/progress` for a `progressToken` were delivered on the session's GET channel *after* the `tools/call` JSON-RPC response had retired the request (a strict reading of the MCP progress lifecycle expects progress before/during an in-flight request). Authorized as an interim deviation via the PR #2424 adversarial review. **CLOSED by the 2f 3b streamed-POST rung**: a call carrying a `progressToken` and an SSE-capable `Accept` now receives progress on the POST response itself, before the result (`docs/mcp-server.md` "Streamed POST — SSE on the response"). The GET-channel shape remains supported, but it is no longer a deviation — it is one of two modes the client selects by what it sends, with the same documented poll-by-`execution_id` fallback. | #2439 | closed |
