...
| 2026-08-11 | 21 MCP tools: DEX+network family (`list_dex_signals`, `get_dex_signal_scope`, `get_dex_perf_cohorts`, `get_dex_perf_fleet`, `get_dex_perf_cohort_diff`, `list_dex_perf_apps`, `list_dex_perf_devices`, `get_dex_app_perf`, `get_dex_group_app_perf`, `compare_app_perf_versions`, `get_network_fleet`, `list_network_devices`) + `execute_*`/writes family (`execute_instruction`, `execute_bundle`, `get_bundle_result`, `revoke_certificate`, `quarantine_device`, `delete_tag`, `set_tag`, `approve_request`, `reject_request`) | Typed output schema / `structuredContent` - formalized as the `kOutputSchemaExempt` closed allowlist in `test_mcp_server.cpp`'s new #2972 completeness gate (any tool NOT in this list now fails CI without one). #2712's first batch ("Phase-1 reads," 25 tools) closed this gap for everything outside these two families; `execute_instruction` here is the SAME waiver as the row above, not a new one - listed for completeness since it's also in the code-level exemption set. | #2712 | 2026-10-31 |
