...
| **Spark** | Compliance/health entries route through `GuardianOutbox`, a FIFO, per-(domain,rule_id)-coalesced, **capacity-bounded** buffer. A push that would exceed capacity is dropped and **counted** (`outbox_backpressure_drops()`), never silent — routed to the agent heartbeat as `yuzu.guardian_outbox_backpressure_drops` and rolled up fleet-wide as `yuzu_fleet_guardian_outbox_backpressure_drops` (#2993, closed); verify current wiring before relying on fleet-level visibility into this specific counter, same caveat D10 already carries for its own counters. |
