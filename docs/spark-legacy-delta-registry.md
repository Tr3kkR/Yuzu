...
| **Ruling** | **D3 RULED (Dave, 2026-08-23), interim fix — option 2 of 3 offered** (accepted-delta / raise-the-default / full M1-shaped edge+refresh redesign): raise the default, not the full redesign, which stays open for later reconsideration. Fixed in `fix/2298-3388-drift-debounce-default` (commit `b73281dab`): `parse_resilience_params` gained a defaulted `default_event_debounce_ms` parameter (all 3 legacy call sites unchanged, still get the implicit 1000 ms) and spark's call site (`guardian_spark_bridge.hpp`) now computes its own default per rule type — lane cadence + the scheduler's own `jitter_pct` margin (`kGuardianServiceLaneCadenceMs`/`kGuardianRegistryLaneCadenceMs`/`kGuardianFileLaneCadenceMs`/`kGuardianLaneJitterPct`, hoisted into `spark.hpp` so the scheduler and this default single-source the same constants) — 72 s on the 60 s lanes, 720 s on the file lane, both engineered to exceed even the shortest jitter-shortened sweep (`period × 0.8`) so debounce reliably survives at least one full sweep. This roughly halves steady-state re-emission versus the pre-fix figures in the Spark cell above; an exact post-fix jitter-floor figure (matching D1/F11's measured-fake-clock derivation) has not been computed and would need one before being cited as precisely as this row's pre-fix numbers are. |
...
| **Operator symptom** | A rule left persistently drifted under spark (e.g. an enforce-mode rule now observe-only per B1, with nobody fixing the underlying drift) generated a steady ~1800/day (60 s lanes) or ~180/day (600 s lane) `drift.detected` stream pre-fix — jitter-adjusted worst case, per the Legacy/Spark cell above — smaller than the pre-fix health flood (~17k/day) but structurally the same class of gap M1 was written to close, left open on the sibling stream. Post-fix, that steady-state rate is roughly halved by design (see Ruling), not eliminated — the full M1-shaped redesign remains the option for whoever revisits this. |
| **Verify at** | `guardian_emit_decider.hpp::decide_emit` (no `last_compliant`-driven terminal dedup on the Drift branch — unchanged by this fix); `guardian_spark_bridge.hpp`'s `rule_assertion_from_rule` (the new per-type default computation); `resilience_strategy.hpp`/`.cpp`'s `parse_resilience_params` (`default_event_debounce_ms` parameter); `spark.hpp` (`kGuardian*LaneCadenceMs`/`kGuardianLaneJitterPct`); `guard_systemd.cpp` `systemd_decide_emit` (contrast: has terminal-state dedup, unrelated to this fix). |
| **Epistemic** | **verified b73281dab** — the fix commit itself; full agent suite green (113009 assertions, 2034 cases), 2 mutation checks confirmed the new test coverage is load-bearing. |
...
| **Operator symptom** | Opposite of the row's own earlier framing: on a routine restart with N armed rules and no pending policy-generation lag, **spark reliably produces N new `guard.compliant` server rows** (a real restart-storm, sizing input for F14's 3-OS matrix / resource baseline) while **legacy generally produces zero** — its boot-time compliant edges are silently lost, and Linux Service never had one to lose. This is a genuine parity-gate-relevant delta, not the "shared, not itself a delta" framing this row previously (incorrectly) settled on. |
