...
**ruled** — the decision is settled, but its implementing fix is drafted on an unmerged
branch, not yet part of this tree; see D3's own Ruling and Epistemic cells for the exact
distinction between "the decision is verified" and "the fix is not." No row currently
carries `open-question` status, but D3 is not a clean example of a fully-resolved row
either — treat it as its own third-and-a-half case: ruled, not yet shippable, not yet
re-stampable to a plain `verified`.)
...
illustrates this combined form; D3 carried it until its 2026-08-23 ruling and now
illustrates the separate ruled-but-unmerged case instead, see its Ruling/Epistemic
cells).
