...
| **Verify at** | `spark_file.cpp`/`spark_registry.cpp`/`spark_service.cpp` factory `#else` branches; `guard_file.cpp:450`/`guard_registry.cpp:548`/`guard_service.cpp:530`/`guard_systemd.cpp` stub returns; `docs/capability-registries/spark_mechanisms.tsv` (independently agrees). |
| **Epistemic** | verified 7b449e589 |
...
| **Operator symptom** | Measured steady-state ceiling, F11 (#3267): **1 edge + 288 refreshes/day/rule/agent** on the 60 s lane, **1 edge + 180/day** on the 600 s file lane (±20% scheduler jitter — 144/day is the exact no-jitter figure, 180/day is the true production ceiling since jitter never shortens below the refresh floor). **Epistemic note on these two numbers:** per F11's own PR scope note, they are "a derived bound, mechanism empirically pinned" — no test asserts a per-day count directly; the mechanism constants (`errored_refresh_ms`, demotion thresholds, lane cadences) are pinned by Catch2, the per-day figure is arithmetic derived from them. Sparse heartbeat tags: `yuzu.guardian_unhealthy_suppressed`/`_refreshed`, `yuzu.guardian_priority_demoted` → fleet gauges `yuzu_fleet_guardian_unhealthy_suppressed`/`_refreshed`, `_priority_demoted` (monitor-only, per-sweep fleet sums). |
