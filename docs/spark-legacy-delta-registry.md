...
ruled and fixed — see its row for the still-live example of what an open-question row
looks like once resolved. No row currently carries this status.)
...
### D3 — Drift re-emission has no per-sweep debounce guard today; a sweep-cadence-aware fix is ruled but not yet merged
...
| **Ruling** | **D3 RULED (Dave, 2026-08-23), interim fix — option 2 of 3 offered** (accepted-delta / raise-the-default / full M1-shaped edge+refresh redesign): raise the default, not the full redesign, which stays open for later reconsideration. **The decision is settled; the code implementing it is not yet part of this tree.** A fix exists on the local branch `fix/2298-3388-drift-debounce-default` (own governance cycle run, currently at `e57ae7319`) — but that branch has never been pushed to `origin` or opened as a PR, so nothing in it is reachable from this branch, from `origin/dev`, or from any merged ref. Verified directly: `git merge-base --is-ancestor <any commit on that branch> origin/dev` fails, `git ls-remote origin refs/heads/fix/2298-3388-drift-debounce-default` returns nothing, and issue #3388 (this row's own tracker) is still **open** with no comment recording this ruling or fix. The intended shape of the fix, for whoever picks this up: `parse_resilience_params` gains a defaulted `default_event_debounce_ms` parameter (legacy call sites unaffected, still get the implicit 1000 ms), and spark's call site computes its own default per rule type from lane cadence + the scheduler's `jitter_pct` margin — engineered to exceed even the shortest jitter-shortened sweep so debounce reliably survives at least one full sweep. Until that branch is pushed, reviewed, and merged, **the code in this tree still has the flat 1000 ms default described in the Spark cell above** — nothing here is fixed yet. |
...
| **Operator symptom** | A rule left persistently drifted under spark (e.g. an enforce-mode rule now observe-only per B1, with nobody fixing the underlying drift) generates a steady ~1800/day (60 s lanes) or ~180/day (600 s lane) `drift.detected` stream **today, in this tree** — jitter-adjusted worst case, per the Legacy/Spark cell above — smaller than the pre-fix health flood (~17k/day) but structurally the same class of gap M1 was written to close, left open on the sibling stream. Once the branch above merges, that steady-state rate is designed to roughly halve, not eliminate — the full M1-shaped redesign remains the option for whoever revisits this. Until then, treat the pre-fix figures as the live number, not the halved one. |
| **Verify at** | `guardian_emit_decider.hpp::decide_emit` (no `last_compliant`-driven terminal dedup on the Drift branch — unaffected by the drafted fix); `resilience_strategy.hpp`/`.cpp`'s `parse_resilience_params` (current flat default, no per-type parameter yet — confirms the fix hasn't landed here); `guard_systemd.cpp` `systemd_decide_emit` (contrast: has terminal-state dedup, unrelated). The drafted fix's own symbols (`kGuardian*LaneCadenceMs`, `default_event_debounce_ms`) do not exist anywhere in this tree — do not cite them as present until the branch above merges. |
| **Epistemic** | **verified b16e5836d** for everything in the Legacy/Spark/Operator-symptom cells (the current, live behavior). The Ruling cell's *decision* is verified (Dave's own words, this date); the Ruling cell's *fix* is **not verified against any merged code** — re-stamp this row once `fix/2298-3388-drift-debounce-default` actually merges, and re-derive the post-fix jitter-floor figure at that point rather than citing the "roughly halved" estimate as precise. |
...
| **Spark** | Compliance/health entries route through `GuardianOutbox`, a FIFO, per-(domain,rule_id)-coalesced, **capacity-bounded** buffer. A push that would exceed capacity is dropped and **counted** (`outbox_backpressure_drops()`), never silent — but not yet routed to a heartbeat tag or fleet gauge as of this writing (tracked: **#2993**); verify current wiring before relying on fleet-level visibility into this specific counter, same caveat D10 already carries for its own counters. |
...
| **Verify at** | `guardian_engine.cpp::emit_guard_event` (sink-not-wired drop, no counter; the blocking `sink(ev)` send when wired); `agent.cpp::emit_guardian_event` (`guardian_sink_stream_` null mid-reconnect drop, no counter — its own comment notes it's "Shared by the GuardianEngine drift sink and the (ruleless) DEX signal observer," so this same silent gap also affects DEX crash-recorder observations, not just Guardian compliance/drift events); `guardian_outbox.hpp` `GuardianOutbox` capacity check + `outbox_backpressure_drops()`; **#2993** (unwired telemetry). |
| **Epistemic** | verified b16e5836d |
