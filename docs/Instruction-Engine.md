...
- **Phase 1** complete — `ResponseStore` (SQLite, TTL-based cleanup, query with filtering/pagination), `AuditStore` (structured events with principal/action/target/result), `TagStore` (agent-synced and server-set tags, validation), `ScopeEngine` (494 LOC recursive-descent parser, 9 operators, AND/OR/NOT combinators, wildcard LIKE, case-insensitive matching). All in `server/core/src/`.
