...
|---|---|---|---|
| `Forensics` | Read-only forensic artefact reads (ShimCache/AmCache/Prefetch, application usage, per-app privacy-permission grants, browser profile inventory). Every catalogue row on it is `ExecuteGate::AdminOrApproval` AND single-target (below) — Administrator by default; the stored rows of a dispatched read follow `Response:Read`. | `Read`, plus the unused CRUD-loop ops (harmless, keeps the loop uniform) | Administrator (CRUD via the loop); deliberately ABSENT from the Viewer read-list — dispatching a forensic read is never a Viewer-tier operation (the stored results, though, follow `Response:Read`, which Viewer holds) |
