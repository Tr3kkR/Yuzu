...
| `staged` (`std::vector<EvalTimingRecord>`) | Local to `evaluate_key` | `reserve(planned * 2)` before the reads (outside `registry_mu_`), populated in the `registry_mu_` commit section | Function exit, or moved into `last_eval_timings_` after the log lines are written | Owns its strings; no borrowed views. Every early return before staging leaves it empty. It is not read after the move. Allocation is the failure path: `reserve()` and the per-entry `event_id` copy run before the real `enqueue_all`, so each is inside a `try`/`catch(...)` that drops the batch's timing (the `staged` tail is erased, which does not allocate) and lets the enqueue and the drain-worker wake proceed; the timing lines are lost, never the event. Pinned by a fault-injection test. |
