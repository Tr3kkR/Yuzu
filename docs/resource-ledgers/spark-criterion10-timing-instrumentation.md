...
| `staged` (`std::vector<EvalTimingRecord>`) | Local to `evaluate_key` | `reserve(planned * 2)` before the reads (outside `registry_mu_`), populated in the `registry_mu_` commit section | Function exit, or moved into `last_eval_timings_` after the log lines are written | Owns its strings; no borrowed views. Every early return before staging leaves it empty. It is not read after the move. |
