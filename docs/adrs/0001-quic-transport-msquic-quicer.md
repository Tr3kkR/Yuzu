...
| PR 1c-4 | Agent-side Subscribe (bidi) + DownloadUpdate (server-stream-via-bidi) lift; UP-14 hard predecessor closed by #904 (bounded dispatcher pool, 2026-05-09); UP-8 hard predecessor closed by #902 (BidiStream::read idle-read deadline, 2026-05-09); `parse_target_address` agent-side test coverage closed by `1af8ab5`; remaining hard predecessor: agent-side Heartbeat per-cycle `stop_source` round-trip test | Pending |
