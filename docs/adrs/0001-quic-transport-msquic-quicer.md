...
| **Per-call dispatcher-thread fanout at enterprise scale (governance UP-14)** — PR 1b-3's bidi server dispatcher spawns one `std::thread` per active bidi RPC. At 100K Subscribe streams (one per agent), Linux default 8 MiB stack means ~800 GiB virtual; even a 256 KiB tuned stack yields ~25 GiB and a fragile thread-creation ceiling. Plus head-of-line block on the cq_worker when joining slow-exiting handlers. | **Hard gate before PR 1c-4 merges** — the Subscribe lift is the first production caller that exercises this fanout. Mitigation options: (a) replace the per-call `std::thread` with a bounded thread pool whose worker count caps active concurrency at ~`max_concurrent_streams_per_connection`; (b) keep per-call threads but tune the stack via `pthread_attr_setstacksize` to 256 KiB before spawn; (c) detach the `handler_thread_.join()` into a dedicated reaper thread to remove the head-of-line block on cq_worker. Decision deferred to PR 1c-4 prep; whichever option is chosen MUST land before Subscribe is lifted. Gateway-mode deployment partially mitigates because the gateway terminates Subscribe and only the per-gateway thread count grows on the server. |
