...
|---|---|---|---|
| `GuaranteedStateStore` | `guaranteed_state_store` | authoritative | Guardian state; high-write; join input. **In flight — ADR-0038** (Wave 1.1): rules/meta reads type-distinguishable (a silent-empty `list_rules` into the Push fan-out is a fleet-wide disarm — the catastrophic read); event/observation ingest fail-soft; mandatory 5-table backfill (ADR-0037 SQLSTATE discrimination); the cleanup thread's bare TTL deletes port to the #2496 clock-guard shape (single-sweeper advisory lock, lockstep events+observations reap, WS-E reaped counters kept). |
