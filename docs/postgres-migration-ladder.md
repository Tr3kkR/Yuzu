...
| `ResponseStore` | `response_store` | authoritative-reads / fail-soft-ingest | **In flight — ADR-0039** (Wave 1.2, promoted after FleetTopologyStore was found N/A). high-write, TTL'd 90d; backfill **SKIPPABLE** (ADR-0009 — expendable telemetry, fresh-start on cutover); reads degrade-distinguishable at the seam (deny-or-benign class, not catastrophic); cleanup thread → #2496 clock-guard reap; `sqlite3_changes()`→RETURNING (#1033). |
| `AuditStore` | `audit_store` | authoritative | SOC 2 retained 365d; backfill **mandatory**. Carries SQLite schema v3 (`audit_retention_meta`, one k/v row) plus the partial index `idx_audit_ttl_id`; both must come across. The retention clock guard (#2360) is migration-REQUIRED behaviour, not an optimisation - dropping it on the way over reinstates an unbounded clock-driven `DELETE` on the evidence chain. **Single-writer assumption, and this is the part that does not port:** the dedup state (`last_reported_`, the previous pass's `Facts`) is a per-PROCESS member while the clock reading it pairs with is durable, so N servers each spend the guard independently; on PG it must become a per-store advisory lease (the ADR-0012 §3 `AppPerfRollup` pattern). Likewise `kMaxAuditDeletesPerPass` is calibrated as a drain rate for ONE hourly loop - N processes x 25k is a different number. |
