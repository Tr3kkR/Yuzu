...
| **WS-1** | Server-plane state → Postgres. **Milestones:** (1a) sessions (DB-time); (1b) `execution_tracker` + command-correlation w/ atomic counters; (1c) HA-critical store subset | **1a PARTIAL** (durable `SessionStore`, wall-clock, write-through + generation validate-cache — `session_store.{hpp,cpp}`, `auth.cpp`). **DB-time NOT yet done**: timestamps are replica-local `system_clock`, not Postgres `now()` — single-replica-correct, but the cross-host-clock authority (the "(DB-time)" the milestone is named for) is a required WS-1a-completion follow-up before a 2nd replica. `execution_tracker` still SQLite (`execution_tracker.hpp:117`); `cmd_execution_ids_` in-proc (`agent_service_impl.hpp:333`) | migration ladder (per store; **serializes at the migration-version counter**, not fully parallel) | **Y** | `authdb`+`security-guardian` (1a); `architect`+`sre`+`cpp-safety` (1b/1c) | P0 | **1a partial: durable+wall-clock shipped (feat/ha-ws1-sessions-pg, in review); DB-clock-authority + 1b/1c outstanding** |
...
| **WS-7** | **HA-PG delivery**: shipped Patroni+etcd+HAProxy Compose profile, **selectable durability (3-node quorum default, distinct failure domains)**, operator-plane LB profile | Single `postgres:18`; single-server compose | — (storage axis; parallel) | **N** (gates storage-HA / RPO=0 claim, not the 2nd server replica) | `release-deploy`+`build-ci`+`sre` | P1 | planned |
