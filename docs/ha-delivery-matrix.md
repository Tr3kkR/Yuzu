...
| **WS-4** | Gateway routing + multi-cluster: fenced `agent→cluster` directory, **net-new distributed intra-cluster agent→node routing**, `gateway_node` convergence | `pg` broadcast-only; per-agent lookup node-local ETS (`yuzu_gw_registry.erl:90`); `gateway_node`+caps now via `set_gateway_route` (single call; NotifyStreamStatus `:637`/`:660`). **4.1 DONE**: born-on-PG fenced `GatewayRouteStore` (`gateway_route_store.{hpp,cpp}`, schema `gateway_route_store`, migration v1 — see `docs/postgres-migration-ladder.md`) — the agent→cluster routing directory (`agent_routes(agent_id PK, cluster_id, gateway_node, connection_epoch, session_id, lease_until, updated_at)` + `connection_epoch_seq`). A SERVER-minted monotonic `connection_epoch` (guarded upsert `WHERE EXCLUDED.connection_epoch > stored`) orders a fresh `ProxyRegister` against an existing row — server-internal ordering only, never on the wire; `session_id` equality guards every post-register write (`announce_connected`/`deregister`/`renew_leases`), so a delayed replay from a superseded session cannot overwrite or tear down a newer re-home. Wire gained only `StreamStatusNotification.cluster_id = 7`; the gateway now carries the agent's existing `x-yuzu-session-id` on a circuit-recovery ProxyRegister REPLAY so the server re-announces (reuses the session) instead of minting a new one. **INERT**: written on connect/disconnect/heartbeat, nothing reads it for dispatch yet (that's 4.2). Posture: construction fail-CLOSED (ADR-0012), runtime write failure fail-**OPEN** today (logged + `yuzu_server_gateway_route_write_failed_total{op,reason}`) — **4.2 MUST flip this to fail-closed/`UNAVAILABLE` once the directory becomes dispatch-authoritative**. **4.2a DONE** (writer-path hardening, `#4246` items #2/#5/#7/#8-counters/#9 — **#4 RE-SCOPED, not closed** (see below)): `deregister` tombstones instead of deleting (closes the late-CONNECTED **resurrection** direction — #5 — so a late CONNECTED for a now-gone session no-ops against the tombstone via `announce_connected`'s fallback INSERT); a clock-guarded background reaper `GatewayRouteStore::reap_stale_routes()` (`ReplicaSafe`, `background_jobs.hpp`, ~5-minute cadence) sweeps expired-past-grace leases and purge-aged tombstones; `ProxyRegister`'s mechanism-(c) branch (a presented-but-locally-unknown session) now renews the presented session in the directory instead of calling `register_fresh`, closing the fresh-branch epoch-clobber gap; a new `yuzu_server_gateway_route_desync_total{op,outcome}` counter gives guard-rejection visibility distinct from the write-failure counter; hot-path writers dropped to a 500ms timeout (reaper keeps 2000ms). Directory is **still INERT** and 4.2 itself (dispatch-authoritative + the fail-closed flip) is **NOT done**. Remaining WS-4 sub-work: **4.2b** (the fail-closed flip + its alert rule, `#4246` #1 and #8's alert-rule half), a durable cross-replica session lookup (`#4246` #3, re-homed to its own slice under WS-5 shared presence), 4.3 (net-new distributed intra-cluster agent→node routing), 4.4 (`gateway_node` convergence reconcile + gateway-side replay-session writeback `#4246` #6 + `agent_id`-on-renew correlation `#4246` #10); and **the per-home stream-generation fence (#4324, re-scoped from `#4246` #4)** — 4.2a's tombstone closed only the late-CONNECTED resurrection (#5), NOT a same-session late-DISCONNECTED tombstoning a newer re-home; that direction has no producer under the shipped gateway (one CONNECTED(S)/one DISCONNECTED(S) per session) so it is unreachable today, but the fence (a per-home token on `NotifyStreamStatus`, spanning the legacy in-memory teardown too) is a **precondition of the first slice that re-CONNECTs under a reused session id (4.3/4.4/#6), and 4.2b's review MUST re-verify the once-per-session property before the directory becomes dispatch-authoritative**. | **WS-1, WS-3, WS-0** (its no-loss/no-dup DoD needs the outbox + receiver dedup) | **Y** | `gateway-erlang`+`security-guardian`+`architect`+**`cpp-safety`** | P1 | **in progress (4.1 + 4.2a done; 4.2b/4.3/4.4 remain)** |
