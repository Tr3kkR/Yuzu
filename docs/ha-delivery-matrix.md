...
| **WS-2** | (**2a**) durable **event outbox** + NOTIFY fan-out [monolith-OK]; (**2b**) **core→presentation event spine** [*defers to ADR-1005 split*]; MCP session/replay durability | **2a-1 DONE**: durable append-only `event_outbox` table (ExecutionTracker migration v3, global monotonic `event_id`), the ExecutionEventBus events (`agent-transition`/`execution-progress`/`execution-completed`) appended ATOMICALLY inside each state-write txn (`upsert_agent_status_once`/`refresh_counts_once`/`mark_cancelled`), clock-guarded `reap_event_outbox` (24h). No consumer yet — bus still serves live SSE. **2a-2 outstanding**: LISTEN/NOTIFY + reconnect cursor-poll delivery (the forward-poll cursor must NOT use a bare `id>cursor` or a `created_at` time-lookback — see the id-ordering contract at `append_event_outbox` for the safe id-gap-pending / txid-snapshot-horizon forms). 2b/MCP session-replay durability outstanding. | 2a: WS-1(1b) ✓. 2b: **ADR-1005 split** | **Y** (2a) | `architect`+`sre`+`security-guardian`(MCP)+`docs-writer` | P1 | **2a-1 done; 2a-2 / 2b outstanding** |
