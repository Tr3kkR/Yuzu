...
| **WS-3** | Coordination seam: **fenced `LeaderElector`** (monotonic epoch checked in claim txn) + leader/**transactional-outbox**/receiver-idempotency worker refactor incl. policy remediation | **3.1 DONE (#4011)**: fenced `LeaderElector` primitive — dedicated never-recycled coordination connection + session advisory lock + monotonic epoch minted into `leader_state`; the fence is an embeddable `epoch_fence_sql()` predicate spliced into the claim's WRITE (NOT a check-then-act boolean read). INERT — no loop wired yet, so no runtime behaviour change. Workers still dispatch per-proc; Deployment CAS + sweep advisory locks already correct. Follow-ups: #4013 (3.2 connection robustness), #4014 (WS-11 leader metrics + /readyz), #4015 (3.3 fence claim-protocol), #4016 (routed-concern row). **3.2** gates the singleton loops leader-only (Phase B — confirm the ADR-1005 split question first); **3.3/3.4** add the transactional command outbox + claim sites (built on ADR-1007 `ConfinedDispatchOutcome`/`not_sent`) + the PolicyEvaluator §6-finding-6a redesign. | **WS-0, WS-1, WS-2(2a outbox)** | **Y** | `architect`+`cpp-safety`+`security-guardian` | P1 | **3.1 done (#4011); 3.2/3.3/3.4 planned** |
...
| **WS-10** | **Background-job replica-safety classification** — checked-in, CI-auditable table (job → fenced-leader-only / independently-replica-safe / disabled-until-fixed); bring #2508 wall-clock passes to clock-guard | **10.1**: `background_jobs.hpp` `kBackgroundJobs` classifies all 32 passes, catastrophic-class sites `consteval`-gated; **10.2**: `app_perf_fleet_store`/`preflight_run_store`/`deployment_run_store` prunes now run through the shared `pg::run_clock_guarded_prune` (advisory-locked, PG-clock); the concurrency reconciler independently gained the same advisory-locked/PG-clock shape (its OWN `pg_try_advisory_xact_lock`, not the shared helper) | audit + `disable-until-fixed` need nothing; **`fenced-leader-only` ENFORCEMENT (10.3) needs WS-3 3.2** | **Y** | `cpp-safety`+`sre`+`compliance-officer` | P0 | **10.1/10.2 done (this branch); 10.3 enforcement pending WS-3** |
