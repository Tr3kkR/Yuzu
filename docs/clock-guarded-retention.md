...
| `execution_artifacts` scratch-dir sweep (agent, #4390) | Agent-side, not a store: removes orphaned `execution_artifacts-<32 hex>` scratch directories under `agent.data_dir` whose last-write time is older than one hour (`execution_artifacts_scratch_sweep_win.cpp`). **Part 5 adopted** — five unconditional caps (root entries enumerated, directories removed per pass, failures per pass, wall-time deadline computed once, entries per candidate); truncation is reported as `deferred`, never as done. **Parts 1–4 and 6 DELIBERATELY NOT adopted, recorded here and in the header banner:** the content is a regenerable copy of a system hive that this plugin itself staged — never operator data — so a wrong wall clock can destroy nothing that cannot be re-created by the next dispatch; and liveness of an in-flight dispatch is proven by the OS (the dispatch holds the directory open without `FILE_SHARE_DELETE` and the loaded hive holds the file), not by the clock — the one-hour floor exists only to cover a concurrent dispatch's create-to-open window. Part 7 holds (absolute threshold, equality is fresh). Single-process by construction; no persisted anchor. |
| `local_security_policy` scratch-dir sweep (agent, Wave 8 PR8.3) | Agent-side, not a store: removes orphaned `local_security_policy-<32 hex>` scratch directories under `agent.data_dir` whose last-write time is older than one hour (`local_security_policy_win.cpp`'s `sweep_stale_scratch_dirs`, policy in `local_security_policy_scratch_sweep.hpp`). **Same adopt/decline split as the `execution_artifacts` row above, decided for this plugin, not inherited:** **part 5 adopted** — five unconditional caps (root entries, removals per pass, failures per pass, one wall-time deadline computed once for the whole pass, entries per candidate); a truncated enumeration or a spent budget is reported as `deferred`, never as done. **Parts 1-4 and 6 DELIBERATELY NOT adopted:** the only thing this sweep can touch is a `secedit /export` output this plugin itself staged — a regenerable copy of the host's own policy, never operator data and never the only copy of anything, so a wrong wall clock can destroy nothing the next dispatch cannot re-create; and an in-flight dispatch's liveness is proven by the OS (the dispatch holds the directory open WITHOUT `FILE_SHARE_DELETE`, so the sweep's DELETE-requesting open takes a sharing violation), not by the clock. There is no persisted anchor to be missing and the agent is single-process here. **Part 7 holds** (absolute threshold; `is_stale` is strictly-greater, so equality and a future-dated mtime are both Fresh). **On the constants:** all five caps and the one-hour floor are the SAME VALUES as the `execution_artifacts` row above (4096 / 64 / 256 / 2000 / 64, 3600 s). That is deliberate rather than inherited by default -- the substrate is identical (Windows filesystem, agent process, the same `confined_fs` primitives, the same create-to-open window), so the same numbers are the right answer and a different one would need its own justification. One DERIVATION differs: `kScratchSweepMaxDirEntries = 64` is reached here because a real export directory holds exactly one file (`policy.inf`), where the sibling reaches the same 64 for a hive plus its `.LOG1`/`.LOG2` sidecars. |
