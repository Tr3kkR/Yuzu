...
- **MCP server** — AI-native fleet management (commercial peers have no equivalent); the tool/resource surface is near-fully self-describing per ADR-1005 Decision 16 (`docs/agentic-first-principle.md` A5) — typed input/output schemas, truthful `readOnlyHint`/`destructiveHint`/`idempotentHint`/`openWorldHint` annotations, a handshake orientation blob, and machine-readable specs (OpenAPI, scope-DSL) discoverable via `resources/list` rather than requiring an agent to already know which tool to call (one narrow tracked residual: the streamed-final fallback envelope, #2990 — `retry_after_ms` closed 2026-08 by #3344)
