...
skipped the gateway. Every CI leg that runs `meson test` (ci.yml Linux/Windows/macOS, nightly asan/tsan/coverage/windows-asan, sanitizer-tests asan/tsan) now configures with `-Drequire_gateway=true`, so
