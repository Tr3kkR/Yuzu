...
precedent - not resolved in this doc-only PR. **Superseded by the tenth round's own ruling
...
the same way as #4020/#4021 below, not following #3816's precedent after all). `sudo -n systemctl
...
to compliant-edge detection. No per-rule double-counting from the legacy/spark mutual-exclusion
...
arms on Linux (legacy AND spark); `guardian_routes.cpp`'s fold loop (`:421-423`) then adds a
Linux Service rule with a real status row to `notimpl` on top of its real bucket, and
`StateRollup::total()` (`:261`) sums both additively - a live, pre-existing defect (legacy's
Linux `drift.detected` already triggers it in production today, no `prefer_spark` needed), not
gated on #4044 or the flip. Filed separately rather than folded into #4044 because it doesn't
depend on #4044's resolution either way. **#4252 is RULED non-gating**, moved from §1's "flagged
as gating F14" framing (now superseded, see the ninth-round paragraph's correction above) to the
"Non-gating but tracked" list alongside #4020/#4021 in §1's Sign-off row - it is a pre-existing
production defect unrelated to `prefer_spark` (same class as those two), and the flip only
widens its surface (every Linux Service rule gets a real status row post-flip via
`emit_compliant_edge=true`, not just drifted ones today), it does not create the defect. Widening
severity is not the same as flip-gating: nothing about `prefer_spark` needs to be true for this
bug to be real and fixable today, so there is no reason PR-5 specifically must wait on it -
tracked so it doesn't exit the ledger by omission, not because the flip depends on it.
**#4044 itself stays open** (not closed by this PR): the ruling is made, but its own action item
- the `guaranteed-state.md` edit - is scheduled for PR-5 per the Scope note, so closing now would
lose that forward pointer.
