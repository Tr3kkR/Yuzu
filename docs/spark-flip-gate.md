...
`RuleRollup::total()` (`:261`) sums both additively - a live, pre-existing defect (legacy's
...
depend on #4044's resolution either way. **#4044 itself stays open** (not closed by this PR):
the ruling is made, but its own action item - the `guaranteed-state.md` edit - is scheduled for
PR-5 per the Scope note, so closing now would lose that forward pointer.
