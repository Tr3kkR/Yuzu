...
| 5 | Firewall both teardown scope guards, make them noncopyable | **Citations stale; likely fixed as a byproduct of a different PR - needs owner reconciliation before risk-accept OR done-marking** | The issue cites two copyable guards with implicitly-noexcept destructors at `guardian_engine.cpp:66–79` / `guardian_spark_runtime.cpp:31–44`. Neither region contains a guard struct any more - both are now unrelated code (reserved-namespace constants; `io_class_for_spark_type`). A shared type, `GuardianRollback` (`agents/core/src/guardian_scope_guard.hpp`), exists and **exactly** mirrors the ask: noncopyable (copy/assign deleted), destructor never propagates (`try { fn(); } catch (...) { count + best-effort log }`). `git log --diff-filter=A` dates its arrival to commit `25f73e231` (2026-07-18, "harden item 3 exception safety per Sol+Fable review") - one day *after* #2233 was filed (2026-07-17), so it plausibly predates or coincides with item 5's own filing rather than being written to close it; used 5× in `guardian_engine.cpp` and 9× in `guardian_spark_runtime.cpp` today. This is suggestive, not conclusive: this verification pass located the *type* and its wide adoption, but could not conclusively trace the two *specific* guard instances the issue names to their current location or confirm they were the ones migrated to `GuardianRollback` (vs. removed/restructured some other way during PR #3821's larger rework). **Do not mark this item DONE without an owner tracing the two named sites through `git log -S` on their original content**; re-verify before PR-2b. |
