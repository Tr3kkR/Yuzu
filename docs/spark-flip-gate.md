...
this is root-privilege config living on BigColin, a box CLAUDE.md itself documents as shared by
4 CI runner agents under one OS identity - not equivalent to a scratch dir on a single-user
Windows box (§8's own precedent for "left behind" state elsewhere in this file). Owner: not
assigned - needs one before this is forgotten. Detection signal: none (no expiry, no periodic
audit of `/etc/sudoers.d/`). Revisit trigger: before this box's next CI-relevant provisioning
change, or on a fixed schedule if one doesn't come sooner. Not filed as a GitHub issue -
config-cleanup debt, not a code defect - but recorded here as genuinely open, not resolved by
"narrowly scoped."
...
limitation to the legacy guard specifically, or confirm this emission is unintended); not
resolved in this doc-only PR. `sudo -n systemctl
...
dashboard-confirmed for its own specific event.
