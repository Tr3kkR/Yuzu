...
- [x] **5. UAT smoke**: arm on spark → induced drift → dashboard edge; `--spark-disable` rollback
      ...
      attempt left open, then reproduced the full path three times across two arm windows
      (the second and third with no restart between them):
      - **Root cause of the prior blocker, not just a retry** (corrected 2026-09-06: an
        ...
        reset a stale login, no `auth.users` touch needed for THIS recipe specifically -
        the admin row already exists and is active. That said, "not against Postgres" is
      ...
      - **Live repro, three times, in ONE arm window with no restart between the second and
        ...
        log line can, and does. First induced edit (2026-09-06T10:18:19Z, before the
