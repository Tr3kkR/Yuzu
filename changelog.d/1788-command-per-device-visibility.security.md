- **`POST /api/command` now confines every dispatch arm to the caller's authorized devices (#1788).**
  ...
  `POST /api/v1/bundles`, the dashboard execute and TAR `purge_source` routes, and
  `POST /api/instructions/{id}/execute` + `POST /api/workflows/{id}/execute`, and the legacy
  ...
  caller's visible set through one shared per-arm seam, and all failing CLOSED (a present-empty
  visible set, never "unfiltered") when the derivation is unwired. (Still deferred: the BACKGROUND
