- Agent: an out-of-memory condition while arming a Spark watch no longer leaves a dead
  ...
  unchanged. One residual is deliberately left and is now visible: if the operating system
  call that stops watching a target fails during this cleanup, the watch is not reclaimed
  — on Windows it can persist for the life of the agent process, so only restarting the
  agent releases it. The agent reports this in its heartbeat as
  `yuzu.spark_arm_race_unwatch_failures`. Spark detection is not yet the active path in a
  default install, so this affects deployments only once it is enabled.
