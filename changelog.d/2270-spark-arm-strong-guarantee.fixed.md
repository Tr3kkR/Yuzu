- Agent: an out-of-memory condition while arming a Spark watch no longer leaves a dead
  ...
  while monitoring nothing. A watch that fails to arm still fails for every subscriber
  sharing it, which is unchanged. Spark detection is not yet the active path in a default
