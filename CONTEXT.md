...
A node or edge lying on many high-value attack paths, such that removing it — patching/isolating the host, or closing the port/flow — severs the most at-risk value for the least defender effort. **Ranked by value-weighted chain centrality (ADR-4003):** for every (entry point, crown jewel) pair, credit accrues to every node on the least-complex — equivalently, most probable (ADR-4003, citing CAVM Annex I.3) — path between them, scaled by that crown jewel's **Asset value**, and summed across every pair a node sits on. This retired an earlier `crown-jewel value × path probability`-only formulation that excluded centrality outright; the current form keeps value as the scaling factor while giving credit to nodes sitting on paths to *multiple* crown jewels, not just the most valuable one. The minimum-effort set of removals that fully severs a trust zone from crown jewels it should not reach is a **segmentation recommendation** (a cost-weighted min-cut). Because the graph is observed-grounded, a chokepoint severs *observed* paths; policy-permitted-but-unobserved paths are out of scope until host-firewall potential-reachability enrichment lands.
...
The initiating principal of an action — human **or** agentic (ADR-1005 terminology). Where a rule applies only to humans, docs say "human operator". Complements the three-way "agent" disambiguation above (agent daemon / governance agent / agentic worker): an agentic worker acting through MCP/REST is an operator; the agent daemon is never one.
