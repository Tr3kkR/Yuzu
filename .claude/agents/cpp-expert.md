---
name: cpp-expert
description: Use for any change touching `.cpp` / `.hpp` / `.h` under `agents/`, `server/`, `sdk/`, `tests/`, or `transport/`. C++23 language expert — reviews and edits for correctness, idiomatic style, std-library use, and portability across GCC 13+, Clang 18+, MSVC 19.38+, and Apple Clang 15+. Catches platform-specific UB, ABI-stability issues at the plugin boundary, and compiler-divergence bugs.
