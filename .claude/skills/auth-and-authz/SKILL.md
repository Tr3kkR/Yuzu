...
6. **SAML 2.0 SP** — thin first slice shipped, **plus group→role mapping**
   ...
   **SCIM linkage + deprovision-revoke has since SHIPPED for SAML too**
   (ADR-2001 PR4a — do NOT list it as a gap): a SAML login now mints its
   ...
   to revoke). See the "SCIM ↔ OIDC/SAML identity linkage" row above and
   `docs/auth-architecture.md` "SAML ↔ SCIM identity linkage" for the honest
   scope — **PR4b (#3066), the SAML deny-at-login backstop, has NOT
   shipped**, so a deprovisioned SAML identity can still re-authenticate and
   mint a fresh session until it lands.
7. ~~**SCIM v2 provisioning**~~ **DONE (Users + Groups→role mapping,
   ...
   *audit* row itself carries no severity — `AuditEvent` has no severity
   ...
   instant. **The deny-at-login backstop (ADR-2001 §4/PR3) has NOT
   shipped** — a deprovisioned linked identity can still re-authenticate via
   OIDC and mint a fresh session (which the next deprovision pass revokes
   again), so the loop isn't fully closed yet. See
   `docs/auth-architecture.md` "SCIM ↔ OIDC identity linkage for
   deprovision" and `docs/adr/2001-scim-oidc-identity-linkage.md`.
   **SAML gained the same coverage via PR4a** (SAML addendum to the same
   ADR): a stable `saml:<entity_id>#<NameID>` session principal
   ...
   (never `transient`), and deprovision-revoke of the linked SAML session
   (SAML mints no tokens, so there is nothing else to revoke). **PR4b
   (#3066), the SAML deny-at-login backstop, has NOT shipped** — the
   identical re-authenticate-and-mint-a-fresh-session window OIDC's PR3
   closes remains open on the SAML side. See `docs/auth-architecture.md`
   "SAML ↔ SCIM identity linkage" and the ADR's SAML addendum.
