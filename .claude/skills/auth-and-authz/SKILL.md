...
| SCIM ↔ OIDC identity linkage + credential revoke on deprovision (ADR-2001) | Shipped PR1+PR2+PR3 (SOC 2 CC6.8) | Login-time link (`oidc::link_oidc_login_to_scim`, `ScimStore::identity_links`/`--oidc-scim-link-claim`, default `sub`, allow-list `{sub,oid}`) + deprovision-time revoke across slug + every linked `oidc:` principal (`deprovision_revoke.{hpp,cpp}`, `oidc_principal.hpp`), credentials-first, fail-closed on non-persist. D1 (`yuzu_scim_deprovision_role_refused_with_active_link_total` + `scim.user.deprovision_role_refused_with_link` audit, `result=failure` — AuditStore has no severity column, so the ADR's "kCritical audit" language is realized as an *optional* `Severity::kCritical` AnalyticsEvent gated on analytics collection being enabled, never the audit row itself) covers an externally-elevated SCIM admin (#2021 guard still applies; a human must revoke that identity's tokens manually). D2 (`yuzu_scim_deprovision_unlinked_total`) covers a federated user who logged in but no link formed (misconfigured `--oidc-scim-link-claim`, or an IdP whose `externalId` shares no claim with any OIDC token Yuzu trusts — genuinely unrevocable via SCIM in that case); login records BOTH the `sub` and `oid` observation candidates (`oidc_login_observations`, keyed `(iss,sub,claim_name)`), so D2 reliably fires on a misconfigured link-claim (an `externalId` matching the *other*, unconfigured claim is still detected). Revocation is durable within ~60s (`ApiTokenStore` validate-cache TTL), not instant. **Deny-at-login backstop (§4/PR3) is now SHIPPED** — `ScimStore::linked_resource_active(iss,sub)` (a LEFT JOIN of `identity_links`⋈`scim_resources`, so an orphaned/hard-DELETEd link denies exactly like an explicit deactivation, never reading as "no link") drives `oidc_login_denied_deprovisioned`, called at a primary pre-mint check AND a post-mint re-check in `/auth/callback`; both deny sites emit the byte-identical `/login?error=sso_failed` redirect (no oracle), audit `auth.oidc.deprovisioned_denied` (`result=failure`), and bump `yuzu_auth_oidc_deprovisioned_denied_total`. **The honest scope, do NOT overclaim:** re-login against an *already-completed* deprovision is fully closed, unconditionally; a login racing an *in-flight* deprovision is narrowed (self-healed by the post-mint re-check in the overwhelming majority of timings) but not eliminated by construction — a microsecond check-then-mint gap is deliberately left open rather than closed via a cross-store lock (which this codebase's store discipline treats as a deadlock hazard), and is instead bounded by the eager revoke PR1/PR2 already provide plus the ~60s validate-cache window. Two remaining named residuals: (a) the migration-v3 partial-unique index on `scim_resources.external_id` is fail-closed — a server carrying a pre-existing duplicate non-empty `external_id` refuses to boot (dedup pre-upgrade, see `docs/user-manual/server-admin.md` Upgrade Notes); (b) **SAML is not yet SCIM-linked at all** — this ADR (link formation, revoke, and the deny-at-login backstop) covers OIDC only; extending to SAML is tracked as PR4 (issue #3066), a genuine open gap, not an oversight. See `docs/auth-architecture.md` "SCIM ↔ OIDC identity linkage for deprovision" and `docs/adr/2001-scim-oidc-identity-linkage.md`. |
...
6. **SAML 2.0 SP** — thin first slice shipped, **plus group→role mapping**
   ...
   reconfigure. Still the **largest single remaining enterprise-friction
   gap**, but state it accurately: the shipped slice *does* complete a full
   SP-initiated login for an IdP that accepts an unsigned AuthnRequest
   (`test_saml_provider.cpp:997-1040`), and admins are reachable via SAML
   (Section 1). The real absences are narrower — request signing, metadata
   fetch, attributes beyond the group claim, the Settings UI, and **all
   Windows support** (a Windows-hosted server cannot use SAML at all). See `docs/auth-architecture.md` "SAML 2.0 SP".
7. ~~**SCIM v2 provisioning**~~ **DONE (Users + Groups→role mapping,
   ...
   *audit* row itself carries no severity — `AuditEvent` has no severity
   ...
   instant. **The deny-at-login backstop (ADR-2001 §4/PR3) has SHIPPED** —
   a deprovisioned linked identity is now refused at OIDC login
   (`ScimStore::linked_resource_active` + `oidc_login_denied_deprovisioned`,
   `auth.oidc.deprovisioned_denied` audit, `yuzu_auth_oidc_deprovisioned_denied_total`).
   **Read the scope precisely — do not flatten this to "CC6.8 complete":**
   re-login against an already-completed deprovision is fully closed; a
   login racing an *in-flight* deprovision is narrowed by a post-mint
   re-check (self-heals the overwhelming majority of timings) but not
   eliminated by construction — a microsecond check-then-mint gap is
   deliberately left open (closing it would need a cross-store lock this
   codebase's store discipline treats as a deadlock hazard), bounded
   instead by the eager PR1/PR2 revoke and the ~60s cache window. **SAML is
   still not SCIM-linked at all** — none of ADR-2001 (link, revoke, or this
   backstop) covers a SAML-authenticated identity; tracked as PR4 (issue
   #3066), a real open gap. See `docs/auth-architecture.md` "SCIM ↔ OIDC
   identity linkage for deprovision" and
   `docs/adr/2001-scim-oidc-identity-linkage.md`.
