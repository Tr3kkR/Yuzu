...
### 1c. Split ↔ HA — CLOSED: decoupled (the split is NOT a prerequisite)

**Is the presentation/core split (ADR-0031) a prerequisite for active-active?
No — resolved 2026-09-07, decoupled.** Monolith active-active is the target; the
split is a parallel programme, not a gate for a 2nd replica. The one in-process
blocker (direct agent `Subscribe` stream) is solved by **gateway-fronting (WS-4)**,
not the split.
...
2. **Phase B — Multi-instance safety (ALL required before a 2nd replica):**
   ...
   **The split question (§1c) is CLOSED — decoupled; the split does not gate a 2nd replica.**
...
3. **Confirm the split question (§1c) before any Phase-B slice** — with the
   ADR-1005 owner. This can reshuffle the phase.
