...
- **Split ⇎ HA are decoupled, but the split *consumes* HA.** Not a prerequisite for a
  2nd replica (HA §1c CLOSED — the agent `Subscribe` blocker is gateway-fronting, HA
  WS-4). But it *inherits* HA WS-1 (sessions, done), rides HA WS-2a's `event_outbox`
  for the spine, and takes MCP replay durability from HA WS-2b — one-way dependencies.
...
**Highest-leverage first slices after WS-0:** WS-A4 (seam + INV-31-4 test) and WS-B1
