...
`.claude/routed-concerns-access-control.md` and `.claude/routed-concerns-software-estate.md`) are
read **from your working tree**, so a branch that predates a change to any of them silently runs
the old pipeline. At the time #2604 merged, 81 of 81 local branches predated it.
...
for f in .claude/skills/governance/SKILL.md .claude/routed-concerns.md \
         .claude/routed-concerns-access-control.md .claude/routed-concerns-software-estate.md \
         CLAUDE.md; do
...
**and** `.claude/routed-concerns-software-estate.md` and walk them row by row against that
...
(twice: once for access-control/request-admission, again for Forensics/per-user-software-data).
...
spine; skipping the third silently skips the Forensics-class plugin rows (e.g.
`browser_inventory`'s privacy contract). A row that is never opened routes nobody,
...
- **Rule 1:** there is **no routed-concerns walk at all** on that leg — its
  ...
  `.claude/routed-concerns-software-estate.md` is referenced anywhere in that file.
  Tracked as **#2684**. This is the only record of that gap in the tree.
...
1. **Re-run every gate whose DOMAIN THE FIX DIFF TOUCHES** — not only those whose
   ...
   - **Gate 3** — re-run the decision matrix, including **all three** routed-concern
     tables: `.claude/routed-concerns.md`,
     `.claude/routed-concerns-access-control.md` AND
     `.claude/routed-concerns-software-estate.md`. Opening only the first misses
     every auth, access-control and request-admission invariant, and skipping
     the third misses every Forensics/per-user-software-data invariant.
