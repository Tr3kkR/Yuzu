#pragma once

#include <array>
#include <span>

#include "../authz_model.hpp"
#include "../command_capability.hpp"

/// @file plugin_action_catalogue_windows_optional_features.hpp
/// One fragment of the command capability catalogue:
/// `windows_optional_features`'s two actions
/// (`agents/plugins/windows_optional_features/src/windows_optional_features_plugin.cpp`).
/// Classified by READING the implementation, per this package's spec.
///
/// Both `list` and `info` are ReadOnly/None — neither ever calls a DISM
/// enable/disable/add/commit export (grep-provable: the plugin resolves
/// exactly eight DismXxx exports, none of them mutating); grouped under
/// the existing `Inventory` securable, the same read-only-fact-collection
/// precedent `power_health`'s `battery`/`thermal`/`power_plan` actions use
/// (`plugin_action_catalogue_power_health.hpp`).
namespace yuzu::server::capdecls {

namespace detail {

inline constexpr std::array<CommandCapability, 2> kPluginActionCatalogueWindowsOptionalFeatures{{
    {
        .plugin = "windows_optional_features",
        .action = "list",
        .dispatch_class = DispatchClass::ReadOnly,
        .mutability = Mutability::None,
        .securable = "Inventory",
        .operation = authz::Operation::Read,
        .risk_tier = authz::RiskTier::Low,
        .system_reserved = false,
        .execute_gate = ExecuteGate::None,
    },
    {
        .plugin = "windows_optional_features",
        .action = "info",
        .dispatch_class = DispatchClass::ReadOnly,
        .mutability = Mutability::None,
        .securable = "Inventory",
        .operation = authz::Operation::Read,
        .risk_tier = authz::RiskTier::Low,
        .system_reserved = false,
        .execute_gate = ExecuteGate::None,
    },
}};

// #1398: every row in kPluginActionCatalogueWindowsOptionalFeatures must
// author .execute_gate — an omission would value-initialize to
// ExecuteGate::Unspecified (the zero enumerator), which is a genuine compile
// failure here rather than a silent runtime gap. See ExecuteGate's doc
// comment in command_capability.hpp.
static_assert(
    ::yuzu::server::detail::all_gates_specified(kPluginActionCatalogueWindowsOptionalFeatures),
    "every row in kPluginActionCatalogueWindowsOptionalFeatures must author .execute_gate");

} // namespace detail

/// A `std::span` view over the fixed catalogue above — one of the several
/// sources a `CommandCapabilityRegistry` is composed from. Inline function
/// over file-scope `constexpr` storage: this header only DECLARES rows, it
/// never aggregates or singleton-owns a registry.
[[nodiscard]] inline std::span<const CommandCapability>
plugin_action_catalogue_windows_optional_features() noexcept {
    return detail::kPluginActionCatalogueWindowsOptionalFeatures;
}

} // namespace yuzu::server::capdecls
