...
void UpdateRegistry::create_tables() {
    ...
    if (rc != SQLITE_OK) {
        spdlog::error("UpdateRegistry: create_tables failed: {}",
...
void UpdateRegistry::upsert_package(const UpdatePackage& pkg) {
    ...
    if (rc != SQLITE_OK) {
        spdlog::error("UpdateRegistry: upsert_package prepare failed: {}",
                       sqlite3_errmsg(db_));
        return;
    }
    ...
    if (rc != SQLITE_DONE) {
        spdlog::error("UpdateRegistry: upsert_package step failed for {}/{}/{}: {}",
                       pkg.platform, pkg.arch, pkg.version, sqlite3_errmsg(db_));
    } else {
        spdlog::info("UpdateRegistry: upserted package {}/{}/{} ({})",
...
void UpdateRegistry::remove_package(const std::string& platform,
                                    const std::string& arch,
                                    const std::string& version) {
    if (!db_) return;

    const char* sql =
        "DELETE FROM update_packages WHERE platform = ? AND arch = ? AND version = ?";

    sqlite3_stmt* stmt = nullptr;
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        spdlog::error("UpdateRegistry: remove_package prepare failed: {}",
                       sqlite3_errmsg(db_));
        return;
    }
    ...
    rc = sqlite3_step(stmt);
    if (rc != SQLITE_DONE) {
        spdlog::error("UpdateRegistry: remove_package step failed for {}/{}/{}: {}",
                       platform, arch, version, sqlite3_errmsg(db_));
    } else {
...
std::vector<UpdatePackage> UpdateRegistry::list_packages() const {
    ...
    if (!db_) return packages;

    const char* sql =
        "SELECT platform, arch, version, sha256, filename, mandatory, "
        "rollout_pct, uploaded_at, file_size "
    ...
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        spdlog::error("UpdateRegistry: list_packages prepare failed: {}",
                       sqlite3_errmsg(db_));
        return packages;
    }
...
std::optional<UpdatePackage> UpdateRegistry::latest_for(const std::string& platform,
    ...
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        spdlog::error("UpdateRegistry: latest_for prepare failed: {}",
                       sqlite3_errmsg(db_));
        return std::nullopt;
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (!best.has_value() ||
            compare_versions(pkg.version, best->version) > 0) {
            best = std::move(pkg);
        }
    }
...
bool UpdateRegistry::is_eligible(const std::string& agent_id, int rollout_pct) {
    return rollout_pct >= 100 ||
           (std::hash<std::string>{}(agent_id) % 100) <
               static_cast<unsigned>(rollout_pct);
}
