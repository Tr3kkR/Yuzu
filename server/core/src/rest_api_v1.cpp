  ...
  "paths": {
    ...
    "/inventory/software": {
      "get": {"summary": "Fleet-wide installed-software inventory (typed daily-sync store, ADR-0016)", "tags": ["Inventory"], "description": "Installed-software rows across the fleet from the typed SoftwareInventoryStore (DISTINCT from the generic /inventory/* routes, which read the generic blob store). Rows carry name, version, publisher, install_date plus the blob-v2 package fields: kind (package|app), ecosystem (rpm|deb|apk|pacman|windows|macos|homebrew), epoch, release, arch, signature_status (rpm stored-tag), distro_id, distro_version — fields an ecosystem does not store are empty, never synthesised. Requires Inventory:Read. Results are scoped to the caller's management groups; out-of-scope devices are dropped and counted in devices_omitted (a positive value means matching software exists outside your scope — an empty/short result does NOT mean the software is absent fleet-wide). Capped at limit rows (max 1000); result_truncated_by_cap=true means more exist past the cap (keyset pagination is a follow-up). On store degradation the endpoint returns 503 (never an empty 200) so a vulnerability query cannot read a transient outage as 'installed nowhere'.", "parameters": [{"name": "name", "in": "query", "schema": {"type": "string"}, "description": "Exact software-name filter (optional)"}, {"name": "agent_id", "in": "query", "schema": {"type": "string"}, "description": "Exact agent filter (optional)"}, {"name": "limit", "in": "query", "schema": {"type": "integer", "default": 100, "maximum": 1000}}], "responses": {"200": {"description": "{data:{software[], count, devices_omitted, result_truncated_by_cap?, audit_persisted?}}"}, "400": {"description": "Non-integer limit"}, "401": {"description": "Unauthenticated"}, "403": {"description": "Requires Inventory:Read"}, "503": {"description": "Software inventory store unavailable or degraded"}}}
        ...
        R"json(
    "/sle/summary": {
      "get": {"summary": "SLE fleet posture headline (ADR-0024)", "tags": ["SLE"], "description": "Fleet-wide software-licensing posture aggregates from the precomputed rollup. Requires SoftwareLicensing:Read — a NEW securable gating the SLE surface, DISTINCT from the License securable (Yuzu's own product licence, capability 22.3) and from Inventory:Read (which still gates the /inventory installed-software catalog). Pinned GLOBAL-only (ADR-0017): a management-group-confined principal is denied (403), never served a partial rollup. Fields: as_of (rollup as-of epoch; 0 = not yet evaluated), evaluated (bool), products, installs, lapsed_products, expiring_soon, next_expiry_at. as_of/evaluated come from the first-class posture meta stamp (license_posture_meta), so an evaluated-but-empty estate reads evaluated=true with zero counts — distinct from a never-run evaluator (evaluated=false, as_of=0). The compliance evaluator recomputes hourly (keep-last-good: on failure the stamp visibly ages). A store degrade returns 503 (never an empty 200). Every response carries X-Correlation-Id.", "responses": {"200": {"description": "{data:{as_of, evaluated, products, installs, lapsed_products, expiring_soon, next_expiry_at}}", "headers": {"X-Correlation-Id": {"schema": {"type": "string"}}}}, "401": {"description": "Unauthenticated"}, "403": {"description": "Requires global SoftwareLicensing:Read (a management-group-confined principal is denied here — the aggregate is pinned global-only)"}, "503": {"description": "Posture rollup unavailable or degraded (A4 envelope, retryable)", "content": {"application/json": {"schema": {"$ref": "#/components/schemas/A4ErrorEnvelope"}}}}}}
    },
    "/sle/licenses": {
      "get": {"summary": "SLE fleet licence posture rows (ADR-0024)", "tags": ["SLE"], "description": "Per-product detected-licence posture rows from the precomputed rollup, filtered then capped at limit (max 1000). Requires GLOBAL SoftwareLicensing:Read (pinned global-only per ADR-0017 — a confined principal is 403'd, never served a partial rollup). Filters: state (licensed|subscription_active|trial|grace|expired|unlicensed|unknown|lapsed — matches products whose corresponding effective-state count is non-zero; lapsed = expired union unlicensed), expiring_within_days (products with a positive next_expiry_at within N days), q (case-insensitive substring on title/vendor/product_key). Empty until the compliance evaluator's first hourly cycle populates the rollup; a store degrade returns 503 (never an empty 200). result_truncated_by_cap=true means more rows match than the page holds.", "parameters": [{"name": "state", "in": "query", "schema": {"type": "string", "enum": ["licensed", "subscription_active", "trial", "grace", "expired", "unlicensed", "unknown", "lapsed"]}}, {"name": "expiring_within_days", "in": "query", "schema": {"type": "integer", "minimum": 0}}, {"name": "q", "in": "query", "schema": {"type": "string"}}, {"name": "limit", "in": "query", "schema": {"type": "integer", "default": 200, "maximum": 1000}}], "responses": {"200": {"description": "{data:{licenses[], count, as_of, result_truncated_by_cap?}}", "headers": {"X-Correlation-Id": {"schema": {"type": "string"}}}}, "401": {"description": "Unauthenticated"}, "403": {"description": "Requires global SoftwareLicensing:Read"}, "503": {"description": "Posture rollup unavailable or degraded (A4 envelope)", "content": {"application/json": {"schema": {"$ref": "#/components/schemas/A4ErrorEnvelope"}}}}}}
    },
    "/sle/licenses/{key}/devices": {
      "get": {"summary": "SLE devices carrying a detected licence for a product key (ADR-0024)", "tags": ["SLE"], "description": "Fan-out list: which devices carry a detected licence for product_key, capped at limit (max 1000). Requires GLOBAL SoftwareLicensing:Read. This is a true fan-out list read registered as an ADR-0017 PR-A flip-wave consumer (umbrella #1634, deny-precedence prerequisite #1715): it ships global-gated, and the admit-then-filter management-group chokepoint (with its 'filters before the LIMIT' completeness guarantee) lands at the flip. Rows resolve through the product registry's alias pairs and collapse to one row per device (worst effective state wins, server-now lapse rule); an unknown product_key is an honest empty list; a store degrade returns 503.", "parameters": [{"name": "key", "in": "path", "required": true, "schema": {"type": "string"}, "description": "Product key (registry norm_key)"}, {"name": "limit", "in": "query", "schema": {"type": "integer", "default": 200, "maximum": 1000}}], "responses": {"200": {"description": "{data:{product_key, devices[].{agent_id, hostname, state, expiry_at}, count, result_truncated_by_cap?}}", "headers": {"X-Correlation-Id": {"schema": {"type": "string"}}}}, "401": {"description": "Unauthenticated"}, "403": {"description": "Requires global SoftwareLicensing:Read"}, "503": {"description": "Fan-out read unavailable or degraded (A4 envelope)", "content": {"application/json": {"schema": {"$ref": "#/components/schemas/A4ErrorEnvelope"}}}}}}
    },
    "/sle/agents/{agent_id}": {
      "get": {"summary": "SLE per-agent detected-licence drill (ADR-0024)", "tags": ["SLE"], "description": "One device's detected software licences (product, type, channel, state, expiry, confidence, exe_hints) INCLUDING the per-user fields user_scope/user_ref (personal data, ADR-0024 Decision 11). Requires SoftwareLicensing:Read SCOPED to the device (tier + management group, ancestor-aware — a global grant passes fleet-wide, otherwise the caller must hold Read via a management group the device is in; 403 outside scope). Individual-identifying data, so every call emits a per-open sle.agent.view behavioural audit and FAILS CLOSED (503 + Sec-Audit-Failed: true) when that audit row cannot persist — the licence PII is never served without durable evidence (SOC 2 CC7.2). Also carries effective_user_ref_mode (ADR-0024 Decision 10 read-back): the user-ref mode the device's LAST STORED sync blob declared ('collect'|'hash'|'omit'; null = the device never synced this source) — the operator's verification that a --license-scan-user-ref knob flip actually landed on the endpoint. A store degrade returns 503 (never an empty 200).", "parameters": [{"name": "agent_id", "in": "path", "required": true, "schema": {"type": "string"}}], "responses": {"200": {"description": "{data:{agent_id, licenses[].{product, vendor, version, license_type, state, expiry_at, channel, key_hint, detector, confidence, exe_hints, user_scope, user_ref, collected_at, first_seen, last_seen}, count, effective_user_ref_mode}}", "headers": {"X-Correlation-Id": {"schema": {"type": "string"}}}}, "401": {"description": "Unauthenticated"}, "403": {"description": "Outside the caller's management scope (SoftwareLicensing:Read on the device)"}, "503": {"description": "Store degraded, OR the sle.agent.view audit row could not persist (the latter carries Sec-Audit-Failed: true).", "headers": {"Sec-Audit-Failed": {"schema": {"type": "string", "enum": ["true"]}, "description": "Present when per-agent licence PII was withheld because the access-audit row failed to persist."}}, "content": {"application/json": {"schema": {"$ref": "#/components/schemas/A4ErrorEnvelope"}}}}}},
      "delete": {"summary": "Decommission an agent's stored per-agent data — the durable erasure cascade (ADR-0024 Decision 11 / GDPR Art. 17)", "tags": ["SLE"], "description": "Fans delete_agent across every registered per-agent store (generic inventory, installed software, app-perf, device-CI, detected licences incl. the per-user user_ref rows). Requires SoftwareLicensing:Delete SCOPED to the device (only Administrator and IT Service Owner hold Delete). AUDIT-BEFORE-ERASE, fail-closed: a durable sle.agent.decommission 'attempt' event must persist BEFORE the cascade runs (503 + Sec-Audit-Failed and NO erasure otherwise), followed by a 'success'/'partial' outcome event carrying the per-store breakdown. HONESTY: each store's delete swallows its own transient failure, so 'deleted' means the store's delete was invoked without an exception — ATTEMPTED, not row-level-confirmed, erasure (#1947 tracks the status-returning upgrade); a store that threw reports failed and the response is 500 — the cascade is idempotent, re-issue the DELETE.", "parameters": [{"name": "agent_id", "in": "path", "required": true, "schema": {"type": "string"}}], "responses": {"200": {"description": "{data:{agent_id, decommissioned:true, stores:{<store>:deleted|skipped|failed}, deleted, skipped, failed}}", "headers": {"X-Correlation-Id": {"schema": {"type": "string"}}}}, "401": {"description": "Unauthenticated"}, "403": {"description": "Lacks SoftwareLicensing:Delete on this device (Operator/Viewer are read/write-only; scope confines a group-limited ITSO)"}, "500": {"description": "Cascade incomplete — one or more stores threw; re-issue the DELETE (idempotent)"}, "503": {"description": "Gate/cascade unwired, OR the attempt-audit row could not persist (Sec-Audit-Failed: true — nothing was erased)", "headers": {"Sec-Audit-Failed": {"schema": {"type": "string", "enum": ["true"]}}}}}}
    },
    "/sle/agents/{agent_id}/surfaces": {
      "post": {"summary": "SLE live detection-surface diagnostics for one device (ADR-0024 Decision 10)", "tags": ["SLE"], "description": "Dispatches the agent's license_scan 'surfaces' action over the command path and returns the live per-surface probe status: which detection surfaces ran (rows yielded) and which failed, with the failure reason token (e.g. privilege_missing, wmi_deadline_exceeded). The flappy per-surface diagnostics are LIVE-ONLY by design — they never persist server-side; the config-stable counterpart is the drill's effective_user_ref_mode. POST because it dispatches a real command to the device. Requires SoftwareLicensing:Read AND Execution:Execute, both SCOPED to the device. The payload carries NO per-user data (per-user outcomes aggregate per surface on the agent; the server-side whitelist parser drops any non-probe_status line). Audited set-and-proceed as sle.agent.surfaces before dispatch. Bounded synchronous poll (~20s): a WMI-degraded host that needs longer answers 504 honestly — the stored drill remains available.", "parameters": [{"name": "agent_id", "in": "path", "required": true, "schema": {"type": "string"}}], "responses": {"200": {"description": "{data:{agent_id, live:true, as_of, count, surfaces[].{surface, status(ok|error), rows?, detail?}}}", "headers": {"X-Correlation-Id": {"schema": {"type": "string"}}}}, "401": {"description": "Unauthenticated"}, "403": {"description": "Outside the caller's management scope (SoftwareLicensing:Read + Execution:Execute on the device)"}, "429": {"description": "Too many concurrent live queries (bounded in-flight cap)"}, "502": {"description": "Device reported an error / malformed output / terminal failure"}, "503": {"description": "Dispatch or response store unavailable, or the device is offline (A4 envelope, retryable)"}, "504": {"description": "Device did not respond within the poll budget"}}}
    },)json"
