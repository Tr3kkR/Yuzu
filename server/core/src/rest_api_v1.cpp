...
const std::string& openapi_spec() {
  ...
  "paths": {
    ...
    "/sle/licenses/{key}/devices": {
      "get": {"summary": "SLE devices carrying a detected licence for a product key (ADR-0024)", "tags": ["SLE"], "description": "Fan-out list: which devices carry a detected licence for product_key, capped at limit (max 1000). Requires GLOBAL SoftwareLicensing:Read. This is a true fan-out list read registered as an ADR-0017 PR-A flip-wave consumer (umbrella #1634, deny-precedence prerequisite #1715): it ships global-gated, and the admit-then-filter management-group chokepoint (with its 'filters before the LIMIT' completeness guarantee) lands at the flip, not in PR1a. In PR1a the per-device posture breakdown is not yet computed, so this returns an honest empty list; a degrade returns 503.", "parameters": [{"name": "key", "in": "path", "required": true, "schema": {"type": "string"}, "description": "Product key (registry norm_key)"}, {"name": "limit", "in": "query", "schema": {"type": "integer", "default": 200, "maximum": 1000}}], "responses": {"200": {"description": "{data:{product_key, devices[].{agent_id, hostname, state, expiry_at}, count, result_truncated_by_cap?}}", "headers": {"X-Correlation-Id": {"schema": {"type": "string"}}}}, "401": {"description": "Unauthenticated"}, "403": {"description": "Requires global SoftwareLicensing:Read"}, "503": {"description": "Fan-out read unavailable or degraded (A4 envelope)", "content": {"application/json": {"schema": {"$ref": "#/components/schemas/A4ErrorEnvelope"}}}}}}
    },
    "/sle/agents/{agent_id}": {
