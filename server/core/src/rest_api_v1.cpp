...
const std::string& openapi_spec() {
  ...
  "paths": {
    ...
    "/agent/plugin-policy": {
      "get": {"summary": "Plugin trust-bundle distribution endpoint (plugin-signing's REST v1 twin)", "tags": ["Settings"], "description": "#4028 — hardened onto PluginSigning:Read (dedicated RBAC securable, Administrator-only, floored in authz_topology_floor.hpp — replaces the prior whole-endpoint admin_fn_ gate with equivalent practical posture) + the A4 envelope + the shared settings_model::build_plugin_signing_settings builder the Settings plugin-signing fragment (GET /fragments/settings/plugin-signing) also calls — this route alone adds trust_bundle_pem (the superset); deliberately no separate /settings/plugin-signing route (#4028 acceptance criteria: harden this route, don't duplicate it). Audited fail-closed (settings.plugin_signing.read): a non-admin token holder learning when the trust anchor rotates (sha256 changes) is useful reconnaissance for a supply-chain attacker (CC6.1 least-privilege). Returns the current PEM trust bundle and the require-signature flag for out-of-band agent-config distribution (operators curl this into --plugin-trust-bundle on each agent host). Pilot-stable: the /api/v1/agent/... path and response shape may change before the GA v1 contract is finalized; the field set is unlikely to shrink.", "responses": {"200": {"description": "{data: {enabled, required, cert_count, sha256, subjects[], bundle_unreadable, trust_bundle_pem}, meta}. Bundle absent (still 200, not 404 — a normal operational state): {enabled: false, required, cert_count: 0, sha256: \"\", subjects: [], bundle_unreadable: false, trust_bundle_pem: \"\"}"}, "403": {"description": "Permission denied (PluginSigning:Read)"}, "500": {"description": "Trust bundle exists on disk but is unreadable (A4 error envelope)"}, "503": {"description": "Audit subsystem unavailable — refuses to serve without durable evidence (Sec-Audit-Failed not applicable; A4 error envelope, correlation_id)"}}}
    },
    "/settings/tls": {
