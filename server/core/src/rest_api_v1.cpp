...
void RestApiV1::register_routes(
    ...
    sink.Get("/api/v1/network/devices",
             [perm_fn, net_perf_fn](const httplib::Request& req, httplib::Response& res) {
                 ...
                 if (req.has_param("cooc")) {
                     const auto t = req.get_param_value("cooc");
                     if (t == "device")
                         cooc = NetCoocFilter::kAlsoDevice;
                     else if (t == "app")
                         cooc = NetCoocFilter::kAlsoApp;
                     else if (t == "network_only")
