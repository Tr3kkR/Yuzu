// Static JavaScript bundles — HTMX core and HTMX-SSE extension
//
// HTMX 2.0.4 (https://github.com/bigskysoftware/htmx) and the htmx-ext-sse
// extension (https://github.com/bigskysoftware/htmx-extensions/tree/main/src/sse)
// are embedded here so the dashboard works in air-gapped environments with
// no external CDN dependencies. Both upstream packages are released under
// the BSD Zero-Clause License (0BSD), which imposes no conditions on
// redistribution.
//
// The HTMX 2.0.4 minified bundle is ~51 KB, which exceeds MSVC's 16380-byte
// raw string literal limit (error C2026). It is split into 4 chunks of
// <=14000 bytes each and concatenated at static-init time into a single
// std::string. The reassembled bundle is byte-identical to the upstream
// minified file.
//
// Consumers in server.cpp expect:
//   extern const std::string kHtmxJs;
//   extern const std::string kSseJs;
// and pass them to httplib::Response::set_content (which accepts
// const std::string&).
