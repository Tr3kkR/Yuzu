...
class EnginePrincipalStore;   // engine-principal lifecycle tools (ADR-1005 item 2b, plan PR 4.3);
...
private:
    ...
    // See the setters above for the member-vs-parameter rationale. Nullable;
    // every engine-principal tool checks before use and answers a clean
    // "unavailable" error rather than dereferencing a null store.
