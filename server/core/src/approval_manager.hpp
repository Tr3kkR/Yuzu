...
/// and decodes straight back to the exempt `kUnspecified`. Each of those two
/// switches carries its own `error` pragma — the one below for this function, and
/// its twin around `to_string` in the .cpp — because there is no project-wide
/// setting; see the pragma's own comment. The trailing `return true` is
