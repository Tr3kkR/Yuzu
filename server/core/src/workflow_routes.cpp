    ...
    sink.Post(R"(/api/instructions/([^/]+)/execute)", [auth_fn, perm_fn, audit_fn, emit_fn,
        ...
        if (approval_manager && def.approval_mode != "auto") {
            ...
            if (needs_approval) {
                ...
                // falsely exempt.
                // #1398 hardening: bind target_action so a FUTURE interactive
                // ticket-redemption implementation (design doc follow-up #6 —
                // none exists on this route today) inherits the same
                // definition-mutation protection ScheduleRunner needed,
                // rather than requiring its own security round later.
                auto result = approval_manager->submit(def_id, session->username, scope_expr, "",
                                                       ApprovalOrigin::kInstruction,
                                                       def.plugin + "." + def.action);
                if (!result) {
