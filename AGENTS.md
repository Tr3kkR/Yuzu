...
Issues: GitHub at `github.com/Tr3kkR/Yuzu` via `gh`. Filing, labelling, and closing follow `docs/agents/issue-standard.md` (ADR-3001) — duplicate search before filing is mandatory, and automation never closes a `security`- or `do-not-close`-labelled issue. **An exploitable vulnerability is never a public issue — private advisory only (`SECURITY.md`).** Label taxonomy: `docs/agents/triage-labels.md`. Command crib: `docs/agents/issue-tracker.md`.
...
| MCP server (incl. the #2405 pre-approval input-schema gate — closed subset compiler, boot-enforced) | `docs/mcp-server.md` | `security-guardian` on `/mcp/v1/`, `mcp_server.*`, `mcp_jsonrpc.hpp`, `mcp_policy.hpp`, `mcp_input_schema.*` change |
...
**This file is stricter than `CLAUDE.md`, deliberately.** `CLAUDE.md`'s routed-concern rows carry the catastrophic-if-violated invariant inline; this file's rows carry only the trigger and the doc. The two are not out of sync and neither should be "fixed" to match the other — they serve different readers, and this file is reloaded far more often. Change one without the other.
