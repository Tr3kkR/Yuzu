...
and `AuthDB` as SQLite stores years after both migrated to Postgres, and cited three
`.codex/agents/*.md` briefs that do not exist. A second copy of a routed-concern table is worse than
no copy, because it reads as current.
...
Repo-committed Codex skills live in `.codex/skills/` — including `governance`, `test`, `release`,
`auth-and-authz`, `ci-cache`, `gateway-eunit`, `gateway-dialyzer`, `cpp-expert`, `cpp-safety`, and
the `yuzu-*` build/proto/ABI helpers. Each documents its own usage; read the SKILL.md rather than a
copy here.
