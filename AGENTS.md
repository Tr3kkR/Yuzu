...
| MCP server architecture, tier-before-RBAC ordering, kill switches, audit pattern, pre-approval input-schema gate (#2405 — closed subset compiler, boot-enforced) | `docs/mcp-server.md` | `security-guardian` on `/mcp/v1/`, `mcp_server.{hpp,cpp}`, `mcp_jsonrpc.hpp`, `mcp_policy.hpp`, `mcp_input_schema.{hpp,cpp}` change |
