/**
 * wmi_bounded.cpp — bounded WMI query/method-call implementation.
 * See wmi_bounded.hpp for the contract. Hoisted from
 * agents/plugins/license_scan/src/licensing_wmi.cpp (roadmap C-8).
 */
...
void extract_row(IWbemClassObject* obj, WmiRow& row) {
...
std::string connect_bounded(const std::wstring& wmi_namespace, ComInit& com,
...
BoundedQueryResult run_bounded_wmi_query(const std::wstring& wmi_namespace, const std::wstring& wql,
    ...
    while (true) {
        ...
        // every iteration. The WBEM_S_TIMEDOUT retry branch below only bounds
        // a Next() that STALLS; a misbehaving provider that dribbles rows just
        // under the per-Next timeout keeps returning WBEM_S_NO_ERROR and never
        // trips that branch — without this check it could run unbounded.
        if (GetTickCount64() - start_ticks >= opts.enumeration_deadline_ms) {
        ...
        ULONG count = 0;
        hr = enumerator->Next(static_cast<long>(opts.next_timeout_ms), 1, &raw_obj, &count);
        ...
        if (hr == WBEM_S_TIMEDOUT) {
            ...
            // yet — still working", not "wedged". Retry, but only while
            // inside the overall enumeration deadline.
...
BoundedQueryResult exec_object_method(const std::wstring& wmi_namespace,
    ...
    if (SUCCEEDED(hr) && in_signature) {
        ...
        for (const auto& [name, value] : in_params) {
            ...
            in_params_instance->Put(name.c_str(), 0, &var.v, 0);
        }
    }
    ...
    while (true) {
        ...
        hr = call_result->GetResultObject(static_cast<long>(opts.next_timeout_ms), out_params.put());
        if (hr == WBEM_S_TIMEDOUT) {
            const ULONGLONG elapsed = GetTickCount64() - start_ticks;
            if (elapsed >= opts.enumeration_deadline_ms) {
                result.error = "wmi_next_timeout";
