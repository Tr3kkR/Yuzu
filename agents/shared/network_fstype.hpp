...
// uninterruptible open/getdents/read pins a worker of the agent's ONE bounded command pool
// (shared with every plugin, quarantine dispatch included) until the server returns, and a
// hard mount can hold it for minutes to forever. Callers read /proc/self/mountinfo, pass each
// entry's filesystem type here, and skip the paths under a match.
//
// This is a DENY-list by name and is inherently incomplete -- the durable fix is a known-local
// allowlist, which needs its own review. It covers the network and paravirtualised/cluster types
// whose backing store is remote: nfs*, cifs, smb3, smbfs, afs, ceph, glusterfs, 9p, virtiofs,
// lustre, beegfs, gfs2, ocfs2, autofs (a direct-map trigger point blocks on traversal), and the
// "fuse.<suffix>" types that are network-backed.
//
// Same list as filesystem_posture_parsers.hpp's `is_network_fstype` (governance G4-04); that
// plugin still carries its own copy and should consume this header (tracked follow-up). TAR's
