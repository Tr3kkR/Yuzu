...
socket_t connect_with_timeout(const char* host, int port, int timeout_ms) {
    ...
    setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const char*>(&tv_recv), sizeof(tv_recv));
#else
...
std::string http_request(const char* host, int port, const std::string& request,
                         int timeout_ms) {
...
CloudIdentity try_aws() {
    ...
    std::string doc_req =
        "GET /latest/dynamic/instance-identity/pkcs7 HTTP/1.1\r\n"
        "Host: 169.254.169.254\r\n"
        "X-aws-ec2-metadata-token: " + token + "\r\n"
        "Connection: close\r\n\r\n";

    auto pkcs7 = http_request(kImdsIp, kImdsPort, doc_req, kTimeoutMs);
    if (pkcs7.empty()) return {};
    ...
    spdlog::info("Cloud identity: detected AWS (instance={}, region={})",
                 id.instance_id, id.region);
    return id;
}
...
CloudIdentity try_azure() {
    ...
    std::string doc_req =
        "GET /metadata/attested/document?api-version=2021-02-01 HTTP/1.1\r\n"
        "Host: 169.254.169.254\r\n"
        "Metadata: true\r\n"
        "Connection: close\r\n\r\n";
    ...
    std::string meta_req =
        "GET /metadata/instance/compute?api-version=2021-02-01 HTTP/1.1\r\n"
        "Host: 169.254.169.254\r\n"
        "Metadata: true\r\n"
        "Connection: close\r\n\r\n";
    ...
    spdlog::info("Cloud identity: detected Azure (vmId={}, location={})",
                 id.instance_id, id.region);
    return id;
}
...
CloudIdentity try_gcp() {
    ...
    spdlog::info("Cloud identity: detected GCP (instance={}, region={})",
                 id.instance_id, id.region);
    return id;
}
...
CloudIdentity detect_cloud_identity() {
    ...
    if (auto aws = try_aws(); aws.valid()) return aws;
    if (auto azure = try_azure(); azure.valid()) return azure;
    if (auto gcp = try_gcp(); gcp.valid()) return gcp;
