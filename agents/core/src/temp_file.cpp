...
static std::wstring build_temp_path_w(const wchar_t* dir,
                                       const char* prefix,
                                       const char* hex,
                                       const char* suffix) {
    ...
    for (const char* p = prefix; *p; ++p) result += static_cast<wchar_t>(*p);
    for (const char* p = hex; *p; ++p)    result += static_cast<wchar_t>(*p);
    if (suffix) {
        for (const char* p = suffix; *p; ++p) result += static_cast<wchar_t>(*p);
    }
    return result;
}
...
static int wide_to_utf8(const wchar_t* wide, char* out, size_t out_size) {
    int written = WideCharToMultiByte(
        CP_UTF8, 0, wide, -1, out, static_cast<int>(out_size), nullptr, nullptr);
    ...
    size_t      path_out_size)
{
    ...
    if (!get_temp_dir_w(directory, temp_dir_w, MAX_PATH)) return -1;

    char hex[33]{};
    if (!generate_random_hex(hex, sizeof(hex))) return -1;

    auto full_path = build_temp_path_w(temp_dir_w, pfx, hex, sfx);
    if (full_path.size() >= MAX_PATH) return -1;
    ...
    size_t      path_out_size)
{
    ...
    if (mkdtemp(tmpl.data()) == nullptr) return -1;
