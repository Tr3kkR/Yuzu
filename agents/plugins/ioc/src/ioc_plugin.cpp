...
std::vector<ConnectionInfo> get_connections() {
        ...
        if (size > 0) {
            ...
            if (GetExtendedTcpTable(buffer.data(), &size, FALSE, AF_INET,
                ...
                for (DWORD i = 0; i < table->dwNumEntries; ++i) {
                    ...
                    conns.push_back({
                        local_buf,
                        ntohs(static_cast<uint16_t>(row.dwLocalPort)),
                        remote_buf,
                        ntohs(static_cast<uint16_t>(row.dwRemotePort)),
                        std::move(state_str),
                        row.dwOwningPid
                    });
                }
    ...
    // --- TCP v6 ---
    {
        ...
        if (size > 0) {
            std::vector<BYTE> buffer(size);
            if (GetExtendedTcpTable(buffer.data(), &size, FALSE, AF_INET6,
                                    TCP_TABLE_OWNER_PID_ALL, 0) == NO_ERROR) {
...
std::string parse_proc_ipv6(std::string_view hex_addr) {
    ...
    for (int word = 0; word < 4; ++word) {
        for (int b = 0; b < 4; ++b) {
            ...
            unsigned val = 0;
            std::from_chars(hex_addr.data() + offset,
                            hex_addr.data() + offset + 2, val, 16);
...
void parse_proc_net(const char* path, bool ipv6,
                    std::vector<ConnectionInfo>& conns) {
    std::ifstream f(path);
    if (!f.is_open()) return;
    ...
    while (std::getline(f, line)) {
        ...
        std::string idx, local, remote, state_hex, tx_rx, tr_tm, retrnsmt,
                    uid_str, timeout_str, inode_str;
...
std::vector<ConnectionInfo> get_connections() {
    ...
    while (std::getline(ss, line)) {
        ...
        ls >> command >> pid_str >> user >> fd >> type >> device >> size_off
           >> node >> name;
        ...
        std::string local_part = (arrow != std::string::npos)
            ? name.substr(0, arrow) : name;
        std::string remote_part = (arrow != std::string::npos)
            ? name.substr(arrow + 2) : "*:0";

        // Split addr:port (handle IPv6 [addr]:port)
        auto split_addr_port = [](const std::string& s)
                -> std::pair<std::string, uint16_t> {
            auto last_colon = s.rfind(':');
            if (last_colon == std::string::npos) return {s, 0};
            uint16_t port = 0;
            auto port_str = s.substr(last_colon + 1);
            std::from_chars(port_str.data(),
...
struct Sha256 {
    ...
    void update(const uint8_t* data, size_t len) {
        ...
        for (size_t i = 0; i < len; ++i) {
            ...
            if (block_len_ == 64) { process_block(); block_len_ = 0; }
        }
    }
...
std::string sha256_file(const std::filesystem::path& path) {
    ...
    while (f.read(reinterpret_cast<char*>(buf.data()),
                  static_cast<std::streamsize>(buf.size())) || f.gcount() > 0) {
...
void check_ip_addresses(yuzu::CommandContext& ctx,
    ...
    for (const auto& ip : ips) {
        ...
        for (const auto& conn : conns) {
            if (conn.remote_addr == ip || conn.local_addr == ip) {
                found = true;
                if (conn.pid > 0) {
                    detail = std::format("Active connection - {} (pid {})",
                                         conn.state, conn.pid);
                } else {
        ...
        if (!found) detail = "Not found in active connections";

        ctx.write_output(std::format("ip|{}|{}|{}",
            escape_pipes(ip), found ? "true" : "false",
            escape_pipes(detail)));
    }
}

void check_domains(yuzu::CommandContext& ctx,
                   const std::vector<std::string>& domains) {
    for (const auto& domain : domains) {
#ifdef _WIN32
        bool found = check_dns_cache(domain);
        std::string detail = found ? "Found in DNS resolver cache"
                                   : "Not in DNS cache";
        ctx.write_output(std::format("domain|{}|{}|{}",
            escape_pipes(domain), found ? "true" : "false",
            escape_pipes(detail)));
#else
        ...
        if (!found) detail = "Not found in /etc/hosts (DNS cache not available)";
...
void check_file_hashes(yuzu::CommandContext& ctx,
    ...
    if (search_paths.empty()) {
        for (const auto& hash : hashes) {
            ctx.write_output(std::format(
                "hash|{}|false|No target file paths specified for hash comparison",
                escape_pipes(hash)));
        }
        return;
    }
    ...
    for (const auto& hash : hashes) {
        ...
        std::transform(search_hash.begin(), search_hash.end(),
                       search_hash.begin(),
        ...
        if (!matched) detail = "No matching files found";

        ctx.write_output(std::format("hash|{}|{}|{}",
            escape_pipes(hash), matched ? "true" : "false",
            escape_pipes(detail)));
    }
}

void check_file_paths(yuzu::CommandContext& ctx,
    ...
    for (const auto& path : paths) {
        ...
        ctx.write_output(std::format("file|{}|{}|{}",
            escape_pipes(path), result.exists ? "true" : "false",
            escape_pipes(detail)));
    }
}

void check_ports(yuzu::CommandContext& ctx,
    ...
    for (const auto& port_str : ports) {
        ...
        auto [ptr, ec] = std::from_chars(port_str.data(),
                                         ...
                                         target_port);
        if (ec != std::errc{}) {
            ctx.write_output(std::format("port|{}|false|Invalid port number",
                escape_pipes(port_str)));
            continue;
        }
        ...
        if (!found) detail = "No listening service on this port";

        ctx.write_output(std::format("port|{}|{}|{}",
            escape_pipes(port_str), found ? "true" : "false",
            escape_pipes(detail)));
    }
}
...
public:
    ...
    int execute(yuzu::CommandContext& ctx,
        ...
        if (ip_list.empty() && domain_list.empty() && hash_list.empty() &&
            path_list.empty() && port_list.empty()) {
