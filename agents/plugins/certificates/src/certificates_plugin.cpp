...
struct CertRecord {
    ...
    std::string to_row() const {
        return std::format("{}|{}|{}|{}|{}|{}|{}|{}",
                           subject, issuer, thumbprint, not_before,
...
bool expires_within_days(const std::string& not_after, int days) {
    ...
    if (not_after.size() < 10) return true;  // can't parse, include it
    ...
    if (expiry == static_cast<std::time_t>(-1)) return true;
...
std::string get_cert_name(PCCERT_CONTEXT cert, DWORD type) {
    char buf[512]{};
    DWORD len = CertGetNameStringA(cert, CERT_NAME_SIMPLE_DISPLAY_TYPE,
                                   type, nullptr, buf, sizeof(buf));
...
std::string get_key_usage(PCCERT_CONTEXT cert) {
    ...
    if (!CertGetIntendedKeyUsage(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING,
                                 cert->pCertInfo, usage_bits, usage_size)) {
        return "(none)";
    }
...
void list_certs_win(yuzu::CommandContext& ctx, std::string_view store_filter,
                    int expiring_days) {
    ...
    for (const auto* store_name : kStores) {
        if (store_filter != "all" && store_filter != store_name) continue;
...
CertRecord parse_openssl_output(const std::string& pem_path,
    ...
    auto subj = run_command(
        std::format("openssl x509 -noout -subject -in \"{}\" 2>/dev/null",
                     pem_path).c_str());
    ...
    auto iss = run_command(
        std::format("openssl x509 -noout -issuer -in \"{}\" 2>/dev/null",
                     pem_path).c_str());
    ...
    auto dates = run_command(
        std::format("openssl x509 -noout -startdate -enddate -in \"{}\" 2>/dev/null",
                     pem_path).c_str());
    ...
    while (std::getline(dss, line)) {
        if (line.starts_with("notBefore=")) {
            ...
            auto date_cmd = run_command(
                std::format("openssl x509 -noout -startdate -dateopt iso_8601 "
                            "-in \"{}\" 2>/dev/null", pem_path).c_str());
            ...
            auto date_cmd = run_command(
                std::format("openssl x509 -noout -enddate -dateopt iso_8601 "
                            "-in \"{}\" 2>/dev/null", pem_path).c_str());
    ...
    auto serial = run_command(
        std::format("openssl x509 -noout -serial -in \"{}\" 2>/dev/null",
                     pem_path).c_str());
    ...
    auto fp = run_command(
        std::format("openssl x509 -noout -fingerprint -sha1 -in \"{}\" 2>/dev/null",
                     pem_path).c_str());
    ...
    auto usage = run_command(
        std::format("openssl x509 -noout -ext keyUsage -in \"{}\" 2>/dev/null",
                     pem_path).c_str());
    if (!usage.empty()) {
...
CertRecord parse_pem_block_macos(const std::string& pem_block,
                                 const std::string& store_name) {
    CertRecord rec;
    rec.store = store_name;
    ...
    auto base_cmd = std::format(
        "echo '{}' | openssl x509 -noout", pem_block);

    // Subject
    auto subj = run_command(
        std::format("{} -subject 2>/dev/null", base_cmd).c_str());
    if (subj.starts_with("subject=")) {
        rec.subject = subj.substr(8);
        while (!rec.subject.empty() && rec.subject.front() == ' ')
            rec.subject.erase(rec.subject.begin());
    ...
    // Issuer
    auto iss = run_command(
        std::format("{} -issuer 2>/dev/null", base_cmd).c_str());
    if (iss.starts_with("issuer=")) {
        rec.issuer = iss.substr(7);
        while (!rec.issuer.empty() && rec.issuer.front() == ' ')
            rec.issuer.erase(rec.issuer.begin());
    ...
    // Dates (ISO 8601)
    auto start_date = run_command(
        std::format("{} -startdate -dateopt iso_8601 2>/dev/null",
                     base_cmd).c_str());
    ...
    auto end_date = run_command(
        std::format("{} -enddate -dateopt iso_8601 2>/dev/null",
                     base_cmd).c_str());
    ...
    // Serial
    auto serial = run_command(
        std::format("{} -serial 2>/dev/null", base_cmd).c_str());
    if (serial.starts_with("serial=")) {
        rec.serial = serial.substr(7);
    } else {
        rec.serial = "(unknown)";
    ...
    auto fp = run_command(
        std::format("{} -fingerprint -sha1 2>/dev/null", base_cmd).c_str());
    auto eq = fp.find('=');
    if (eq != std::string::npos) {
        auto hex = fp.substr(eq + 1);
        std::string clean;
    ...
    // Key usage
    auto usage = run_command(
        std::format("{} -ext keyUsage 2>/dev/null", base_cmd).c_str());
    if (!usage.empty()) {
        std::istringstream uss(usage);
        std::string uline;
        std::string last_line;
...
void details_cert_macos(yuzu::CommandContext& ctx, std::string_view thumbprint) {
    ...
    if (check(sys_pem, "System.keychain")) return;
    if (check(root_pem, "SystemRootCertificates.keychain")) return;
...
void delete_cert_macos(yuzu::CommandContext& ctx, std::string_view thumbprint,
    ...
    auto result = run_command(
        std::format("security delete-certificate -Z {} "
