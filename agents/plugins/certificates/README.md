...
| Linux | dedicated unprivileged `yuzu` account in production; this sample was captured as euid 0 in a container, not representative of that account | `list`/`details`: none — native filesystem read + in-process libcrypto parse, no subprocess. `delete`: write access to `/etc/ssl/certs` to remove the matching file via `unlinkat` on the held `/etc/ssl/certs` dirfd (`certificates_plugin.cpp:1009`). | 2026-09-06, Debian GNU/Linux 13 (trixie) aarch64 container, euid 0 | An unreadable/unparseable PEM file reports `(unreadable)`/`(skipped)` and `CONSTRAINED`/`PARTIAL` rather than being silently dropped (`certificates_plugin.cpp:779-783`); `details` reports `not_available\|... scan incomplete` rather than a false `status\|not_found` when a candidate file couldn't be inspected (`certificates_plugin.cpp:899`); an unreadable `/etc/ssl/certs` itself, or an enumeration that fails part-way, reports `not_available\|/etc/ssl/certs could not be opened` and CONSTRAINED/PARTIAL, provenance `posix:cert-dir` (`certificates_plugin.cpp:797-799,816-817`); a `delete` whose target's identity (link inode, target text, and target inode) changed between match and unlink reports `error\|certificate file changed during delete; nothing removed`, provenance `posix:delete-recheck` (`certificates_plugin.cpp:1019-1022`), instead of unlinking a different file than the one matched |
...
|---|---|---|---|
| `CONSTRAINED` | `PARTIAL` | `libcrypto:unreadable-file` | Linux: a PEM file under `/etc/ssl/certs` could not be read or parsed (`certificates_plugin.cpp:779,971`) |
| `CONSTRAINED` | `PARTIAL` | `cryptoapi:store-open` | Windows: a CryptoAPI store could not be opened for `list`/`details`/`delete` (`certificates_plugin.cpp:371,402,452`) |
| `CONSTRAINED` | `PARTIAL` | `posix:cert-dir` | Linux: `/etc/ssl/certs` could not be opened, or a readdir(3) enumeration failed part-way through (`certificates_plugin.cpp:799,817,831,945`) |
| `CONSTRAINED` | `PARTIAL` | `posix:delete-recheck` | Linux: `delete`'s pre-unlink identity recheck found the entry changed or could not be re-derived since the earlier match (`certificates_plugin.cpp:1021,1028`) |
...
1. **Windows store-open failures are no longer silent, and the Linux delete recheck still has a narrow residual window.** A CryptoAPI store that neither `CertOpenStore` attempt can open now reports `not_available|<store> store could not be opened` (`list`/`details`) or `error|<store> store could not be opened; nothing removed` with a non-zero exit (`delete`), and marks the result CONSTRAINED/PARTIAL via `cryptoapi:store-open` — the Windows leg does call `set_result_status` for this case (`certificates_plugin.cpp:369-371,402,449-453`). On Linux, `delete` re-derives the matched entry's identity — its own device/inode and, for a symlink, the target's link text plus the target's device/inode — immediately before `unlinkat`, but POSIX has no unlink-by-fd, so a residual `fstatat`-then-`unlinkat` window remains (`certificates_plugin.cpp:980-1010`); because identity binds inode and target text/inode rather than content, an in-place rewrite of the SAME inode between match and unlink is not detected. A FIFO, device, socket or directory encountered during the scan is skipped silently via a nonblocking, post-open `S_ISREG` check rather than blocking the scan (`certificates_plugin.cpp:634,685-689`), and deleting a symlinked entry always removes the link itself via `unlinkat`, never the file it points to.
...
- Tests: `tests/unit/test_certificates_macos.cpp` · `tests/unit/test_certificates_store_honesty.cpp` · `tests/unit/test_certificates_x509.cpp`
- Privilege row: `docs/agent-privilege-model.md`
- Changelog: `changelog.d/20260817-wave2-certificates-in-process-x509.changed.md` · `changelog.d/20260914-certificates-store-honesty-sentinels.changed.md` · `changelog.d/3245-certificates-linux-delete-toctou.fixed.md` · `changelog.d/3247-certificates-windows-thumbprint-case.fixed.md` · `changelog.d/3406-certificates-login-keychain-argv.changed.md`
<!-- END GENERATED -->
