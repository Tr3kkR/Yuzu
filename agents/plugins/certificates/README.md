...
|---|---|---|---|---|
| Windows | agent service account (LocalSystem today, #1442) | `list`/`details`: none — `CertEnumCertificatesInStore` needs no elevation. `delete`: `SeBackupPrivilege` + `SeRestorePrivilege` to write the certificate store; already granted to the service account under LocalSystem today, no new grant. | 2026-09-07, bare-metal Windows 10.0.26200.0 x64, as SYSTEM | `enumerate_store()` returns a typed result that records whether the store actually opened under LocalMachine or fell back to CurrentUser, never a silent empty vector (`certificates_plugin.cpp:337-408`); `list` reports `not_available\|<store> store could not be opened` (provenance `cryptoapi:store-open`, `certificates_plugin.cpp:420-428`) or `not_available\|<store> store enumeration incomplete` when `CertEnumCertificatesInStore` ends abnormally part-way through a store (provenance `cryptoapi:store-enum`, `certificates_plugin.cpp:430-435`), then keeps scanning the remaining stores; when LocalMachine could not be opened but CurrentUser could, `list` emits a `not_available\|<store> store (LocalMachine) could not be opened; rows read from CurrentUser` row before that store's rows (provenance `cryptoapi:store-fallback`, `certificates_plugin.cpp:437-449`); `details` marks the result partial per store internally and, unlike `list`, does not emit a row for a plain open/enumeration failure at the point it happens — it keeps scanning, and only if a match is never found does it report one combined `not_available\|<store(s)> could not be opened/enumeration incomplete/read from CurrentUser; scan incomplete` row naming every affected store, instead of a false `status\|not_found`; a CurrentUser fallback is the one exception to "no row until the end" — `details_cert_win` discloses each fallback-served store's row-level substitution immediately, either right before a certificate row it serves a match from, or (if an earlier fallback store found no match before a later store did) as its own row alongside that later match — see the Result-status table's `cryptoapi:store-fallback` entry below for the exact shape (`details_cert_win`); `delete` opens the named store under LocalMachine only, with no CurrentUser retry (#4377) — a store that cannot be opened there reports `error\|<store> store could not be opened; nothing removed` (`certificates_plugin.cpp:552-562`), and a store whose enumeration ends abnormally reports `error\|<store> store enumeration ended abnormally; nothing removed` (`certificates_plugin.cpp:592-600`), both with a non-zero exit instead of `status\|not_found` |
| macOS | agent daemon; the shipped LaunchDaemon has no `UserName` key, so it runs as root — but this sample was captured unprivileged (see Measured) | System/root keychain reads: none (in-process `SecItemCopyMatching`, no subprocess). Login keychain read: root, via `launchctl asuser` + `sudo -u` — live today only because the daemon runs as root; needs a dormant sudoers grant once #1455 narrows the agent off root. `delete` (System/MY only): a sudoers grant for `/usr/bin/security delete-certificate -t /Library/Keychains/System.keychain *` under the future non-root account; moot today. | 2026-09-07, bare-metal macOS 26.6.2 arm64, euid 501 (jsmith) — unprivileged, captured via the test harness (`PluginHandle::load` + `LocalDispatcher::run`, no `init()`), not the production root LaunchDaemon | System/root keychain read failure emits `not_available\|<keychain> not readable (no read permission)`, `not_available\|<keychain> read timed out` (15 s per-keychain bound exceeded), `not_available\|<keychain> read refused (bounded-call ceiling)`, `not_available\|<keychain> read failed`, or `not_available\|<keychain> scan incomplete` (the read was capped before it finished) — each + `CONSTRAINED`/`PARTIAL` (`certificates_plugin.cpp:1463-1486`); an in-process `/dev/console` owner recheck immediately before the login-keychain spawn emits `not_available\|console user changed` (`certificates_plugin.cpp:1523`) or `not_available\|console user recheck failed` (`certificates_plugin.cpp:1527`); a failed login-keychain spawn emits `not_available\|login keychain read failed (<detail>)` naming the termination reason, on both `list` (`certificates_plugin.cpp:1581`) and `details` (`certificates_plugin.cpp:1831`) — the macOS sample shows a login-keychain degradation (`macos.txt:164-165`) |
| Linux | dedicated unprivileged `yuzu` account in production; this sample was captured as euid 0 in a container, not representative of that account | `list`/`details`: none — native filesystem read + in-process libcrypto parse, no subprocess. `delete`: write access to `/etc/ssl/certs` to remove the matching file via `unlinkat` on the held `/etc/ssl/certs` dirfd (`certificates_linux_store.hpp:418`). | 2026-09-06, Debian GNU/Linux 13 (trixie) aarch64 container, euid 0 | An unreadable/unparseable PEM file reports `(unreadable)`/`(skipped)` and `CONSTRAINED`/`PARTIAL` rather than being silently dropped, provenance `libcrypto:unreadable-file` (`certificates_plugin.cpp:691-696`); `details` reports `not_available\|... scan incomplete` rather than a false `status\|not_found` when a candidate file couldn't be inspected (`certificates_plugin.cpp:805-811`); `/etc/ssl/certs` could not be opened (including a non-directory or symlink at that path) reports `not_available\|/etc/ssl/certs could not be opened` and CONSTRAINED/PARTIAL, provenance `posix:cert-dir` (`certificates_plugin.cpp:855-857`); a readdir(3)/fdopendir enumeration of `/etc/ssl/certs` that fails part-way through reports `not_available\|/etc/ssl/certs enumeration incomplete` (`list`) or folds into `delete`'s `error\|/etc/ssl/certs enumeration incomplete; nothing removed`, provenance `posix:cert-enum` (`certificates_plugin.cpp:727-729,894-897`); a `delete` whose target's identity (link inode, target text, and target inode) changed between match and unlink reports `error\|certificate file changed during delete; nothing removed`, provenance `posix:delete-recheck` (`certificates_linux_store.hpp:389-431`; `certificates_plugin.cpp:883,890`), instead of unlinking a different file than the one matched |

Binaries/subprocesses/network: Windows — none, in-process CryptoAPI/`crypt32` only. macOS — `/usr/bin/openssl` (per-block PEM parse, `certificates_plugin.cpp:1164-1167`), `/usr/bin/security` (`find-certificate` for the login keychain, built by `build_login_keychain_read_argv` in `agents/shared/macos_console_user.hpp`; `delete-certificate`, `certificates_plugin.cpp:2063-2066`), `/usr/bin/stat` (console-user resolution, `certificates_plugin.cpp:1292`) — all via the bounded subprocess runner, pre-split argv, no shell. Linux — none, in-process libcrypto only (`certificates_plugin.cpp:82-87`). No network access on any OS.
...
|---|---|---|---|
| `CONSTRAINED` | `PARTIAL` | `libcrypto:unreadable-file` | Linux: a PEM file under `/etc/ssl/certs` could not be read or parsed (`certificates_plugin.cpp:691,867`) |
| `CONSTRAINED` | `PARTIAL` | `cryptoapi:store-open` | Windows: a CryptoAPI store could not be opened for `list`/`details`/`delete` (`certificates_plugin.cpp:427,485,561`) |
| `CONSTRAINED` | `PARTIAL` | `cryptoapi:store-enum` | Windows: `CertEnumCertificatesInStore` ended abnormally part-way through a store for `list`/`details`/`delete` (`certificates_plugin.cpp:434,491,597`) |
| `CONSTRAINED` | `PARTIAL` | `cryptoapi:store-fallback` | Windows: the LocalMachine store could not be opened and `list`/`details` read the CurrentUser store instead; `list` discloses it with a `not_available` row before that store's rows; `details` discloses every such store the same way — either as its own row immediately before a match served from it, or, if an earlier fallback store didn't match, as a combined row emitted alongside whichever later store's match ends the scan — and folds every fallback store into the combined scan-incomplete verdict only when no store ever matches (`certificates_plugin.cpp:448,501-533`) |
| `CONSTRAINED` | `PARTIAL` | `posix:cert-dir` | Linux: `/etc/ssl/certs` could not be opened (including a non-directory or symlink at that path) (`certificates_plugin.cpp:711,743,857`) |
| `CONSTRAINED` | `PARTIAL` | `posix:cert-enum` | Linux: a readdir(3)/fdopendir enumeration of `/etc/ssl/certs` failed part-way through (`certificates_plugin.cpp:729,794,896`) |
| `CONSTRAINED` | `PARTIAL` | `posix:delete-recheck` | Linux: `delete`'s pre-unlink identity recheck found the entry changed or could not be re-derived since the earlier match (`certificates_plugin.cpp:883,890`) |
| `CONSTRAINED` | `PARTIAL` | `login-keychain` | macOS: console-user resolution failed, the console user changed or its owner recheck failed immediately before the login-keychain spawn, or the login-keychain read/scan failed, degraded, or was capped (`certificates_plugin.cpp:1523,1527,1568-1583`) |
| `CONSTRAINED` | `PARTIAL` | `secitem:System.keychain` | macOS: System.keychain was not readable (no read permission), timed out, was refused at the bounded-call ceiling, failed to open, or its scan was capped (`certificates_plugin.cpp:1463-1486`) |
| `CONSTRAINED` | `PARTIAL` | `secitem:SystemRootCertificates.keychain` | macOS: SystemRootCertificates.keychain was not readable (no read permission), timed out, was refused at the bounded-call ceiling, failed to open, or its scan was capped (`certificates_plugin.cpp:1463-1486`) |

A clean run on any OS — one that hits none of the degradations above — leaves the agent's default `UNDECLARED`/`UNKNOWN` status; the table rows above are the only places the plugin calls `set_result_status` — the sample captures show this (`windows.txt:84,89,93`; `linux.txt:155,160,164`; `macos.txt:170,174`).
...
1. **Windows store-open failures are no longer silent, `delete` on Windows never substitutes the CurrentUser store for an unopenable LocalMachine store (#4377), the Linux delete recheck still has a narrow residual window, and a macOS System/root keychain's empty answer is only trusted when the keychain is confirmed readable.** A CryptoAPI store that neither `CertOpenStore` attempt can open now reports `not_available|<store> store could not be opened` (`list`/`details`) or `error|<store> store could not be opened; nothing removed` with a non-zero exit (`delete`), and marks the result CONSTRAINED/PARTIAL via `cryptoapi:store-open` — the Windows leg does call `set_result_status` for this case (`certificates_plugin.cpp:420-428,485,552-562`); `delete` opens the named store under LocalMachine only and has no CurrentUser retry path at all (`certificates_plugin.cpp:546-555`), unlike `list`/`details`, which disclose a CurrentUser fallback rather than presenting it as a clean LocalMachine read. On Linux, the held-dirfd scan/delete core now lives in `certificates_linux_store.hpp`, with syscall-level tests in `tests/unit/test_certificates_linux_store.cpp` covering symlink retarget, inode swap, a failed recheck, and mid-enumeration failure; `delete` re-derives the matched entry's identity — its own device/inode and, for a symlink, the target's link text plus the target's device/inode — immediately before `unlinkat`, but POSIX has no unlink-by-fd, so a residual `fstatat`-then-`unlinkat` window remains (`certificates_linux_store.hpp:342-358,389-431`); because identity binds inode and target text/inode rather than content, an in-place rewrite of the SAME inode between match and unlink is not detected. A FIFO, device, socket or directory encountered during the scan is skipped silently via a nonblocking, post-open `S_ISREG` check rather than blocking the scan (`certificates_linux_store.hpp:232-252`), and deleting a symlinked entry always removes the link itself via `unlinkat`, never the file it points to. On macOS, a System/root keychain whose `SecItemCopyMatching` answer comes back empty is classified `zero certificates` only when the keychain's own read-permission status bit is set (`keychain_read.cpp:116`); otherwise it is reported as `not_available|<keychain> not readable (no read permission)` (`certificates_plugin.cpp:1463-1486`) and the result is CONSTRAINED/PARTIAL — an unreadable keychain is never a silent empty inventory (#2318).
2. **The login keychain read is a registered exception, not a rung-1 promotion.** System.keychain/SystemRootCertificates.keychain are read in-process via `SecItemCopyMatching`, bounded to 15 seconds per keychain via agent-core's `read_keychain_bounded` and abandoned (not cancelled) on timeout (`certificates_plugin.cpp:966-975`); the login keychain stays on the `launchctl asuser` + `sudo -u` + `security find-certificate` subprocess (rung 2, #3406) because the LaunchDaemon has no login keychain of its own (`certificates_plugin.cpp:16-23`) — do not "fix" this to a direct API call. Immediately before that subprocess spawn the plugin re-reads `/dev/console`'s owner in-process and refuses to spawn on a mismatch, shrinking (not eliminating) the fast-user-switch race window from tens of seconds to microseconds (`certificates_plugin.cpp:1508-1528`).
3. **SystemRootCertificates.keychain delete is permanently rejected.** It is sealed under System Integrity Protection; `delete_cert_macos` rejects `store=root` outright before ever shelling out (`certificates_plugin.cpp:1972-1982`) — a hard OS limitation, not a bug to fix.
4. **A macOS delete self-verifies before reporting success.** `delete_cert_macos` re-enumerates the target keychain after `security delete-certificate` exits 0 and only reports `status|deleted` on a positively-proven absence; a keychain that can't be re-read or still shows the certificate reports `error|...`, never a false `status|deleted` (`certificates_plugin.cpp:2076-2113`).
...
- Tests: `tests/unit/test_certificates_linux_store.cpp` · `tests/unit/test_certificates_macos.cpp` · `tests/unit/test_certificates_store_honesty.cpp` · `tests/unit/test_certificates_x509.cpp`
- Privilege row: `docs/agent-privilege-model.md`
- Changelog: `changelog.d/20260817-wave2-certificates-in-process-x509.changed.md` · `changelog.d/20260914-certificates-store-honesty-sentinels.changed.md` · `changelog.d/2318-certificates-keychain-honesty.fixed.md` · `changelog.d/3245-certificates-linux-delete-toctou.fixed.md` · `changelog.d/3246-certificates-bounded-keychain-read.fixed.md` · `changelog.d/3247-certificates-windows-thumbprint-case.fixed.md` · `changelog.d/3406-certificates-login-keychain-argv.changed.md` · `changelog.d/4377-certificates-windows-delete-fail-closed.fixed.md` · `changelog.d/4446-certificates-linux-windows-followups.fixed.md`
<!-- END GENERATED -->
