...
Binaries/subprocesses/network: Windows — none, in-process CryptoAPI/`crypt32` only. macOS — `/usr/bin/openssl` (per-block PEM parse, `certificates_plugin.cpp:897-900`), `/usr/bin/security` (`find-certificate`/`delete-certificate`, `certificates_plugin.cpp:1649,1790`), `/usr/bin/stat` (console-user resolution, `certificates_plugin.cpp:1025`) — all via the bounded subprocess runner, pre-split argv, no shell. Linux — none, in-process libcrypto only (`certificates_plugin.cpp:82-87`). No network access on any OS.
...
- Changelog: `changelog.d/20260817-wave2-certificates-in-process-x509.changed.md` · `changelog.d/2318-certificates-keychain-honesty.fixed.md` · `changelog.d/3246-certificates-bounded-keychain-read.fixed.md` · `changelog.d/3406-certificates-login-keychain-argv.changed.md`
<!-- END GENERATED -->
