...
| `CONSTRAINED` | `PARTIAL` | `cryptoapi:store-fallback` | Windows: the LocalMachine store could not be opened and `list`/`details` read the CurrentUser store instead; `list` discloses it with a `not_available` row before that store's rows, `details` folds it into the scan-incomplete verdict (`certificates_plugin.cpp:448,496`) |
