...
| `CONSTRAINED` | `PARTIAL` | `<key>:errno_<n>`, `<key>:efivars:shape`, `<key>:timeout`, `<key>:output_truncated`, `<key>:spawn_failed`, `<key>:exit_<n>`, `<key>:exit_signal`, `<name>:win32_<n>`, `<name>:type_<n>`, `<name>:size_<n>`, `<name>:oversized`, `ci_policy:enumeration_incomplete` | At least one row was `unreadable` and none was refused; one `<key>:<cause>` token per unreadable row, and `<key>:efivars:shape` is an efivar that is not 4 + 1 bytes. An `absent` row never appears here. |
