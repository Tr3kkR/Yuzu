...
2. **Configuration, not evaluation — and on Windows `audit_policy`, not the effective audit state.** Linux and macOS report what the files state, not what PAM or sudo would decide: `pwquality.conf.d` and `faillock` drop-ins are not read, PAM `@include` lines and `include`/`substack` controls are not followed, `sudoers` include directives are listed and not followed, `audit.rules` counts are of the file (not `/etc/audit/rules.d`) and not the live kernel rules, and a `pwpolicy` expression is reported verbatim (only `minimum_length` is derived from it). The sharpest case is Windows `audit_policy`: `secedit /export` returns the nine **legacy** `[Event Audit]` categories, and where Advanced Audit Policy subcategories are in force — the Windows 10/11 default, and the norm under GPO — those are not what the host actually audits. A category reading `none` means the legacy category is unset, **not** that the host is not auditing; the committed rig capture is exactly that shape (a default Windows 11 Pro, all nine `none`). `auditpol /get /category:*` is the effective state and is not read — that would be a second argv leaf with its own sink-manifest row. **Silent by design, not failures.** One absent PAM file while a sibling alternative exists; a file read successfully that sets none of the reported keys; a PAM line that is not `type control module`; `login.defs` keys outside the per-action allow-list; `pwpolicy` policy keys other than `policyIdentifier`/`policyContent`/`policyParameters` (e.g. the localised `policyContentDescription`); and a Windows `[Event Audit]` category the export omits. Each simply produces no row and no token, so for these cases the output cannot distinguish "not set" from "not read".
3. **The Windows leg is measured on real hardware — one standalone host, as LocalSystem only. It also leaves a file on disk for the life of a dispatch, and longer after a crash.** It is rung 2 by design; a rig session ran the real `secedit /export` as LocalSystem from an elevated scheduled task and it succeeded with `SeSecurityPrivilege` present but Disabled (sample and the `RIG PROBE` banner in `local_security_policy_win.cpp`). That shows the export works as LocalSystem; it does **not** show which privilege is required or that the dedicated `NT SERVICE\YuzuAgent` account succeeds, and how a refused run reports (expected `secedit:exit_<n>`) is unmeasured. That host was **not domain-joined**: on a domain-joined member the export is the local security database *after* GPO application, and domain-account password policy is not reported at all. The staged export under `agent.data_dir` holds the whole `SECURITYPOLICY` area, not only the reported keys. It is removed on return; after a crash or service stop it stays until a later Windows dispatch finds it more than an hour old, indefinitely if none follows. `secedit` may also append to `%windir%\security\logs\scesrv.log` (Microsoft's documented default; unmeasured).
4. **Privileged files read as `unreadable` for an unprivileged agent, and in a container the Linux leg reports the container, not the host.** `/etc/sudoers` (`0440`), `/etc/sudoers.d/*` and Linux `/etc/audit/audit.rules` (`0640`) need root or a read-override capability; without it the source is an `unreadable` row and the action reports `PERMISSION_DENIED` (or `CONSTRAINED` if another source was readable), never an empty list. macOS `audit_control` is absent by default and reads `absent`. In a container, the Linux leg reads the `/etc` its process sees. The shipped `deploy/docker/Dockerfile.agent` image runs as the unprivileged `yuzu-agent` user and mounts no host `/etc`, so `login.defs`, PAM, `audit.rules` and `sudoers` are the image's own (debian:trixie-slim installs neither `sudo` nor `auditd`, so `sudoers` and `audit_policy` read `absent`) — however the container is networked. The committed Linux sample is such a container run. Reading a bind-mounted host `/etc` has not been tested.
