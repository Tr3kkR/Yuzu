...
|---|---|---|---|---|
| Windows | agent service account (LocalSystem today, #1442) | `secedit /export` needs an elevated token. The agent already holds `SeSecurityPrivilege` (`scripts/install-agent-user.ps1`, granted for the Security event log); whether that privilege alone suffices is **not yet measured**: the probe is the `RIG PROBE` banner in `local_security_policy_win.cpp`, still marked pending, and no `docs/samples/windows.txt` exists yet | Not yet: no Windows capture and no real `secedit` export fixture is committed, so the Windows leg's real-host behaviour is unverified | `ERROR_ACCESS_DENIED` reading the export reports `PERMISSION_DENIED` with `secedit:access_denied`; a non-zero `secedit` exit reports `CONSTRAINED` with `secedit:exit_<n>`; the action returns 1 with the row `constrained\|<token>` |
...
3. **The Windows leg is unmeasured on real hardware at this writing.** It is rung 2 by design, and its fixtures, sample and the `SeSecurityPrivilege` sufficiency probe are pending the Windows rig session; until they land, the Windows rows and the Windows privilege cell are the code's declared behaviour, not a captured one.
...
- Tests: `tests/unit/test_local_security_policy_local_dispatcher.cpp` · `tests/unit/test_local_security_policy_parsers.cpp` · `tests/unit/test_local_security_policy_scratch_sweep.cpp`
