...
| `UNAVAILABLE` | full | `windows:winspool:printer_not_found` (plain local names) — `linux:cups:not_found` / `macos:cups:not_found` | `clear_queue`: the target printer or job no longer exists — Windows `OpenPrinterW` (`ERROR_INVALID_PRINTER_NAME`) for a plain local printer name (no path separator or colon); macOS/Linux cupsd's own `client-error-not-found` (IPP `0x0406`) response |
