...
|---|---|---|---|---|
| Windows | agent service account | None — `EnumPrintersW`/`EnumJobsW` need no elevated right | the-rig, `SYSTEM`, 2026-09-08 (`docs/samples/windows.txt`) | n/a — reads do not fail closed; a transport/decode failure reports `CONSTRAINED`, never a permission error |
| macOS | agent daemon, dedicated unprivileged account by design (`docs/agent-privilege-model.md:12`) | None to read | this Mac, euid 501, 2026-09-14 (`docs/samples/macos.txt`) | n/a — reads do not fail closed |
| Linux | agent daemon, dedicated unprivileged account (`yuzu`), never root by design (`docs/agent-privilege-model.md:12`) | None to read | this Mac, euid 501, 2026-09-14 (`docs/samples/linux.txt`) | n/a — reads do not fail closed |
