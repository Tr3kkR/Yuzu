...
| `field_3` | string | - | all | `Default` | status row: the constraint reason, "-" when supported. profile row: the profile directory name (the Local State info_cache key), e.g. "Default", "Profile 1" — redacted whole to "[redacted-email]" when the raw value contains an e-mail-shaped substring (bare, decorated or several addresses, not only a bare address — some Chromium builds key a signed-in profile by its account e-mail). |
