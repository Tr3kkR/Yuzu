...
**PRIVACY CONTRACT.** No row this plugin emits, on any action or any OS leg, ever carries a browsing-account identifier, browsing history, cookies or bookmarks — no `gaia_id`, no e-mail address, no Chromium `info_cache` `user_name`/`gaia_name`. This is enforced structurally in `browser_inventory_parsers.hpp`: `BrowserProfileRow` — the JSON row model every OS leg builds from — simply has no such field, so no code path in any parser could put one on the wire. Two deliberate, accepted exceptions, both decided 2026-09-22: **(1)** the `profiles` row's LOCAL OS/home-directory username (e.g. `alice`) — it identifies which of several local users on a shared machine a profile belongs to, is machine-local (not a personal/cloud identifier), and is added only when the Linux leg formats the wire row, never inside the JSON parser, never a `BrowserProfileRow` field. **(2)** `display_name` (`info_cache[dir].name`, a `BrowserProfileRow` field) — Chromium-family browsers commonly auto-populate a profile's display name from the signed-in account's real name, so **this field can legitimately carry a personal name**; it is emitted as-is (it is the whole point of a profile inventory — telling "Work" apart from "Personal") rather than filtered, because a reliable real-name/PII detector does not exist and a false sense of redaction is worse than an honest, documented residual risk. No leg opens any file inside a profile directory (`Preferences`, `Secure Preferences`, `History`, `Cookies`, ...) — see caveat 3.
