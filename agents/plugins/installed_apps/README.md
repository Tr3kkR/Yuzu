...
**Parsing `list` rows.** `app|name|version|publisher|install_date|install_location|bundle_id` — seven escape-aware tokens. Split on `|` not preceded by `\`, then decode `\|` to `|`; `\` never otherwise appears (folded to `/`, which makes a UNC path `//srv/share` and is not reversible), CR/LF are folded to a space, and any field over 4 KiB is cut. `-` means empty or not applicable. Agents on plugin < 1.2.0 emit five tokens: treat a missing sixth or seventh as `-`. REST and MCP return this text verbatim; only the dashboard decodes `\|`, inside its merged cell.
