...
5. **`amcache` reports `constrained|dest_dir_acl` when its scratch directory pre-exists and fails an owner/DACL/reparse-point safety check.** No legitimate install scenario pre-creates this directory (`install-agent-user.ps1` provisions nothing under it), so this should never fire on a clean deployment — if it does, remove the offending directory (`<agent.data_dir>\execution_artifacts`, or `C:\ProgramData\yuzu\agent\execution_artifacts` when `agent.data_dir` is unset) and re-dispatch; the plugin will recreate it correctly.
