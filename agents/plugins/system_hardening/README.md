...
5. **Containers read the host kernel's values.** The eleven Linux keys are kernel-global, not namespaced, so an agent in a container (including the shipped `deploy/docker/Dockerfile.agent` image) reports the HOST kernel's settings, not anything the container configured. A runtime that masks or does not mount `/proc/sys` makes every key read `unreadable` (`<key>:errno_19`), not `absent`. The Linux sample above was captured in a container (Docker VM on this Mac), so it shows that VM's kernel.
6. **Fixed allowlists, no discovery.** The plugin reads only the keys listed above; it does not walk `/proc/sys` or the registry, and it does not report Linux LSM state (SELinux, AppArmor) or macOS SIP and Gatekeeper posture.
