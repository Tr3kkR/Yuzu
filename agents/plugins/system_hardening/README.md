...
- **Linux.** An `ENOENT` is `absent` only when `statfs` confirms the key's nearest existing directory up to `/proc/sys` is a mounted procfs (`surface_is_procfs`, in the pure header). Otherwise `remap_enoent_for_surface` turns it into `ENODEV` and the key reads `unreadable` with `<key>:errno_19`. Measured 2026-09-23 in a container: a tmpfs over `/proc/sys`, and a `subset=pid` procfs, each gave eleven `errno_19` rows and `CONSTRAINED`/`PARTIAL`; a tmpfs over `/proc/sys/kernel` made only the six `kernel.*` keys `unreadable`. A leaf that opens but is not a regular file (a FIFO, device or directory mounted at the path) is never read and reports `unreadable` with `<key>:not_regular`.
