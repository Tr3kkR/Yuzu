...
| `CONSTRAINED` | `PARTIAL` | `<key>:errno_<n>`, `<name>:win32_<n>`, `<name>:type_<n>`, `<name>:oversized`, `proc_sys:not_visible` | At least one key was `unreadable` and none was refused; the status reason lists one `<key>:<cause>` token per unreadable key, and the matching row reads `unreadable`. An `absent` key never appears here. Windows decoder failures (`empty_blob`, `odd_length`, `not_qword_aligned`, `oversized`, `no_hex_digits`, `odd_hex_digits`, `bad_hex`) appear as `<name>:<token>`. |
