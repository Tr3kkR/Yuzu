...
3. **`absent` does not downgrade the run, and is only reported where the surface is confirmed present.** A missing optional key (no Yama, no `MitigationOptions` on a default Windows install) adds no token and the run stays `OK`; a consumer that needs to know reads the `absent` row. A hidden Linux surface (no procfs at the key's nearest existing directory: `ProcSubset=pid`, a tmpfs overmount, a masked container `/proc/sys`) or a missing Windows `Session Manager\kernel` key reads `unreadable` instead, and all eleven Linux keys absent on a confirmed procfs adds a backstop `proc_sys:not_visible` token.
4. **Windows: the self rows are the agent process only, and only part of the mitigation layout is hardware-confirmed.** The `self.*` rows come from `GetProcessMitigationPolicy` on the agent's own process. The system-wide policy is the `mitigation.*` registry rows. Five of the sixteen nibble positions (`dep`, `sehop`, `aslr_bottom_up`, `aslr_high_entropy`, `cfg`) and only nibble value `1` were confirmed on the-rig (2026-09-21); the other positions, values `2`-`15`, trailing `ext_q<n>` QWORDs and every `MitigationAuditOptions` row follow the documented structure unmeasured.
5. **Fixed allowlists, no discovery; a container reads the host kernel.** The plugin reads only the keys listed above and does not report Linux LSM state (SELinux, AppArmor) or macOS SIP and Gatekeeper posture. The Linux keys are kernel-global, so an agent in a container (including the shipped `deploy/docker/Dockerfile.agent` image) reports the host kernel's settings; the Linux sample above was captured in one (measured 2026-09-23: a tmpfs over `/proc/sys`, and a `subset=pid` procfs, each gave eleven `unreadable` `errno_19` rows and `CONSTRAINED`/`PARTIAL`; a tmpfs over `/proc/sys/kernel` made only the six `kernel.*` keys `unreadable`).
