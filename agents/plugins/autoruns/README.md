...
5. **Windows per-user Startup-folder redirect resolution is wrong for the common case, not just a redirected one.** `HKCU\...\Explorer\User Shell Folders\Startup` is populated with a `REG_EXPAND_SZ` value on essentially every Windows profile at creation — redirected or not. When present, `%USERPROFILE%`-style tokens in that value are expanded (`autoruns_win.cpp`, `expand_env_strings`) against the *agent process's own* environment, not the enumerated profile's — and this agent runs as LocalSystem. In practice this usually resolves to a path that doesn't exist on disk (LocalSystem's own Startup folder is normally empty), so the typical observable effect is the profile's real Startup entries going completely unreported, not a visibly-wrong-but-present location. `win_startup_folder_user` reports `constrained` (`startup_redirect_env_mismatch`) whenever this branch is taken, so it is never silently presented as a clean, complete `supported|0` result. Tracked for a follow-up fix; the underlying resolution is not corrected in this change. The `catalog` action still declares this source plain `SUPPORTED` (the mechanism is implemented; degradation is host-state-dependent, not a permanent catalog fact, unlike `lnx_systemd_timers_user`/`mac_login_items`) — don't read `catalog`'s `SUPPORTED` as a claim that `list` will read clean. The Windows sample below was captured under an interactive elevated-user session, not LocalSystem (the actual production identity), so it does not exercise this bug and correctly shows a clean result — it is not evidence the bug doesn't occur in production.
...
- Changelog: `changelog.d/20260909-autoruns-plugin.added.md`
<!-- END GENERATED -->
