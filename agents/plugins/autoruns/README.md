...
5. **Windows per-user Startup-folder redirect resolution is wrong for the common case, not just a redirected one.** `HKCU\...\Explorer\User Shell Folders\Startup` is populated with a `REG_EXPAND_SZ` value on essentially every Windows profile at creation — redirected or not. When present, `%USERPROFILE%`-style tokens in that value are expanded (`autoruns_win.cpp`, `expand_env_strings`) against the *agent process's own* environment, not the enumerated profile's — and this agent runs as LocalSystem. So the resolved path becomes LocalSystem's own profile directory, attributed to the enumerated user, on most stock profiles, not only genuinely-redirected ones. Tracked for a follow-up fix; not corrected in this change.
