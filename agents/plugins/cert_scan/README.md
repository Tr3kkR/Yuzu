...
|---|---|---|---|---|
| Windows | Agent service account. Home-directory discovery reads `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList` (already-established read, shared with `users`/`installed_apps`/`license_scan`); the subsequent per-user FILESYSTEM read (as opposed to registry-hive access, which those other plugins use) of another local user's home directory is genuinely new territory for this codebase — no existing plugin precedent measures whether the agent service account's ACLs permit reading another standard user's `%USERPROFILE%` tree by default. | None requested by this plugin. | **Not yet measured on this host** — see Caveats. | A per-file/per-subdirectory access-denied partway through a walk is skipped silently (`enumerate_files`'s `skip_permission_denied` directory option, plus per-entry error-code checks); the walk continues over the rest of the tree. A ROOT that cannot be opened AT ALL is different — it's recorded in `EnumerateFilesResult::root_outcomes` and surfaces as `CONSTRAINED`/Partial via the CC-07 result status (see Result status below), never a silent "0 findings" indistinguishable from a genuinely clean scan. |
| macOS | Agent daemon (root LaunchDaemon, `docs/agent-privilege-model.md`). Reading another user's home directory unprivileged-but-as-root is the same access shape `autoruns`'s `collect_user_launchagents` already exercises for `/Users/*`. | None. | Not yet captured on this host — see Caveats. | Same behaviour as Windows: per-entry denials are skipped silently, a whole-root denial is reported via result status. |
| Linux | Agent daemon. Same `/home/*` enumeration shape `autoruns`'s Linux collector already exercises; reading a `0700`-permission home directory the agent account doesn't own is the REALISTIC DEFAULT OUTCOME on a shared multi-user box — the root itself opens (the `/home` listing is typically world-readable) but `recursive_directory_iterator` on that specific user's home directory fails immediately, recorded as an inaccessible root. | None. | Not yet captured on this host — see Caveats. | Same behaviour as Windows/macOS: an inaccessible home directory is recorded and reported via result status, never silently absorbed into a "0 findings" result. |
...
| `OK` | Full | `cert_scan:scan` | Every configured/discovered root was opened and walked to completion (no inaccessible roots, `max_files_per_scan` not reached). |
| `CONSTRAINED` | Partial | `cert_scan:scan` | At least one root could not be opened at all (the realistic default-scan outcome on a shared box: the agent runs unprivileged against another user's `0700`/ACL-restricted home directory), or `max_files_per_scan` was reached before every root finished. Never reported as a false-clean `OK`. |
| `UNAVAILABLE` | Unknown | `cert_scan:scan` | Auto-discovery (no `paths` supplied) found zero home directories to even attempt — not a read failure, but not a completed scan of anything either. |
...
3. **PEM parsing has one remaining disclosed multi-block edge case.** `certificates_x509::parse_pem_certs` (reused unmodified from the `certificates` plugin) reads whichever PEM block comes first in the file and stops immediately if it isn't a `CERTIFICATE` block — a key-before-certificate bundle therefore reports the key but misses the certificate (the conventional ordering, certificate(s) first then key, is unaffected). Separately, a file concatenating TWO keys of the SAME TRADITIONAL format (e.g. two `-----BEGIN RSA PRIVATE KEY-----` blocks) with different encryption states reports only the first block's encryption status — a rare shape in practice (concatenated PKCS#8 encrypted+unencrypted keys in one file, a more realistic combined-bundle shape, IS correctly reported as two independent findings). See `cert_scan_rules.hpp`'s `classify_content()` doc comment.
4. **Windows per-user filesystem ACL behaviour is unmeasured, and the worst-case single-scan read volume is ~244 GiB.** Every existing Yuzu plugin that reads "every local user's data" on Windows does so via the registry (`ProfileList`/`HKEY_USERS` hive access); this plugin is the first to read another local user's home directory as a **filesystem** tree, and whether the agent service account can do so by default (vs. needing SeBackupPrivilege) has not been measured on a real multi-user host — an inaccessible root degrades honestly either way (recorded, reported via result status, see Privileges above). Separately, `max_file_size_bytes` × `max_files_per_scan` (5 MiB × 50,000) bounds one scan at ~244 GiB worst case, disclosed in `cert_scan_collect.hpp`'s `ScanConfig` header comment; the directory-exclusion list and depth cap both reduce how much of that ceiling is realistically hit, but neither lowers the theoretical maximum.
