...
1. **The documented `cap_net_raw` grant for `wake` looks over-stated against the current code.** `docs/agent-privilege-model.md:120` says `wol.wake` runs "with raw socket privileges from `cap_net_raw`" (macOS) / "`cap_net_raw` set on agent binary" (Linux). But `wol_plugin.cpp:215` opens `socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)` — a plain UDP datagram broadcast socket, not `SOCK_RAW` — and a `SO_BROADCAST` UDP send does not require `CAP_NET_RAW` on Linux or its macOS/BSD equivalent. Stated plainly: the privilege-model row asserts a grant the code, as written, does not need. This README does not resolve which is authoritative — it surfaces both so the doc can be re-verified against the binary that actually ships.
