...
| `UNAVAILABLE` | partial | `tar_db_unavailable` | `tar.db` failed to open read-only (missing file, lock contention, corruption) or failed `PRAGMA quick_check(1)` on open — every capture in this package hits the open-failure branch, since none of the capture hosts had a live `tar.db` with usage collection enabled at the default path. |
