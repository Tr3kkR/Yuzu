...
| `CONSTRAINED` | partial | `usage_feeder_disabled` | `last_used` only: `tar_config.usage_feeder_enabled` reads `"false"` — TAR's own fold has stopped advancing (its `usage`/`process` collector is off), so `usage_daily` would otherwise silently serve a frozen, aging window as if it were live. `summary` surfaces the same signal via its `meta` row's `feeder_enabled` field instead of refusing to run. |
