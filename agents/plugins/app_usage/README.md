...
|---|---|---|---|---|
| Windows | agent daemon account (same process as every other plugin; no per-plugin account — `docs/agent-privilege-model.md` has no app_usage row yet) | None — opening `tar.db` read-only needs only whatever file access the agent's own account already has (the file is created and owned by the agent via the `tar` plugin) | captured 2026-09-15 as interactive user (elevated) (`docs/samples/windows.txt`) — not the agent's production service-account posture; no elevated grant is exercised either way | result status `UNAVAILABLE`/`PARTIAL`, row `constrained\|tar_db_unavailable\|<path>\|<sqlite error>` |
| macOS | agent daemon account, dedicated unprivileged by design (`docs/agent-privilege-model.md:12`) | None — same read-only open | captured 2026-09-15 at euid 501 (`docs/samples/macos.txt`), reading a real on-disk `tar.db` | same shape; a schema-missing or disabled host instead reports `CONSTRAINED`/`PARTIAL`, row `constrained\|usage_schema_missing` / `constrained\|usage_source_disabled` |
| Linux | agent daemon account, dedicated unprivileged by design (`docs/agent-privilege-model.md:12`) | None — same read-only open | captured 2026-09-15 (`docs/samples/linux.txt`) | same shape as Windows/macOS |
...
| `CONSTRAINED` | partial | `usage_source_errored` | `tar_config.usage_enabled` holds a value that is neither `"true"` nor `"false"` — TAR itself is in an inconsistent tri-state (mirrors #560's `canonical_source_enabled`). |
...
| `UNAVAILABLE` | partial | `foreground_not_captured` | `foreground`: always — this source has no per-session focus-time capture on any platform (see Caveats). |
...
2. **`foreground` is always constrained, pending a user-context bridge.** Per-session focus-time and per-session attribution are not captured by any of the three platform legs today; the action exists in the descriptor and definition set so the roadmap can fill it in without a schema change once session-scope attribution lands, but every call to it reports `UNAVAILABLE`/`foreground_not_captured` unconditionally.
