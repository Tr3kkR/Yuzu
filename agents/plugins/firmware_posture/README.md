...
3. **Update-pending is Linux-only and needs fwupd.** Windows and macOS report no update verdict. On Linux the verdict covers only devices fwupd knows about and only their updatable ones; a host with a reachable bus but no daemon reports `update_pending|unavailable`, a host whose bus cannot be opened reports `update_pending|unreadable` with `fwupd:bus_open:<errno>` (`enoent` when the bus socket is missing), and an agent built without libsystemd reports `update_pending|unreadable` with `fwupd:not_built`. The unopenable-bus case is expected on the shipped container agent image, which has libsystemd but no D-Bus: it is not an agent fault, and it makes the command report FAILURE on every run. To read update state there, mount the host's `/run/dbus/system_bus_socket` (a security decision: it widens the container's authority) or scope `firmware` to native agents. `<call>` in a token is `bus_open`, `get_devices` or `get_upgrades`, and `<errno>` is a lowercase errno name (`enoent`, `enotdir`, `eio`, ...) or `errno_<n>`.
4. **Windows and Linux live captures are recorded, but not every shape.** The Windows `GetSystemFirmwareTable` path was probed as LocalSystem (`docs/samples/windows.txt`) and the Linux sample is a real run of the built `.so` (`docs/samples/linux.txt`), but that container has no `/sys/class/dmi` and no system bus (the sample shows `fwupd:bus_open:enoent`; the reachable-bus `unavailable` shape is proven only by a unit fixture, not an end-to-end run), so the populated-DMI shape has not been captured from a physical Linux host. Two limits follow. On Linux a sandbox that hides `/sys/class/dmi` from a host that has DMI (a gVisor-style runtime) is indistinguishable from a host with none, so the DMI fields read `absent`. On Windows a missing `root\CIMV2` namespace or `Win32_BIOS` class reported at WMI connect or query time reads `absent` (an explicit absent row), and a damaged WMI repository would look the same; a WMI fault during enumeration reads `unreadable` with a `wmi:wmi_next_failed_<hresult>` token.
