 ...
 *
 * Windows source — deliberately NOT PDH, NOT WMI, NOT per-process
 * OpenProcess handles (no ACCESS_DENIED gaps, no handle churn): ONE
 ...
 * set. CPU% needs two snapshots; the previous one is held in plugin memory
 * keyed by (pid, create_time) so PID reuse never miscounts. Linux
 * (/proc/[pid]/stat) and macOS (proc_pid_rusage) are kPlanned.
 *
 * Privacy: only image NAMES are recorded — never command lines (the
 * stronger default; cmdlines can carry secrets). The operator's TAR
 ...
 * sampling stays on).
 *
 * The derivation + top-N selection is PURE (derive_proc_samples) and
 * unit-tested on every host; read_proc_counters() is the impure shell.
 */
