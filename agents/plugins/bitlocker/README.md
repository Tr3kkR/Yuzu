...
4. **`docs/agent-privilege-model.md` disagrees with itself on the Windows grant.** Line 99 (read-only table) names `SeBackupPrivilege`; line 122 (privileged-actions table) names `Administrators` group membership. Neither is enforced in code today — the WMI namespace access check is purely a runtime COM error the plugin classifies via `is_permission_denied()`.
