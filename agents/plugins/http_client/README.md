...
| **Version** | 1.1.0 |
| **Kind** | Action · read-only (`get`, `head`) / mutating (`download`) · on-demand (no scheduled gather) |
...
| **Security** | `download`: securable `FileRetrieval` · operation Write · risk High · dispatch Destructive · approval gate AdminOrApproval. `get`/`head`: securable `Infrastructure` · operation Read · risk Low · dispatch ReadOnly · approval gate none |
| **Roles** | execute: endpoint-admin (`download`); endpoint-admin, endpoint-operator (`get`, `head`) · author: content-author |
<!-- END GENERATED -->
...
**Declared limits per leg** (the descriptor's fallback text, verbatim): none — every action/OS leg in `kActionDescriptors` declares a `nullptr` fallback.
<!-- END GENERATED -->
...
**`download` — `status|path|size|hash`**

| Field | Type | Values | Available | Example |
|---|---|---|---|---|
| `status` | string | `ok` | W, M, L | `ok` |
| `path` | string | absolute filesystem path | W, M, L | `/tmp/yuzu_capture/yuzu_capture_download.html` |
| `size` | int64 | non-negative integer, bytes | W, M, L | `559` |
| `hash` | string | 64-character lowercase hex string | W, M, L | `ff67a9d764d6a2367a187734e697f6a53217db9a21c101d410a113ca871a299d` |

**`get` — `status|body`**

| Field | Type | Values | Available | Example |
|---|---|---|---|---|
| `status` | int32 | HTTP status code, or 0 on request failure | W, M, L | `200` |
| `body` | string | free text, or an error string on failure | W, M, L | `<!doctype html><html lang="en">...` |

**`head` — `status|content_length|content_type` (declared; code emits `status\|headers` instead — see above)**

| Field | Type | Values | Available | Example |
|---|---|---|---|---|
| `status` | int32 | HTTP status code, or 0 on request failure | W, M, L | `200` |
| `content_length` | int64 | not emitted by current code | none | `-` |
| `content_type` | string | not emitted by current code | none | `-` |
<!-- END GENERATED -->
...
- Capability rows: `server/core/src/capability_decls/plugin_action_catalogue_c.hpp:278,295,305`
- Tests: `tests/unit/test_new_plugins.cpp` (`DESCRIPTOR_TEST("http_client", ...)` at line 329; URL-validation cases from line 349; SSRF/private-IP classification mirror tests from line 397)
