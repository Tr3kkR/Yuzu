...
4. **Linux reads machine-level JSON policy files only.** Only the `managed` and `recommended` `*.json` files under `/etc/opt/chrome`, `/etc/chromium` and `/etc/opt/edge` are read; per-user policy, snap and flatpak Chromium paths and Firefox `policies.json` are not, and every row reports scope `machine`. The leg is stricter than Chromium's own loader in four places, and every one fails safe (a constrained or extra row, never a false `OK`) except the second: Chromium tolerates trailing commas in these files (this leg reports `linux:json_unparseable` for the file); Chromium parses every file in the directory, not only `*.json`, so a valid policy file with another name is applied by the browser but skipped here; Chromium follows symlinks, this leg refuses them (`linux:symlink_refused`); and where two files set the same policy Chromium applies the last file in name order, while this leg reports one row per file with its `source`.
