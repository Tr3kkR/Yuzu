...
std::unordered_map<uint64_t, int> build_inode_to_pid_map() {
    ...
    while ((proc_entry = readdir(proc_dir)) != nullptr) {
        ...
        while ((fd_entry = readdir(fd_dir)) != nullptr) {
            ...
            if (inode > 0) map.emplace(inode, pid);
        }
        closedir(fd_dir);
    }
    closedir(proc_dir);
...
void parse_proc_net_file(const char* path,
    ...
    while (std::getline(f, line)) {
        ...
        if (inode > 0) {
            ...
            if (it != inode_map.end()) pid = it->second;
        }
...
void enumerate_and_stream(yuzu::CommandContext& ctx) {
    ...
    for (pid_t pid : pids) {
        ...
        for (size_t i = 0; i < fd_count; ++i) {
            ...
            auto key = std::format("{}:{}:{}:{}:{}", proto, local_addr, local_port,
...
void emit_tcp4(yuzu::CommandContext& ctx) {
    ...
    for (DWORD i = 0; i < table->dwNumEntries; ++i) {
        ...
        ctx.write_output(std::format("tcp|{}|{}|{}|{}|{}|{}",
            ...
            tcp_state_str_win(row.dwState), static_cast<int>(row.dwOwningPid)));
    }
}
