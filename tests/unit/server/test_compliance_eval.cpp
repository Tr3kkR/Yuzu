...
TEST_CASE("ComplianceEval: string comparison fallback for >=", "[compliance_eval][basic]") {
    ...
    CHECK(evaluate_compliance_bool("result.version >= '1.0'", fields) == true);
    CHECK(evaluate_compliance_bool("result.version >= '2.0'", fields) == true);
    CHECK(evaluate_compliance_bool("result.version >= '3.0'", fields) == false);
}

TEST_CASE("ComplianceEval: double-quoted strings", "[compliance_eval][basic]") {
    std::map<std::string, std::string> fields = {{"name", "yuzu-agent"}};
    CHECK(evaluate_compliance_bool("result.name == \"yuzu-agent\"", fields) == true);
}
...
TEST_CASE("ComplianceEval: AND both true", "[compliance_eval][combinator]") {
    ...
    CHECK(evaluate_compliance_bool("result.a == '1' AND result.b == '2'", fields) == true);
}

TEST_CASE("ComplianceEval: AND one false", "[compliance_eval][combinator]") {
    std::map<std::string, std::string> fields = {{"a", "1"}, {"b", "9"}};
    CHECK(evaluate_compliance_bool("result.a == '1' AND result.b == '2'", fields) == false);
}
...
TEST_CASE("ComplianceEval: OR one true", "[compliance_eval][combinator]") {
    ...
    CHECK(evaluate_compliance_bool("result.x == '1' OR result.y == '2'", fields) == true);
}
...
TEST_CASE("ComplianceEval: OR both false", "[compliance_eval][combinator]") {
    ...
    CHECK(evaluate_compliance_bool("result.x == '1' OR result.y == '2'", fields) == false);
}

TEST_CASE("ComplianceEval: NOT true", "[compliance_eval][combinator]") {
    std::map<std::string, std::string> fields = {{"status", "running"}};
    CHECK(evaluate_compliance_bool("NOT result.status == 'failed'", fields) == true);
}

TEST_CASE("ComplianceEval: NOT false", "[compliance_eval][combinator]") {
    std::map<std::string, std::string> fields = {{"status", "failed"}};
    CHECK(evaluate_compliance_bool("NOT result.status == 'failed'", fields) == false);
}
...
TEST_CASE("ComplianceEval: nested parentheses with OR and AND",
    ...
    CHECK(evaluate_compliance_bool("(result.a == '1' OR result.b == '2') AND result.c == '3'",
                              fields) == true);
}
...
TEST_CASE("ComplianceEval: double NOT", "[compliance_eval][combinator]") {
    std::map<std::string, std::string> fields = {{"status", "failed"}};
    CHECK(evaluate_compliance_bool("NOT NOT result.status == 'failed'", fields) == true);
}
...
TEST_CASE("ComplianceEval: contains match", "[compliance_eval][string]") {
    ...
    CHECK(evaluate_compliance_bool("result.name contains 'server'", fields) == true);
}

TEST_CASE("ComplianceEval: contains mismatch", "[compliance_eval][string]") {
    std::map<std::string, std::string> fields = {{"name", "DatabaseHost"}};
    CHECK(evaluate_compliance_bool("result.name contains 'server'", fields) == false);
}
...
TEST_CASE("ComplianceEval: empty expression returns true", "[compliance_eval][edge]") {
    ...
    CHECK(evaluate_compliance_bool("", fields) == true);
}
...
TEST_CASE("ComplianceEval: whitespace-only expression returns true",
    ...
    CHECK(evaluate_compliance_bool("   ", fields) == false);
}
...
TEST_CASE("ComplianceEval: missing field in result is empty string",
    ...
    CHECK(evaluate_compliance_bool("result.missing == 'running'", fields) == false);
}

TEST_CASE("ComplianceEval: deeply nested parentheses", "[compliance_eval][edge]") {
    std::map<std::string, std::string> fields = {{"a", "1"}};
    CHECK(evaluate_compliance_bool("(((result.a == '1')))", fields) == true);
    CHECK(evaluate_compliance_bool("(((result.a == '2')))", fields) == false);
}

TEST_CASE("ComplianceEval: type coercion -- string '42' > 40", "[compliance_eval][edge]") {
    std::map<std::string, std::string> fields = {{"count", "42"}};
    CHECK(evaluate_compliance_bool("result.count > 40", fields) == true);
    CHECK(evaluate_compliance_bool("result.count > 42", fields) == false);
    CHECK(evaluate_compliance_bool("result.count > 50", fields) == false);
}

TEST_CASE("ComplianceEval: negative numbers", "[compliance_eval][edge]") {
    std::map<std::string, std::string> fields = {{"temp", "-5"}};
    CHECK(evaluate_compliance_bool("result.temp < 0", fields) == true);
    CHECK(evaluate_compliance_bool("result.temp > -10", fields) == true);
    CHECK(evaluate_compliance_bool("result.temp == -5", fields) == true);
    CHECK(evaluate_compliance_bool("result.temp == -3", fields) == false);
}

TEST_CASE("ComplianceEval: boolean literal true", "[compliance_eval][edge]") {
    std::map<std::string, std::string> fields = {{"enabled", "true"}};
    CHECK(evaluate_compliance_bool("result.enabled == true", fields) == true);
}

TEST_CASE("ComplianceEval: boolean literal false", "[compliance_eval][edge]") {
    std::map<std::string, std::string> fields = {{"enabled", "false"}};
    CHECK(evaluate_compliance_bool("result.enabled == false", fields) == true);
    CHECK(evaluate_compliance_bool("result.enabled == true", fields) == false);
}
