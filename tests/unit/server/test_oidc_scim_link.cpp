...
//
// Both deny sites in auth_routes.cpp bump the TOTAL counter
// (`yuzu_auth_oidc_deprovisioned_denied_total`) on every DENY, then split
// into `yuzu_auth_oidc_deprovisioned_denied_genuine_total` (scim_id present)
// vs `yuzu_auth_oidc_deprovision_denied_store_unavailable_total` (scim_id
// absent) on the SAME `decision.scim_id` predicate the U6 `reason=` ternary
// above already keys off — a Postgres outage must never inflate the
// genuine/alertable sub-counter. Mirrors the U6 tests' approach exactly
// (the route handler itself is unreachable without a live IdP, see the
// file-header docstring): a local mirror of auth_routes.cpp's exact
// increment logic, driven by a REAL `OidcLoginDenyDecision` from
// `oidc_login_denied_deprovisioned`.
TEST_CASE("#3069: the deny-counter split bumps genuine XOR store-unavailable, "
    ...
    // Mirrors auth_routes.cpp's exact construction at all four deny sites:
    //   m->counter("yuzu_auth_oidc_deprovisioned_denied_total").increment();
    //   if (!decision.scim_id)
    //       m->counter("yuzu_auth_oidc_deprovision_denied_store_unavailable_total").increment();
    //   else
    //       m->counter("yuzu_auth_oidc_deprovisioned_denied_genuine_total").increment();
    auto bump_split_counters = [](yuzu::MetricsRegistry& m, const OidcLoginDenyDecision& decision) {
        m.counter("yuzu_auth_oidc_deprovisioned_denied_total").increment();
    ...
    SECTION("store-unavailable DENY (scim_id absent) -> store_unavailable_total + total, "
        ...
        CHECK(m.counter("yuzu_auth_oidc_deprovision_denied_store_unavailable_total").value() ==
              1.0);
...
// deny sites' identical increment logic. The post-mint re-check site
// (auth_routes.cpp's concurrent-deprovision-race check) bumps the same
// three counters via the SAME predicate — pin it too, mirroring the U6
// post-mint test's rationale for a separate TEST_CASE rather than folding
...
TEST_CASE("#3069: the post-mint re-check deny-counter split also bumps genuine XOR "
    ...
    SECTION("store-unavailable DENY on recheck (scim_id absent) -> store_unavailable_total + "
        ...
        auto decision = oidc_login_denied_deprovisioned(&broken_store, "https://idp.example.com/",
                                                         "sub-3069-recheck-unavailable",
                                                         "ext-3069-recheck-unavailable");
        REQUIRE(decision.denied);
        REQUIRE_FALSE(decision.scim_id.has_value());
    ...
    SECTION("resolved-deprovisioned DENY on recheck (scim_id present) -> genuine_total + "
        ...
        auto r = store.create_resource("u3069-recheck-inactive-user", "ext-3069-recheck-inactive");
        REQUIRE(r.has_value());
        REQUIRE(store.upsert_link("https://idp.example.com/", "sub-3069-recheck-inactive",
                                  r->scim_id));
        REQUIRE(store.set_active(r->scim_id, false));

        auto decision = oidc_login_denied_deprovisioned(
            &store, "https://idp.example.com/", "sub-3069-recheck-inactive",
            "ext-3069-recheck-inactive");
        REQUIRE(decision.denied);
        REQUIRE(decision.scim_id.has_value());
