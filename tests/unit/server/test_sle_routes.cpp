...
// The route is NAMED for licences but the cascade erases SIX per-agent stores spanning
// FOUR securables' READ (SoftwareLicensing/software_licensing, Inventory/inventory+
// software_inventory+device_inventory, GuaranteedState/app_perf_daily, Forensics/
// app_usage). Rather than a hand-maintained conjunction over all four — the shape ADR-0024
// Decision 9 REVERSED once a fourth securable joined — the gate is now ONE scoped
// securable, `Decommission:Delete` (PLAN-01 ruling (b)), that authorizes for the whole
// blast radius regardless of how many stores it grows to cover.
