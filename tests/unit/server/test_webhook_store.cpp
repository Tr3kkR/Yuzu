 ...
 *
 * No legacy-SQLite backfill test coverage: the dedicated TEST_CASE suite
 * (2026-08-25) was removed as part of a fresh-start-by-default policy
 * change (ADR-0009 amendment). WebhookStore::migrate_from_sqlite()/
 * migrate_from_sqlite_impl() themselves are UNCHANGED and still present
 * (their removal is a separate, later step) -- this store shipped its
 * backfill (PR #3563) the same day the amendment landed, too late for
 * the "don't build it" guidance to reach it; only its TEST suite is
 * being retired here, same treatment as the other already-migrated
 * stores' backfill tests.
 */
