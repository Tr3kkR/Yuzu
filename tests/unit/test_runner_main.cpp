...
// (-Db_coverage) and ASan/LSan/TSan legs need a normal process exit for
// their atexit dumps (gcov's .gcda write, LSan's leak report) — hard_exit()
// skips atexit entirely by design. The exit-42 hazard is a Windows-debug-CI
// phenomenon (#1648); lifting this guard to other platforms would silently
// break those legs' own instrumentation.
//
