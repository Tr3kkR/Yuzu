...
`CommandCapability` rows) is authored as NINETEEN independent, hand-written
sources: the eighteen per-plugin-group fragment headers listed in
...
mechanically checks that these nineteen sources, taken together, actually
...
`.plugin = "..."` / `.action = "..."` pair declared across the nineteen
...
`plugin.action`, when:
  ...
  1. A plugin declares an action that has no catalogue row anywhere across
     the nineteen sources (a MISSING row) — a plugin ships a capability the
     dispatch-classification layer would report `Unclassified` for.
  2. One of the eighteen per-group fragments declares a `plugin.action` no
  ...
  3. The same `plugin.action` is declared by more than one of the nineteen
