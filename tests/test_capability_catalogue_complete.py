...
`CommandCapability` rows) is authored as EIGHTEEN independent, hand-written
sources: the seventeen per-plugin-group fragment headers listed in
...
mechanically checks that these eighteen sources, taken together, actually
...
`.plugin = "..."` / `.action = "..."` pair declared across the eighteen
...
`plugin.action`, when:
  ...
  1. A plugin declares an action that has no catalogue row anywhere across
     the eighteen sources (a MISSING row) — a plugin ships a capability the
     dispatch-classification layer would report `Unclassified` for.
  2. One of the seventeen per-group fragments declares a `plugin.action` no
  ...
  3. The same `plugin.action` is declared by more than one of the eighteen
