...
| Agent daily-sync framework + installed-software inventory (ADR-0016) — agent pushes per-source endpoint state on a daily cadence over the `ReportInventory` RPC, kind to the network (stable per-agent phase spread, hash-skip when unchanged, server `need_full` cold-cache resend [jittered], weekly full-floor). Agent seam: `SyncScheduler` (`sync_scheduler.*`) + `__sync__` KV namespace + `LocalDispatcher` in-process plugin reuse; source #1 `installed_software` (`sync_source_installed_software.*`, machine-scope, reuses the `installed_apps` plugin). Server: shared `inventory_ingestion.*` ingest seam wired IDENTICALLY on the direct `ReportInventory` AND gateway `ProxyInventory` paths; born-on-PG **`SoftwareInventoryStore`** (schema `software_inventory_store`, normalized rows, current-state-only, server-recomputed canonical hash; **`inventory_state.last_seen`/`first_seen` = SERVER receipt time, never the agent-supplied `collected_at`** — the freshness gauge keys on `last_seen`, so trusting agent time let a future-skewed/hostile agent hide a dark endpoint; #1685/ADR-0016 Update 2026-06-27. `collected_at` stays proto-carried but drives no persisted column) — **COEXISTS with** the preserved generic SQLite `InventoryStore` (still backs the `kInventoryQuery` scope source + eval engine). New `Inventory` RBAC securable (`Inventory:Read`). Adding a sync source = register a `SyncSource` on the scheduler + (if fleet-queryable) add a typed table to the store; gateway proto field adds need `agent_pb`+`gateway_pb` regen. **Windows registry reads in any plugin/source feeding a stored or fleet-queryable surface MUST use the wide `Reg*W` APIs + `WideCharToMultiByte(CP_UTF8)`, never `Reg*A`** (returns the system code page → silently corrupts non-ASCII names → breaks `WHERE name=$1`; #1662). Established idiom: the `registry`/`processes` plugins. | `docs/adr/0016-agent-daily-sync-framework.md` + `docs/user-manual/inventory.md` | `security-guardian` + `docs-writer` + `architect` on any change to `sync_scheduler.*` / `sync_source_*` / `inventory_ingestion.*` / `software_inventory_store.*` / the `ReportInventory`+`ProxyInventory` handlers / the `Inventory` securable |
