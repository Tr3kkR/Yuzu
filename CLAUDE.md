...
3. **Prose: `docs-writer` owns WORDING** (including in-code comments and log/error strings); **the DOMAIN agent owns TRUTH**. A comment that *contradicts* the code is a truth finding at native severity and any agent may raise it; wording-only is capped at NICE. **Absence is neither** — a required doc that is MISSING contradicts nothing, so it is a truth finding derived as `I7`, never capped at NICE (#2620).
...
Every finding is recorded to a run ledger with `pass_ordinal`, stable `finding_id`, `provenance`, `source` and `disposition`. `source` (`agent`/`collaborator`/`external-model`) exists because independent review is only independent if the WORKING COPIES are — on #2604 two external models independently confirmed a claim that was wrong, all three reviewers sharing one stale checkout; a fragment is therefore appended to after the run ends, and an absent `collaborator` row means nothing was recorded, NOT that no external review happened. `refuted` is a distinct disposition from `rejected` and carries its evidence in `refuted_by` (#2619). It is not a database, and not a signed-waiver merge contract - gating findings are still resolved before the gate passes. Stored as a repo-committed fragment on the `changelog.d` model — `governance.d/<PR#>-<slug>.<random>.jsonl`, `mktemp`-created so concurrent runs cannot collide, one file per RUN (`pass_ordinal` separates rounds within it). It lives in the repo because the record is evidence for whoever reviews the PR, and a per-machine path is unreadable by them by construction (#2618). `YUZU_GOV_LOG_DIR` redirects a throwaway local run outside the tree. Retention is undecided and nothing prunes them.
