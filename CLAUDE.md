...
| Fleet visualization (3D) — REST surface (`/api/v1/viz/fleet/topology` + `/fragments/...`), kill switch (`--viz-disable` / `YUZU_VIZ_DISABLE`), `machines_max` DoS cap (M-1, default 5000 / ceiling 100000), tier-before-permission ordering (kill switch precedes RBAC), audit actions `viz.fleet_topology` + `viz.fleet_topology.invalidate` (target_type `FleetTopology`, results `success` / `denied` / `failure`), fragment `</script>` escape via `escape_json_for_script`, fetcher executions-tracker opt-out rationale, store-internal metrics (`yuzu_viz_refill_*`) gauge-as-snapshot pattern | `docs/user-manual/rest-api.md` §Fleet Visualization + `docs/user-manual/audit-log.md` viz rows + `docs/user-manual/metrics.md` §Fleet visualization metrics; standing invariants live in `viz_routes.cpp` file header | `security-guardian` + `docs-writer` on any change to `viz_routes.{hpp,cpp}` / `fleet_topology_store.{hpp,cpp}` / `--viz-disable` / `Config::viz_disable` |
