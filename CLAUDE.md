...
- `forward_gateway_pending()` lives in `server.cpp` and drains the queue via `gw_mgmt_stub_->SendCommand()` — `gw_mgmt_stub_` remains a raw gRPC stub on the management plane until #376 PR 1c-5 lifts `gateway_service_impl` onto the transport abstraction
