...
| **Headless platform, process decomposition & access-control spine (ADR-1005 + ADR-0031/0032/0033)** — ADR-1005 (merged): every capability behavior reachable by an authenticated external principal via versioned REST **and** MCP (or a recorded ledger exception), discoverable (A2/A3), A4 envelope, RBAC + audit at the API — **no UI-only capabilities** (a dashboard fragment is not an API twin). **On-behalf-of assertions rejected on every ingress** (sole exception: the four health-probe paths); engine principals are a distinct principal class, never impersonation. Exec plan holds phase status + vuln milestones (M3 = the parity **+ confinement** gate). ADR-0031/0032/0033 (accepted 2026-07-14, **design-only, binds prospectively**): presentation / core / engine are separate binaries; **core owns the public API and is the sole authority** — every other component may only **narrow** authority. **No private core API** (INV-31-4); **no cross-component DB access** (INV-31-3). Engines are **headless** (voids 2c F-10 + exec-plan Decision 3): core admits every run, mints a short-lived **audience-bound grant**, and confines every engine read against the **admitting operator's current** authority; **no grant is a bearer capability over data** — even a cached result is redeemed at core. **Interlock: no ballot-A5 code ships before engine principals + the ADR-0017 gate (#1716) + the audit-evidence schema + the release log + the capability registry.** **One core-owned approval primitive — no new approval gate may be built outside it**; four-eyes compares distinct **human roots**; effects cross core as plan-hash-bound **Execution Plans**; mutations fail closed on audit failure. | `docs/adr/1005-...md` + `docs/adr-1005-execution-plan.md` + `docs/adr/0031-presentation-core-engine-decomposition.md` + `0032-use-case-admission-protocol.md` + `0033-access-control-spine.md` | `consistency-auditor` on every capability-adding PR; `architect` + `security-guardian` on any new REST route / MCP tool / dashboard fragment / ingress surface / **approval gate** / engine surface, and on `on_behalf_guard.hpp` / `principal_class.hpp` / the pre-routing chokepoint in `server.cpp`; `docs-writer` on the 2c topology / host-requirements / exec-plan docs |
