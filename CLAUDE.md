...
| Spark detection layer (ADR-0021) — `SparkEngine` is the agent's sole detection primitive (watchers multiplexed **by mechanism, not rule**; inline tier core-internal + enforce-only + µs-latency, queued tier for all else). Rung 1 shipped: instantiated **observe-only** in `agent.cpp` behind `--spark-disable`, **no consumer armed** — legacy `IGuard` still detects+enforces. Guardian is the first consumer (rung 2). | `docs/adr/0021-spark-reflex-architecture.md` + `docs/spark-stage2-guardian-consumer-design.md` | `security-guardian` + `cpp-safety` on `spark_engine.*` / `spark.hpp` / `spark_mechanism.*` / `spark_*.{hpp,cpp}` |
