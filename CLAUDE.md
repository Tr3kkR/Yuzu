...
> Server `:50052` was a placeholder `ManagementService` listener that returned `UNIMPLEMENTED` to every RPC; removed in #376 PR 1c-5 along with the `--management*` CLI flags.
...
- `forward_gateway_pending()` lives in `server.cpp` and drains the queue via `gw_mgmt_channel_->bidi_stream("yuzu.server.v1.ManagementService/SendCommand", ctx)` (a `transport::Channel` since #376 PR 1c-5); request frame written then `writes_done()` (server-streaming-as-bidi). On `transport::StatusCode::Unavailable` the command is re-queued via `AgentRegistry::reenqueue_gateway_pending` (D.1 retry — `attempts++` on each retry, 3-strike cap). Detached `std::thread` per command preserved — maps onto a per-command QUIC stream in the post-PR-3 native-QUIC redesign.
...
| Transport abstraction — `yuzu::transport::` Channel/ServerListener/BidiStream interface, Credentials zeroisation, ClientCertMode partial-order reload contract, frame-size hierarchy (`kDefaultMaxFrameSize`/`kAbsoluteMaxFrameSize`), method-name regex, BackoffPolicy bounds, Status::detail visibility contract, ServerCall `delete this` async-state-machine pattern, **bounded bidi dispatcher pool** (`ListenerOptions::bidi_dispatcher_pool_size`, default `clamp(64, hw*8, 4096)`; saturation rejects with `ResourceExhausted` via UnaryPendingFinish reaper — closes UP-14/UP-13, #904), deferred PR ladder (#376 PR 1b-3 bidi Merged, PR 1c-1 typed-proto helpers Merged, PR 1c-2 server-side AgentService lift Merged, PR 1c-3 agent unary lift Merged, **bidi `read()` idle-deadline** (`BidiStream::read(msg, deadline=0ms)`; default zero = no deadline; expiry cancels stream and surfaces `DeadlineExceeded` via `final_status()` on the side that armed it; `read_pb` has matching overload — closes UP-8, #902; **gRPC wire-status caveat**: peer observes `Cancelled` regardless of handler-supplied final status because `gctx_.TryCancel()` sends RST_STREAM before `Finish` runs — local `final_status()` is the canonical signal, not the wire trailer), PR 1c-4 agent Subscribe + DownloadUpdate — Merged: (i) #916 contract softening, (ii) Subscribe + DownloadUpdate lift, (iii) legacy gRPC channel + diagnostic block + keepalive-args cleanup. Keepalive args now live inside `transport::GrpcChannel` (closes SRE-1); msquic backend will configure its own primitives independently. **Server-side `CallContext::cancel` is backend-best-effort per #916** (handlers detect peer disconnect via `read()`-returns-false, the canonical cross-backend pattern), **PR 1c-5 mgmt+gateway lift — Merged**: drops `:50052` placeholder `ManagementService` entirely (BREAKING — `--management*` CLI flags removed); lifts `GatewayUpstreamServiceImpl` (`:50055`) onto `transport::ServerListener` and the outbound `gw_mgmt_stub_` (server→gateway `SendCommand`) onto `transport::Channel` as bidi-with-`writes_done()`. **D.1 retry**: detached `std::thread` per pending command preserved (maps onto per-command QUIC stream post-PR-3); `Unavailable` failures re-queue with `attempts++`, 3-strike cap via `AgentRegistry::reenqueue_gateway_pending`; non-`Unavailable` failures drop with `yuzu_gateway_forward_dropped_total{reason=...}` metric. **Wire-equivalence framing softened — wire-equivalent through PR 1c-6; native-QUIC redesign (per-command stream IDs, persistent channels, 0-RTT, stream priority) moved to a post-PR-3 redesign PR so it anchors against the real msquic backend.** **PR 1c-6 cleanup — Merged 2026-05-11**: gateway `/readyz` probes grpcbox listener supervisors via `grpcbox_services_sup:services_sup_name/1` (#896 third bullet — role-named `agent_listener` / `mgmt_listener` JSON keys matching server-side naming); trailing-metadata SYMMETRIC scrub at the inbound wire boundary in `grpc_channel.cpp` — `sanitise_status_detail` applied to BOTH key AND value at unary path and bidi `final_status()` (#897); `server.cpp::build_tls_credentials` deleted, inline TLS pre-flight uses `std::filesystem::file_size > 0` rather than `read_file_contents().empty()` so PEM bytes never enter process memory (closes governance round 1 UP-319 secure_zero regression vs the deleted helper's RAII zeroisation). Three `<grpc*>` includes removed from server.cpp. UP-56 wildcard `bound_endpoint().host` resolution deferred — no production broadcast call site today. PR 3 msquic, PR 5 dual-stack) | `docs/adrs/0001-quic-transport-msquic-quicer.md` + `transport/include/yuzu/transport/transport.hpp` | `cpp-expert` on any `transport/**/*.{cpp,hpp}` change; `build-ci` on `transport/meson.build`; `security-guardian` on Credentials / zeroisation / mTLS surface change |
