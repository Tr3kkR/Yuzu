...
**Prometheus alert rules are PARSE-gated by `promtool` for all rules, BEHAVIOUR-gated only for the alerts that have cases** in `tests/prometheus/yuzu-alerts.test.yml` (today: the `YuzuAuditRetention*` liveness pair). Nothing enforces that a rule change ships a case, so a green check on an edit to any other rule proves parseability only. Harness: `tests/prometheus/run_promtool_tests.py`, run by both `meson test --suite docs` and the `docs-lint` workflow's job `prometheus-rules`, which publishes the status check named **"Prometheus alert rules"** (the job id is not the check name — that distinction is what branch protection matches on). Same skip-vs-fail shape as `YUZU_TEST_ENABLE_PG`: `YUZU_TEST_ENABLE_PROMTOOL_DOCKER` **unset** → needs a native promtool of the pinned major, else skip; **set but docker broken** → FAIL. The opt-in exists because `scripts/ci/flake-retry.py` runs `meson test` with **no `--suite` filter** on three REQUIRED legs, so any docker-dependent `docs`-suite test would put a container registry on all three — that shape was shipped once and reverted (#2553). **A new test here that pulls an image must be opt-in gated the same way.** Alert-authoring conventions (when to add a case, how cross-metric rules match labels) live in `docs/observability-conventions.md`.
