...
| `-Dtransport` | `grpc` | Transport backend(s): `grpc`, `msquic`, or `both`. `msquic` is the #376 PR 3 native-QUIC backend — dormant (not wired into server/agent), exercised only by the transport test suite. |
...
- `msquic` is present with the `0-rtt` feature (#376 PR 3). Two non-obvious properties: (1) the vcpkg msquic port is **`ONLY_DYNAMIC_LIBRARY`** — it ships as a shared lib (`libmsquic.{so,dylib}` / `msquic.dll`) and must be co-located with the binaries at runtime, unlike every other transport dep which is static-linked. (2) The `0-rtt` feature forces **`quictls/openssl 3.1.7-quic1`** (a fork of OpenSSL 3.1.x, NOT upstream) rather than schannel on Windows — see ADR-0001's cryptographic-module-posture section and #891; do not cite "OpenSSL 3.x" generically in customer answers. `transport/meson.build` references `msquic` with `method: 'cmake'` pinned. Only pulled when `-Dtransport` includes `msquic`.
...
| Transport abstraction — `yuzu::transport::` Channel/ServerListener/BidiStream interface, Credentials zeroisation, ClientCertMode partial-order reload contract, frame-size hierarchy, BackoffPolicy bounds, Status::detail visibility, ServerCall `delete this` async-state-machine pattern, bounded bidi dispatcher pool (#904), bidi `read()` idle-deadline (#902), `CallContext::cancel` backend-best-effort (#916), gRPC wire-status caveat (peer observes `Cancelled` regardless of handler-supplied final status — local `final_status()` is canonical). #376 PR ladder through PR 1c-6 Merged (server+agent+mgmt+gateway lifted to `transport::`); native-QUIC redesign deferred to post-PR-3 to anchor against real msquic backend. PR 1c-5 BREAKING: dropped `:50052` placeholder `ManagementService` + `--management*` CLI flags. PR 1c-6: gateway `/readyz` listener-supervisor probe (#896), symmetric trailing-metadata scrub at wire boundary (#897), `build_tls_credentials` deleted in favour of inline `file_size > 0` pre-flight (PEM bytes never enter memory). PR 3 ALL 9 increments landed (`c63293f`): bidi, back-pressure, deadlines, YUZU_ALLOW_INSECURE_TLS env gate + multi-ALPN, TransportMetricSink parity, bounded bidi pool (saturation = `ResourceExhausted` "transport: bidi dispatcher saturated", gRPC matched), peer_uri parity (`ipv4:`/`ipv6:` prefix). Backend dormant. Follow-ups: #1041 (singleton teardown + IdleTimeoutMs), #1042 (full mTLS — PKCS12, Require, peer_san_identities, CA pinning; pre-PR-5 hard gate). Test-only msquic back-pressure probes gated behind `YUZU_TRANSPORT_TESTING`. Next: PR 4 quicer Erlang gateway, PR 5 dual-stack. | `docs/adrs/0001-quic-transport-msquic-quicer.md` + `transport/include/yuzu/transport/transport.hpp` | `cpp-expert` on any `transport/**/*.{cpp,hpp}` change; `build-ci` on `transport/meson.build`; `security-guardian` on Credentials / zeroisation / mTLS surface change |
