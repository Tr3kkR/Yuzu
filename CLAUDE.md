...
**Prometheus alert rules are PARSE-gated by `promtool` for all rules, BEHAVIOUR-gated only for the alerts that have cases** in `tests/prometheus/yuzu-alerts.test.yml` (today: the `YuzuAuditRetention*` liveness pair). Nothing enforces that a rule change ships a case, so a green `prometheus-rules` check on an edit to any other rule proves parseability only. Harness: `tests/prometheus/run_promtool_tests.py`, run by both the `docs-lint` workflow's `prometheus-rules` job and `meson test --suite docs`. Same skip-vs-fail shape as `YUZU_TEST_ENABLE_PG`: `YUZU_TEST_ENABLE_PROMTOOL_DOCKER` **unset** → needs a native promtool of the pinned major, else skip; **set but docker broken** → FAIL. The opt-in exists because `scripts/ci/flake-retry.py` runs `meson test` with **no `--suite` filter** on three REQUIRED legs, so any docker-dependent `docs`-suite test would put a container registry on all three — that shape was shipped once and reverted (#2553). **A new test here that pulls an image must be opt-in gated the same way.** Changing an alert's `expr` means updating `tests/prometheus/yuzu-alerts.test.yml`; cross-metric rules use PromQL's default all-label matching, and an explicit `on(...)` needs a stated reason.
