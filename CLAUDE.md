...
| TAR dashboard page — three frames (retention-paused source list, scope-walking-aware SQL, process tree viewer reconstructed from `process_live` seed + events), URL structure, permissions, observability | `docs/tar-dashboard.md` | `architect` on any `/dashboard/tar` or `/api/v1/tar/...` route change; `plugin-developer` on TAR action surface (`tar.status`, `tar.process_tree`, `tar.purge_source`); `docs-writer` on dashboard nav / template change |
