...
function generateCompose(c) {
  ...
  // HTTPS is served unless plaintext mode explicitly disables it.
  const webScheme = c.tlsMode === 'plaintext' ? 'http' : 'https';
...
`;
  ...
  if (c.tlsMode === 'default' && c.persistCerts) {
    if (c.persistentVolumes) {
  ...
  y += `    healthcheck:\n`;
  // In plaintext mode the dashboard is HTTP; otherwise it serves HTTPS and the
  // healthcheck must probe https with -k (the default cert is self-signed/internal).
  const hcUrl = c.tlsMode === 'plaintext'
    ? 'http://localhost:8080/livez'
    : 'https://localhost:8080/livez';
  const hcFlags = c.tlsMode === 'plaintext' ? '-sf' : '-skf';
  y += `      test: ["CMD", "curl", "${hcFlags}", "${hcUrl}"]\n`;
  y += `      interval: 10s\n`;
  y += `      timeout: 5s\n`;
  y += `      retries: 5\n`;
  ...
  if (c.persistentVolumes) {
    y += `\nvolumes:\n`;
    y += `  server-data:\n`;
    if (c.tlsMode === 'default' && c.persistCerts) y += `  server-certs:\n`;
